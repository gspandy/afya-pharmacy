  ...
  <#list periods as period>
    ...
    <td<#if currentPeriod> class="current-period"<#else><#if (period.calendarEntries?size > 0)> class="active-period"</#if></#if>>
      ...
      <#if security.hasEntityPermission("HUMANRES", "_ADMIN", session)>
      	<a class="add-new" href='<@ofbizUrl>NewEvent?workEffortTypeId=EVENT&currentStatusId=CAL_TENTATIVE&estimatedStartDate=${period.start?string("yyyy-MM-dd HH:mm:ss")}&estimatedCompletionDate=${period.end?string("yyyy-MM-dd HH:mm:ss")}${addlParam?if_exists}</@ofbizUrl>'>${uiLabelMap.CommonAddNew}</a>
      </#if>
      <br class="clear"/>
      <#list period.calendarEntries as calEntry>
        <hr/>
        <#if calEntry.workEffort.estimatedStartDate?has_content>
		...
		<#if calEntry.workEffort.crmEventId?exists>
			<a href="<@ofbizUrl>ViewCrmEvent?crmEventId=${calEntry.workEffort.crmEventId?if_exists}</@ofbizUrl>">${calEntry.workEffort.workEffortName?default("Undefined")}</a>&nbsp;</td>
	    </#if>
