...
-->
  <div class="screenlet-title-bar">
    <ul>
      ...
      <li><a href="<@ofbizUrl>week?start=${next.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>">NextWeek</a></li>
      <li><a href="<@ofbizUrl>week?start=${nowTimestamp.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>">ThisWeek</a></li>
      <li><a href="<@ofbizUrl>week?start=${prev.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>">PreviousWeek</a></li>
    </ul>
    <br class="clear"/>
  </div>
...
<table cellspacing="0" class="basic-table calendar">              
  ...
  <tr<#if currentPeriod> class="current-period"<#else><#if (period.calendarEntries?size > 0)> class="active-period"</#if></#if>>
    ...
    <#else>
      ...
      <br/><#if calEntry.workEffort.trainingId?exists>
      			<a href="<@ofbizUrl>ViewTraining?trainingId=${calEntry.workEffort.trainingId}${addlParam?if_exists}</@ofbizUrl>" class="event">${calEntry.workEffort.workEffortName?default("Undefined")}</a>&nbsp;</td>
    	   </#if>
    </#if>
    </#list>
