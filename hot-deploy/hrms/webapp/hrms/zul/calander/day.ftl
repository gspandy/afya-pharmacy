...
-->
  <div class="screenlet-title-bar">
    <ul>
      ...
      <li><a href="<@ofbizUrl>day?start=${next.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>">NextDay</a></li>
      <li><a href="<@ofbizUrl>day?start=${nowTimestamp.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>">Today</a></li>
      <li><a href="<@ofbizUrl>day?start=${prev.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>">PreviousDay</a></li>
    </ul>
    <br class="clear"/>
  </div>
...
<table cellspacing="0" class="basic-table calendar">              
  <tr class="header-row">             
    ...
    <td colspan=${maxConcurrentEntries}>CalendarEntries</td>
  </tr>
  ...
  <tr<#if currentPeriod> class="current-period"<#else><#if (period.calendarEntries?size > 0)> class="active-period"</#if></#if>>
    <td class="label">
      ...
      <#if security.hasEntityPermission("HUMANRES", "_ADMIN", session)>
       <a  href="addTraining?workEffortTypeId=EVENT&currentStatusId=CAL_TENTATIVE&estimatedStartDate=${period.start?string("yyyy-MM-dd HH:mm:ss")}&estimatedCompletionDate=${period.end?string("yyyy-MM-dd HH:mm:ss")}${addlParam?if_exists}">${uiLabelMap.CommonAddNew}</a>
    	<#-- <a  href="<@ofbizUrl>EditEvent?workEffortTypeId=EVENT&currentStatusId=CAL_TENTATIVE&estimatedStartDate=${period.start?string("yyyy-MM-dd HH:mm:ss")}&estimatedCompletionDate=${period.end?string("yyyy-MM-dd HH:mm:ss")}${addlParam?if_exists}</@ofbizUrl>">${uiLabelMap.CommonAddNew}</a>-->
      </#if>
    </td>
    <#if security.hasEntityPermission("HUMANRES", "_ADMIN", session)>
    ...
    <br/><a href="<@ofbizUrl>ViewTraining?trainingId=${calEntry.workEffort.trainingId}${addlParam?if_exists}</@ofbizUrl>">${calEntry.workEffort.workEffortName?default("Undefined")}</a>&nbsp;</td>
    <#--<br/><a href="<@ofbizUrl>userViewTraining?trainingId=${calEntry.workEffort.trainingId}${addlParam?if_exists}</@ofbizUrl>">${calEntry.workEffort.workEffortName?default("Undefined")}</a>&nbsp;</td>-->
    </#if>
    </#list>
    <#else>
    <#list period.calendarEntries as calEntry>
