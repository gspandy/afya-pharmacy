...
height: auto;
  ...
  <div class="screenlet-title-bar">
    <ul>
      ...
      <li><a href='<@ofbizUrl>month?start=${next.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>'>NextMonth</a></li>
      <li><a href='<@ofbizUrl>month?start=${nowTimestamp.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>'>ThisMonth</a></li>
      <li><a href='<@ofbizUrl>month?start=${prev.time?string("#")}<#if eventsParam?has_content>&${eventsParam}</#if>${addlParam?if_exists}</@ofbizUrl>'>PreviousMonth</a></li>
    </ul>
    <br class="clear"/>
  </div>
...
<table cellspacing="0" class="basic-table calendar">              
  ...
  <#list periods as period>
    ...
    <td<#if currentPeriod> class="current-period"<#else><#if (period.calendarEntries?size > 0)> class="active-period"</#if></#if>>
      ...
      <#--<a class="add-new" href='<@ofbizUrl>EditEvent?workEffortTypeId=EVENT&currentStatusId=CAL_TENTATIVE&estimatedStartDate=${period.start?string("yyyy-MM-dd HH:mm:ss")}&estimatedCompletionDate=${period.end?string("yyyy-MM-dd HH:mm:ss")}${addlParam?if_exists}</@ofbizUrl>'>${uiLabelMap.CommonAddNew}</a>-->
      </#if>
      <br class="clear"/>
      <#list period.calendarEntries as calEntry>
        ...
        <br/>
        <#if calEntry.workEffort.trainingId?exists>
        	<a href="<@ofbizUrl>ViewTraining?trainingId=${calEntry.workEffort.trainingId}${addlParam?if_exists}</@ofbizUrl>" class="event">${calEntry.workEffort.workEffortName?default("Undefined")}</a>&nbsp;${calEntry.eventStatus?default("&nbsp;")}
        </#if>
        <br/>
      </#list>
    </td>
