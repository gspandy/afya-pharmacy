...
conditions = [];
EntityCondition cn1 = EntityCondition.makeCondition("varianceReasonId",EntityOperator.EQUALS,"ISSUE_EMPLOYEE");
EntityCondition cn2 = EntityCondition.makeCondition("varianceReasonId",EntityOperator.EQUALS,"RCV_EMPLOYEE");
...
for(GenericValue gv : physicalInventoryAndVarianceList){
	...
	GenericValue personGv = delegator.findOne("Person",false,UtilMisc.toMap("partyId", gv.getString("partyIdTo")));
	if(personGv != null)
		map.put("partyIdTo", personGv.getString("firstName") + " " + personGv.getString("lastName") );
	else
		map.put("partyIdTo", "" );
