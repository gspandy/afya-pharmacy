...
/** Select all records for logged in partyId and its subordinates **/ 
 ...
 dispatchCtx.entityName="MaxTerminationDetail";
 ...
 if (!isAdmin) { 			
		 ...
		 EntityCondition condition = new EntityConditionList(orConditionList,EntityOperator.OR);
		   ...
		   execCtx = new HashMap();
		   execCtx.entityName="MaxTerminationDetail";
		 ...
		 /** Admin should see only Terminations approved by Manager or those processed by him **/
		 statusList = [];
		 statusList.add("ET_MGR_REJECTED");
		 EntityCondition statusCond = EntityCondition.makeCondition("statusId",EntityOperator.NOT_IN, statusList);
