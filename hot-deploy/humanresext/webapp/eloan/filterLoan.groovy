...
/** Select all records for logged in partyId and its subordinates **/ 
 ...
 dispatchCtx.entityName="MaxELoanDetail";
 ...
 if (!isAdmin) { 			
		 ...
		 EntityCondition condition = new EntityConditionList(orConditionList,EntityOperator.OR);
		   ...
		   execCtx = new HashMap();
		   execCtx.entityName="MaxELoanDetail";
		 ...
		 /** Admin should see only ELoans approved by Manager or those processed by him **/
		 statusList = [];
		 statusList.add("EL_MGR_APPROVED");
		// statusList.add("EL_MGR_REJECTED");
		 statusList.add("EL_ADM_APPROVED");
		 statusList.add("EL_ADM_REJECTED");
