...
<div class="screenlet">
  ...
  <div class="screenlet-body">
    <#list searchConstraintStrings as searchConstraintString>
        <div>&nbsp;<a href="<@ofbizUrl>WorkEffortSearchResults?removeConstraint=${searchConstraintString_index}&amp;clearSearch=N</@ofbizUrl>" class="btn btn-primary">X</a>&nbsp;${searchConstraintString}</div>
    </#list>
    <div><span class="label">${uiLabelMap.CommonSortedBy}</span> ${searchSortOrderString}</div>
    <div><a href="<@ofbizUrl>WorkEffortSearchOptions</@ofbizUrl>" class="btn btn-primary">${uiLabelMap.CommonRefineSearch}</a></div>
    ...
    <center>
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <#list workEffortIds as workEffortId><#-- note that there is no boundary range because that is being done before the list is put in the content -->
          ...
          <tr>
            <td>
              <a href="<@ofbizUrl>EditWorkEffort?workEffortId=${workEffortId}</@ofbizUrl>" class="btn btn-link">${workEffortId} ${(workEffort.workEffortName)?if_exists}</a>
            </td>
          </tr>
        </#list>
      </table>
