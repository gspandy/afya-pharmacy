...
public class PaymentWorker {
    ...
    public static GenericValue getPaymentAddress(Delegator delegator, String partyId) {
        ...
        if (purpose != null) {
            try {
                ...
                Debug.logError(e,
                        "Trouble getting PostalAddress record for contactMechId: " + purpose.getString("contactMechId"), module);
            }
        }
            ...
            if (UtilValidate.isNotEmpty(paymentApplications)) {
                for (GenericValue paymentApplication : paymentApplications) {
                    ...
                    if (actual.equals(Boolean.FALSE) && paymentApplication.get("invoiceId") != null
                            && payment.get("actualCurrencyAmount") != null && payment.get("actualCurrencyUomId") != null) {
                        GenericValue invoice = paymentApplication.getRelatedOne("Invoice");
                        if (payment.getString("actualCurrencyUomId").equals(invoice.getString("currencyUomId"))) {
    ...
    public static BigDecimal getPaymentNotApplied(GenericValue payment) {
        BigDecimal tdsAmount = payment.getBigDecimal("tdsAmount");
        if (tdsAmount!=null && tdsAmount.compareTo(BigDecimal.ZERO)==1)
            return payment.getBigDecimal("amount").add(tdsAmount).subtract(getPaymentApplied(payment))
