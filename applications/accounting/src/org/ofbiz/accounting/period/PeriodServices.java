...
public class PeriodServices {
	...
	/*
	 * find the date of the last closed CustomTimePeriod, or, if none available,
	 * the earliest date available of any CustomTimePeriod
	 */
	public static Map<String, Object> findLastClosedDate(DispatchContext dctx, Map<String, ?> context) {
		Delegator delegator = dctx.getDelegator();
		String organizationPartyId = (String) context.get("organizationPartyId"); // input
																					// parameters
		...
		try {
			// try to get the ending date of the most recent accounting time
			// period before findDate which has been closed
			...
			List<GenericValue> closedTimePeriods = delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(findClosedConditions),
					UtilMisc.toSet("customTimePeriodId", "periodTypeId", "isClosed", "fromDate", "thruDate"), UtilMisc.toList("thruDate DESC"), null,
					false);
			...
			if ((closedTimePeriods != null) && (closedTimePeriods.size() > 0) && (closedTimePeriods.get(0).get("thruDate") != null)) {
				...
				// uh oh, no time periods have been closed? in that case, just
				// find the earliest beginning of a time period for this
				// organization
			...
			result.put("lastClosedTimePeriod", lastClosedTimePeriod); // ok if
																		// this
																		// is
																		// null
																		// - no
																		...
																		// been
																		// closed
			result.put("lastClosedDate", lastClosedDate); // should have a value
															// - not null
			return result;
	...
	public static String createCurrentCustomTimePeriod(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
		if(UtilValidate.isNotEmpty(request.getParameter("currentCustomTimePeriodId"))){
		GenericValue userLogin = (GenericValue) request.getSession().getAttribute("userLogin");
		GenericDelegator delegator = (GenericDelegator) userLogin.getDelegator();
		delegator.storeByCondition("PartyAcctgPreference", UtilMisc.toMap("currentCustomTimePeriod", request.getParameter("currentCustomTimePeriodId"),"partyId","Company"), EntityCondition.makeCondition("partyId", EntityOperator.EQUALS, "Company"));
