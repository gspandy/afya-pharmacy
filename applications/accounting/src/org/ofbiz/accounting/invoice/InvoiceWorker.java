...
public class InvoiceWorker {
     ...
     */
     ...
     public static BigDecimal getInvoiceTotal(GenericValue invoice, Boolean actualCurrency) {
        ...
        try {
            ...
            List invoiceItemTypes = getTaxableInvoiceItemTypeIds(invoice.getDelegator());
            invoiceItemTypes.add("PITM_DISCOUNT_ADJ");
            invoiceItems = EntityUtil.filterByAnd(
                    invoiceItems, UtilMisc.toList(
                            EntityCondition.makeCondition("invoiceItemTypeId", EntityOperator.NOT_IN,invoiceItemTypes )
                    ));
        } catch (GenericEntityException e) {
        ...
        if (invoiceItems != null) {
         ...
         try {
             itemBillings = invoice.getDelegator().findByAnd("OrderItemBilling", UtilMisc.toMap("invoiceId", invoice.getString("invoiceId")));
             if (itemBillings.size() > 0) {
                 GenericValue orderItemBilling = EntityUtil.getFirst(itemBillings);
                 GenericValue orderHeader = orderItemBilling.getRelatedOne("OrderHeader");
                 GenericValue productStore = orderHeader.getRelatedOne("ProductStore");
                 //Sales Order Scenario
                 if (productStore != null) {
                     if(!"Y".equalsIgnoreCase(productStore.getString("pricesIncludeTax"))){
                 ...
                 if("PURCHASE_ORDER".equals(orderHeader.getString("orderTypeId"))) {
                     ...
                     invoiceTotal=invoiceTotal.add(invoiceTaxTotal);
             }
         }catch(GenericEntityException e){
                 e.printStackTrace();
         }
    ...
    public static BigDecimal getDiscountAmount(GenericValue invoice){
        BigDecimal returnVal = BigDecimal.ZERO;
        try {
            List<GenericValue> items = invoice.getDelegator().findByAnd("InvoiceItem",UtilMisc.toMap("invoiceId",invoice.getString("invoiceId"),"invoiceItemTypeId","ITM_DISCOUNT_ADJ"),null);
            for(GenericValue item : items){
                returnVal=returnVal.add(item.getBigDecimal("amount").negate());
            }
        } catch (GenericEntityException e) {
            e.printStackTrace();
        }
