...
public class InvoiceServices {
    ...
    public static Map<String, Object> createInvoiceForOrderAllItems(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            Object invoiceDate = context.get("invoiceDate");
            if(invoiceDate!=null){
                context.put("eventDate",(Timestamp)invoiceDate);
            }
    ...
    public static Map<String, Object> createInvoiceForOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            if (productStore != null)
                createInvoiceContext.put("productStoreId", productStore.get("productStoreId"));
            ...
            if (billingAccount != null) {
                ...
                */
                ...
                if (UtilValidate.isNotEmpty(billingAccount.getString("contactMechId"))) {
                    Map<String, Object> createBillToContactMechContext = new HashMap<String, Object>();
                    createBillToContactMechContext.put("invoiceId", invoiceId);
                    createBillToContactMechContext.put("contactMechId", (String) billingAccount.getString("contactMechId"));
                    createBillToContactMechContext.put("contactMechPurposeTypeId", "BILLING_LOCATION");
                    createBillToContactMechContext.put("userLogin", userLogin);
                ...
                if (UtilValidate.isNotEmpty(billingLocations)) {
                    for (GenericValue ocm : billingLocations) {
                        Map<String, Object> createBillToContactMechContext = new HashMap<String, Object>();
                        createBillToContactMechContext.put("invoiceId", invoiceId);
                        createBillToContactMechContext.put("contactMechId", ocm.getString("contactMechId"));
                        createBillToContactMechContext.put("contactMechPurposeTypeId", "BILLING_LOCATION");
                        createBillToContactMechContext.put("userLogin", userLogin);
                        Map<String, Object> createBillToContactMechResult = dispatcher.runSync("createInvoiceContactMech", createBillToContactMechContext);
                        if (ServiceUtil.isError(createBillToContactMechResult)) {
            ...
            if (payToAddress != null) {
                Map<String, Object> createPayToContactMechContext = new HashMap<String, Object>();
                createPayToContactMechContext.put("invoiceId", invoiceId);
                createPayToContactMechContext.put("contactMechId", payToAddress.getString("contactMechId"));
                createPayToContactMechContext.put("contactMechPurposeTypeId", "PAYMENT_LOCATION");
                createPayToContactMechContext.put("userLogin", userLogin);
            ...
            for (GenericValue currentValue : billItems) {
                ...
                if (orderItem == null && itemIssuance != null) {
                    ...
                    GenericValue inventoryItem = shipmentReceipt.getRelatedOne("InventoryItem");
                    if (inventoryItem.getString("fixedAssetId") != null)
                        fixedAsset = delegator.findOne("FixedAsset", false, UtilMisc.toMap("fixedAssetId", inventoryItem.getString("fixedAssetId")));
                ...
                if ("RENTAL_ORDER_ITEM".equals(orderItem.getString("orderItemTypeId"))) {
                    List<GenericValue> workOrderItems = delegator.findByAnd("WorkOrderItemFulfillment", UtilMisc.toMap("orderId", orderItem.getString("orderId"), "orderItemSeqId", orderItem.getString("orderItemSeqId")));
                    ...
                    if (workEffort != null) {
                        BigDecimal days = workEffort.getBigDecimal("reservLength");
                        billingAmount = billingAmount.multiply(days).setScale(invoiceTypeDecimals, ROUNDING);
                    }
                }
                String overrideGlAccountId = orderItem.getString("overrideGlAccountId");
                if (fixedAsset != null) {
                    String fixedAssetTypeId = fixedAsset.getString("fixedAssetTypeId");
                    GenericValue fixedAssetTypeGlAccountGV = delegator.findOne("FixedAssetTypeGlAccount", UtilMisc.toMap("fixedAssetTypeId", fixedAssetTypeId, "fixedAssetId", "_NA_", "organizationPartyId", billToCustomerPartyId), false);
                    overrideGlAccountId = fixedAssetTypeGlAccountGV.getString("assetGlAccountId");
                }
                ...
                createInvoiceItemContext.put("overrideGlAccountId", overrideGlAccountId);
                ...
                if ("ItemIssuance".equals(currentValue.getEntityName())) {
                    ...
                    delegator.createOrStore(shipmentItemBilling);


                }

                //Added for handling the Purchase Variance. The Variance is recorded in the
                // ShipmentReceipt.
                BigDecimal variance = null;
                if (shipmentReceipt != null) {
                    variance = shipmentReceipt.getBigDecimal("quantityVariance");
                    if (variance != null && variance.signum() == 1) {
                        invoiceItemSeqNum++;
                        invoiceItemSeqId = UtilFormatOut.formatPaddedNumber(invoiceItemSeqNum, INVOICE_ITEM_SEQUENCE_ID_DIGITS);
                        createInvoiceItemContext.clear();
                        createInvoiceItemContext.put("invoiceId", invoiceId);
                        createInvoiceItemContext.put("invoiceItemSeqId", invoiceItemSeqId);
                        createInvoiceItemContext.put("invoiceItemTypeId", "PINV_PUR_VAR");
                        createInvoiceItemContext.put("description", orderItem.get("itemDescription"));
                        createInvoiceItemContext.put("quantity", variance);
                        if (orderItem == null)
                            orderItem = shipmentReceipt.getRelatedOne("OrderItem");
                        BigDecimal unitPrice = orderItem.getBigDecimal("unitPrice");
                        if (product == null)
                            product = shipmentReceipt.getRelatedOne("Product");
                        createInvoiceItemContext.put("amount", unitPrice);
                        createInvoiceItemContext.put("productId", product.getString("productId"));
                        createInvoiceItemContext.put("uomId", product.getString("quantityUomId"));
                        createInvoiceItemContext.put("userLogin", userLogin);
                        createInvoiceItemContext.put("taxableFlag", product.get("taxable"));
                        createInvoiceItemResult = dispatcher.runSync("createInvoiceItem", createInvoiceItemContext);

                        GenericValue orderItemBilling = delegator.makeValidValue("OrderItemBilling", UtilMisc.toMap("orderId", orderId, "orderItemSeqId",
                                orderItem.get("orderItemSeqId"), "invoiceId", invoiceId, "invoiceItemSeqId", invoiceItemSeqId, "shipmentReceiptId", shipmentReceipt.get("receiptId"),
                                "quantity", variance, "amount", unitPrice));
                        delegator.create(orderItemBilling);
                        if (ServiceUtil.isError(createInvoiceItemResult)) {
                            return ServiceUtil.returnError(UtilProperties.getMessage(resource, "AccountingErrorCreatingInvoiceItemFromOrder", locale), null, null, createInvoiceItemResult);
                        }
                    }
                ...
                for (GenericValue adj : itemAdjustments) {
                    ...
                    if (adj.get("amount") != null) {
                        ...
                        BigDecimal amountPerQty = adj.getBigDecimal("amount").divide(originalOrderItem.getBigDecimal("quantity"), 100, ROUNDING);
                        amount = amountPerQty.multiply(billingQuantity);
                        if (variance != null) {
                            amount = amount.add(amountPerQty.multiply(variance));
                        }
...
//                        amount = amount.setScale(invoiceTypeDecimals, ROUNDING);
                    ...
                    if (amount.signum() != 0) {
                        ...
                        BigDecimal totalDiscount = adj.getBigDecimal("amount");
                        BigDecimal computedDiscount = adjAlreadyInvoicedAmount.add(amount).setScale(TAX_DECIMALS, BigDecimal.ROUND_CEILING);
                        BigDecimal newAmount = null;
                        if (computedDiscount.compareTo(totalDiscount) > 1 || computedDiscount.compareTo(totalDiscount) == 0) {
                            newAmount = totalDiscount.subtract(adjAlreadyInvoicedAmount);
                        }
                        System.out.println(" *********** Recalculated the Order Adjustment Amount from " + amount + " to " + newAmount);
                        if (newAmount != null)
                            amount = newAmount;
            ...
            for (GenericValue adj : headerAdjustments) {
                ...
                if ("SHIPPING_CHARGES".equals(adj.getString("orderAdjustmentTypeId"))) {
                    ...
                    BigDecimal adjAmount = adj.getBigDecimal("amount").setScale(invoiceTypeDecimals, ROUNDING);
                    System.out.println(" Order Header Adjustments " + adjAmount);
                    //calcHeaderAdj(delegator, adj, invoiceType, invoiceId, invoiceItemSeqId,
                    ...
                    createInvoiceItemAdjContext.put("invoiceItemSeqId", invoiceItemSeqId);
                    createInvoiceItemAdjContext.put("invoiceItemTypeId", getInvoiceItemType(delegator, adj.getString("orderAdjustmentTypeId"), null, invoiceType, "INVOICE_ITM_ADJ"));
                    createInvoiceItemAdjContext.put("amount", adjAmount);
                    createInvoiceItemAdjContext.put("orderId", orderId);
                    createInvoiceItemAdjContext.put("overrideGlAccountId", adj.get("overrideGlAccountId"));
                    createInvoiceItemAdjContext.put("userLogin", userLogin);
                    // some adjustments fill out the comments field instead
                    String description = (UtilValidate.isEmpty(adj.getString("description")) ? adj.getString("comments") : adj.getString("description"));
                    ...
                    createOrderAdjustmentBillingContext.put("invoiceId", invoiceId);
                    createOrderAdjustmentBillingContext.put("invoiceItemSeqId", invoiceItemSeqId);
                    createOrderAdjustmentBillingContext.put("amount", adjAmount);
                    createOrderAdjustmentBillingContext.put("userLogin", userLogin);
...
//                            divisor, multiplier, baseAmount, TAX_DECIMALS, TAX_ROUNDING, userLogin, dispatcher, locale);
            ...
            if (!"N".equals(autoApproveInvoice)) {
                String nextStatusId = "PURCHASE_INVOICE".equals(invoiceType) ? "INVOICE_IN_PROCESS" : "INVOICE_IN_PROCESS";
            ...
            if("PURCHASE_ORDER".equals(orderHeader.getString("orderTypeId")) && "PRICEINCLUDETAX".equals(orderHeader.getString("formToIssue"))) {
                readjustInvoiceItems(delegator, invoiceId);
            }
    ...
    private static void readjustInvoiceItems(Delegator delegator, String invoiceId) {
        try {
            EntityCondition ec = EntityCondition.makeCondition("invoiceId",invoiceId);
                    List < GenericValue > allInvoiceItems = delegator.findList("InvoiceItem",ec, null, null, null, true);
            Map<String,BigDecimal> taxPerLine = new HashMap<String,BigDecimal>();

            List<GenericValue> adjustments = EntityUtil.filterByAnd(allInvoiceItems,UtilMisc.toList(EntityCondition.makeCondition("invoiceItemTypeId",EntityOperator.EQUALS,"PITM_DISCOUNT_ADJ")));
            ...
            List<GenericValue> invoiceItems = EntityUtil.filterByAnd(allInvoiceItems,UtilMisc.toList
                    (EntityCondition.makeCondition("invoiceItemTypeId",EntityOperator.NOT_EQUAL,"PITM_SALES_TAX"),
                            (EntityCondition.makeCondition("invoiceItemTypeId",EntityOperator.NOT_EQUAL,"PITM_DISCOUNT_ADJ"))));
            for(GenericValue invoiceItemGV : invoiceItems) {
                BigDecimal amount = invoiceItemGV.getBigDecimal("amount");
                BigDecimal quantity = invoiceItemGV.getBigDecimal("quantity");
                totalAmount=totalAmount.add(amount);
                totalQuantity=totalQuantity.add(quantity);
                amountMap.put(invoiceItemGV.getString("invoiceItemSeqId"), amount);
                quantityMap.put(invoiceItemGV.getString("invoiceItemSeqId"),quantity);
            }

            BigDecimal discountTotalAmount = BigDecimal.ZERO;
            List<GenericValue> discountItems = EntityUtil.filterByAnd(allInvoiceItems,UtilMisc.toList
                    (EntityCondition.makeCondition(
                            EntityCondition.makeCondition("invoiceItemTypeId",EntityOperator.EQUALS,"PITM_DISCOUNT_ADJ"))));
            for(GenericValue invoiceItemGV : discountItems) {
                BigDecimal amount = invoiceItemGV.getBigDecimal("amount");
                discountTotalAmount=discountTotalAmount.add(amount);
            }

            for(GenericValue adjItem : adjustments){
            ...
            for(GenericValue invoiceItemGV : invoiceItems) {
                BigDecimal amount = invoiceItemGV.getBigDecimal("amount");
                BigDecimal quantity = invoiceItemGV.getBigDecimal("quantity");
                BigDecimal lineAmount = amount.multiply(quantity);
                BigDecimal ratioByValue = amountMap.get(invoiceItemGV.getString("invoiceItemSeqId")).divide(totalAmount,2,BigDecimal.ROUND_HALF_UP);
                BigDecimal adjVal = ratioByValue.multiply(totalAdjustments);
                lineAmount=lineAmount.subtract(adjVal);
                amount = lineAmount.divide(quantity,6,BigDecimal.ROUND_HALF_UP).divide(new BigDecimal(1.16), 6, BigDecimal.ROUND_HALF_UP);
                BigDecimal amountAfterDiscount=amount.subtract(discountTotalAmount.divide(quantity, 6,BigDecimal.ROUND_HALF_UP));
                BigDecimal newLineAmount=lineAmount.subtract(amount.multiply(quantity));
                invoiceItemGV.set("amount", amountAfterDiscount);
                delegator.store(invoiceItemGV);
                taxPerLine.put(invoiceItemGV.getString("invoiceItemSeqId"),newLineAmount);
                discountTotalAmount=BigDecimal.ZERO;
            }

            List<GenericValue> taxItems = EntityUtil.filterByAnd(allInvoiceItems,UtilMisc.toMap("invoiceItemTypeId","PITM_SALES_TAX"));
            for(GenericValue taxGV : taxItems){
                taxGV.set("quantity", BigDecimal.ONE);
                taxGV.set("amount", taxPerLine.get(taxGV.getString("parentInvoiceItemSeqId")));
                delegator.store(taxGV);
            }
    ...
    public static Map<String, Object> createCommissionInvoices(DispatchContext dctx, Map<String, Object> context) {
        ...
        for (Map.Entry<String, List<Map<String, Object>>> commissionParty : commissionParties.entrySet()) {
            ...
            m.put("commissionInvoiceId", invoiceId);
            m.put("salesRepresentative ", partyIdBillFrom);
            invoicesCreated.add(m);
        }
    ...
    public static Map<String, Object> setInvoicesToReadyFromShipment(DispatchContext dctx, Map<String, Object> context) {
        ...
        /*
        Commented by Pradyumn as resolution to Bug:0010185
            Bug: First and Last Invoice for a Sales Shipment should be in In Process Status
                ...
                if (billAvail != null && billAvail.signum() == 1) { // this checks if billAvail is a positive non-zero number
                    if (issueQty != null && issueQty.compareTo(billAvail) > 0) {
                        ...
                        if ("ShipmentReceipt".equals(issue.getEntityName())) {
                            ...
                            //Pradyumna: Bill what is issued.Bulk Cement scenarios
                            issue.set("quantity", issueQty);
                        }
                        billAvail = ZERO;
                    } else {
    ...
    private static String getInvoiceItemType(Delegator delegator, String key1, String key2, String invoiceTypeId, String defaultValue) {
        ...
        try {
            ...
            //Override for RAW_MATERIAL ONLY
            //Bug:9744 SMR-HRMSMerged
            if (UtilValidate.isNotEmpty(key2) && "RAW_MATERIAL".equals(key2)) {
                itemMap = delegator.findByPrimaryKeyCache("InvoiceItemTypeMap", UtilMisc.toMap("invoiceItemMapKey", key2, "invoiceTypeId", invoiceTypeId));
            }
        } catch (GenericEntityException e) {
    ...
    public static Map<String,Object> createInvoiceFromReturnForPriceWithTax(DispatchContext dctx, Map<String, Object> context){
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        Locale locale = (Locale) context.get("locale");
        ...
        try {
            String invoiceTypeId = "CUST_RTN_INVOICE";
            String description = "Return Invoice for Customer Return #" + returnId;
...
// keep track of the invoice total vs the promised return total (how much the customer promised to return)
            ...
            int invoiceItemSeqNum = 1;
            String invoiceItemSeqId = UtilFormatOut.formatPaddedNumber(invoiceItemSeqNum, INVOICE_ITEM_SEQUENCE_ID_DIGITS);
            Map<GenericValue,BigDecimal> totalPerReturnItem = new HashMap<GenericValue,BigDecimal>();
            for (GenericValue item : billItems) {
                // we need the related return item and product
                GenericValue returnItem = item.getRelatedOneCache("ReturnItem");
                if (returnItem == null) continue;
                BigDecimal returnPrice = returnItem.getBigDecimal("returnPrice");
                BigDecimal quantity = item.getBigDecimal("quantityAccepted");
                totalPerReturnItem.put(item,returnPrice.multiply(quantity));
            }
            ...
            // Just to prevent NPE
                ...
                BigDecimal quantity = item.getBigDecimal("quantityAccepted");
                BigDecimal unitPrice = totalPerReturnItem.get(item).divide(new BigDecimal(1.16),6,ROUNDING).divide(quantity,6, ROUNDING)
                        .setScale(6, ROUNDING);
                ...
                BigDecimal taxAmount = totalPerReturnItem.get(item).subtract(quantity.multiply(unitPrice)).setScale(DECIMALS, ROUNDING);
                GenericValue adjustment = EntityUtil.getFirst(delegator.findByAnd("OrderAdjustment", "orderId", returnItem.getString("orderId"),
                        "orderAdjustmentTypeId","SALES_TAX"));

                if(adjustment!=null) {
                    input = new HashMap<String, Object>();
                    input.put("invoiceId", invoiceId);
                    input.put("invoiceItemTypeId", "CRT_SALES_TAX_ADJ");
                    input.put("quantity", BigDecimal.ONE);
                    input.put("invoiceItemSeqId", "" + invoiceItemSeqId); // turn the int into a string with ("" + int) hack
                    input.put("amount", taxAmount);
                    input.put("description", returnItem.get("description"));
                    input.put("productId", returnItem.get("productId"));
                    input.put("description", "Return Sales Tax");
                ...
                for (GenericValue otherAdjustment : adjustments) {
                    invoiceItemSeqNum += 1;
                    invoiceItemSeqId = UtilFormatOut.formatPaddedNumber(invoiceItemSeqNum, INVOICE_ITEM_SEQUENCE_ID_DIGITS);
                    if (otherAdjustment.get("amount") == null) {
                        Debug.logWarning("Return adjustment [" + otherAdjustment.get("returnAdjustmentId") + "] has null amount and will be skipped", module);
                        continue;
                    }

                    // determine invoice item type from the return item type
                    invoiceItemTypeId = getInvoiceItemType(delegator, otherAdjustment.getString("returnAdjustmentTypeId"), null, invoiceTypeId, null);
                    if (invoiceItemTypeId == null) {
                        return ServiceUtil.returnError(errorMsg + "No known invoice item type for the return adjustment type ["
                                + otherAdjustment.getString("returnAdjustmentTypeId") + "]");
                    }
                    BigDecimal amount = otherAdjustment.getBigDecimal("amount");
                    if (Debug.verboseOn()) {
                        Debug.logVerbose("Creating Invoice Item with amount " + otherAdjustment.getBigDecimal("amount") + " prorated to " + amount
                                + " for return adjustment [" + otherAdjustment.getString("returnAdjustmentId") + "]", module);
                    }
                    ...
                    input.put("invoiceItemSeqId", "" + invoiceItemSeqId); // turn the int into a string with ("" + int) hack
                    input.put("productId", returnItem.get("productId"));
                    input.put("description", otherAdjustment.get("description"));
                    input.put("overrideGlAccountId", otherAdjustment.get("overrideGlAccountId"));
                    input.put("parentInvoiceId", invoiceId);
                    input.put("parentInvoiceItemSeqId", parentInvoiceItemSeqId);
                    input.put("userLogin", userLogin);
    ...
    public static Map<String, Object> createInvoiceFromReturn(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            int invoiceItemSeqNum = 1;
            String invoiceItemSeqId = UtilFormatOut.formatPaddedNumber(invoiceItemSeqNum, INVOICE_ITEM_SEQUENCE_ID_DIGITS);
            String priceWithVat = "N";
            GenericValue firstBillItem = EntityUtil.getFirst(billItems);
            if("ShipmentReceipt".equals(firstBillItem.getEntityName()) && "CUST_RTN_INVOICE".equals(invoiceTypeId)){
                GenericValue returnItem = firstBillItem.getRelatedOneCache("ReturnItem");
                GenericValue orderHeader = returnItem.getRelatedOne("OrderHeader");
                GenericValue productStore = orderHeader.getRelatedOne("ProductStore");
                priceWithVat = productStore.getString("pricesIncludeTax");
                String currencyUom = UtilProperties.getPropertyValue("general.properties", "currency.uom.id.default", "ZMW");
                if("Y".equals(priceWithVat)&& !"H Form".equals(orderHeader.getString("formToIssue"))) {
                    boolean priceIncludeTax=true;
                    if(!currencyUom.equals(orderHeader.getString("currencyUom"))){
                        GenericValue adjustment = EntityUtil.getFirst(delegator.findByAnd("OrderAdjustment", "orderId", returnItem.getString("orderId"),
                                "orderAdjustmentTypeId","SALES_TAX"));
                        if(adjustment==null)
                    ...
                    if(priceIncludeTax) {
                        serviceResults = createInvoiceFromReturnForPriceWithTax(dctx, context);
                        Map<String, Object> results = ServiceUtil.returnSuccess();
                        results.put("invoiceId", serviceResults.get("invoiceId"));
                        return results;
                    }
                }
            }
        ...
        if (paymentId == null || paymentId.equals("")) {
            ...
            try {
                if (UtilAccounting.isReceipt(payment)) {
                    paymentApplyAvailable = paymentApplyAvailable.add(payment.getBigDecimal("tdsAmount"));
                }
            } catch (GenericEntityException e) {
