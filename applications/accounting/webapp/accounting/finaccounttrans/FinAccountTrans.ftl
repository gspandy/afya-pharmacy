        ...
        <div align="right">
            <#if grandTotal?exists>
                <#if glReconciliations?has_content>
                    ...
                    <input id="submitButton" type="submit" class="btn btn-success" onclick="javascript:document.selectAllForm.submit();" value="${uiLabelMap.AccountingAssignToReconciliation}" disabled="disabled" />
                <#else>
            ...
            <#else>
                <input id="submitButton" type="submit" class="btn btn-success" onclick="javascript:document.selectAllForm.submit();" value="${uiLabelMap.AccountingReconcile}" disabled="disabled" />
            </#if>
        </div>
    </#if>
...
<#-- Header Ends-->
    ...
    <td>
        ...
        <a href="<@ofbizUrl>DepositSlip.pdf?finAccountTransId=${finAccountTrans.finAccountTransId}</@ofbizUrl>" target="_BLANK" class="btn">${uiLabelMap.AccountingDepositSlip}</a>
        <#else>
    ...
    <td>${finAccountTrans.comments?if_exists}</td>
        ...
        <td>
            <#if finAccountTrans.statusId?has_content && finAccountTrans.statusId == 'FINACT_TRNS_CREATED'>
                <a href="javascript:document.cancelFinAccountTrans_${finAccountTrans.finAccountTransId}.submit();" class="btn btn-mini btn-danger">${uiLabelMap.CommonCancel}</a>
            </#if>
        </td>
        </#if>
    ...
    <input name="organizationPartyId_o_${finAccountTrans_index}" type="hidden" value="${defaultOrganizationPartyId}"/>
        ...
        <#if !(grandTotal?exists)>
            <#if (parameters.glReconciliationId?has_content && parameters.glReconciliationId != "_NA_")>
                ...
                <td><a href="javascript:document.removeFinAccountTransFromReconciliation_${finAccountTrans.finAccountTransId}.submit();" class="btn btn-mini btn-danger">${uiLabelMap.CommonRemove}</a></td>
                </#if>
            </#if>
        </#if>
        <#if ((glReconciliationId?has_content && glReconciliationId == "_NA_") && (glReconciliations?has_content && finAccountTransList?has_content)) || !grandTotal?exists>
