...
context.paymentAmount=payment.getBigDecimal("amount");
if(payment.getBigDecimal("tdsAmount")!=null){
	context.paymentAmount=payment.getBigDecimal("tdsAmount").add(payment.amount);
}

println " paymentAmount " + context.paymentAmount;
println " payment " + payment;

decimals = UtilNumber.getBigDecimalScale("invoice.decimals");
rounding = UtilNumber.getBigDecimalRoundingMode("invoice.rounding");

if(("CUSTOMER_REFUND" == payment.paymentTypeId && "CREDIT_MEMO" == payment.paymentMethodId) || ("DEBIT_MEMO" ==payment.paymentMethodId &&
        "DEBIT_NOTE" == payment.paymentTypeId)) {
    exprList = [EntityCondition.makeCondition("partyId", EntityOperator.EQUALS, payment.partyIdTo),
                EntityCondition.makeCondition("partyIdFrom", EntityOperator.EQUALS, payment.partyIdFrom)];
    println ' CREDIT MEMO Payment' ;

}else{//Incoming Payment
    println ' Other scenarios Payment' ;
