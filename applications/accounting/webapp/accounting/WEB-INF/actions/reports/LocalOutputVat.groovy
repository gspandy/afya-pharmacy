...
PreparedStatement statement = conn.prepareStatement("SELECT DISTINCT invoiceId,invoiceDate,invoiceTypeId,statusId,customerInvoiceNumber,customerPartyId,customerName,tPin,geo,country, \n" +
	"\t description,productName,currencyUomId,amountBeforeVat,vatAmount,(amountBeforeVat+vatAmount) AS totalAmount FROM \n" +
	"\t (SELECT IF(I.`INVOICE_TYPE_ID`='SALES_INVOICE',I.`PARTY_ID`,I.`PARTY_ID_FROM`) AS customerPartyId, \n" +
	"\t (SELECT GROUP_NAME FROM PARTY_GROUP PG WHERE PG.`PARTY_ID`=customerPartyId) AS customerName, \n" +
	"\t (SELECT VAT_TIN_NUMBER FROM PARTY_GROUP PGR WHERE PGR.`PARTY_ID`=customerPartyId) AS tPin, \n" +
	"\t I.`INVOICE_ID` AS invoiceId,I.`INVOICE_DATE` AS invoiceDate,I.`INVOICE_TYPE_ID` AS invoiceTypeId, \n" +
	"\t I.`STATUS_ID` AS statusId,I.`DESCRIPTION` AS description,I.`REFERENCE_NUMBER` AS customerInvoiceNumber, \n" +
	"\t (SELECT GROUP_CONCAT(DISTINCT internal_name SEPARATOR ' | ') FROM INVOICE_ITEM II JOIN PRODUCT P ON II.`PRODUCT_ID` = P.`PRODUCT_ID` \n" +
	"\t WHERE II.`INVOICE_ID` = I.`INVOICE_ID` AND II.`INVOICE_ITEM_TYPE_ID` NOT IN ('ITEM_SALES_TAX','INV_SALES_TAX','CRT_SALES_TAX_ADJ','CRT_EXCISE_TAX_ADJ') \n" +
	...
	"\t AND ATENT.`GL_ACCOUNT_ID` NOT IN ('9500000','8000000','1020106')) AS amountBeforeVat, \n" +
	...
	"\t WHERE PCMP.`CONTACT_MECH_PURPOSE_TYPE_ID` = 'GENERAL_LOCATION' AND PCMP.`THRU_DATE` IS NULL AND PCMP.`PARTY_ID` = customerPartyId GROUP BY PCMP.`PARTY_ID`) AS geo, \n" +
	...
	"\t JOIN INVOICE I ON AT.`INVOICE_ID` = I.`INVOICE_ID` \n" + dynamicQuery +
	"\t AND ATE.`GL_ACCOUNT_ID` NOT IN ('9500000','8000000')) T WHERE vatAmount IS NOT NULL AND geo = 'ZMB' \n" +
	"\t AND invoiceTypeId IN ('SALES_INVOICE','CUST_RTN_INVOICE') AND statusId IN ('INVOICE_READY','INVOICE_PAID');");
