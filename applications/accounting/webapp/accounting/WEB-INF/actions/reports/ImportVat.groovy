...
PreparedStatement statement = conn.prepareStatement("SELECT DISTINCT invoiceId,invoiceDate,invoiceTypeId,statusId,billOfEntry,supplierPartyId,supplierName,tPin,geo,country, \n" +
	...
	"\t (SELECT IF(I.PARTY_ID_FROM = 'ZRA001',I.`LINKED_SUPPLIER_ID`,I.`PARTY_ID_FROM`) AS supplierPartyId, \n" +
	...
	"\t WHERE II.`INVOICE_ID` = I.`LINKED_INVOICE_ID` AND II.`INVOICE_ITEM_TYPE_ID` NOT IN ('PITEM_SALES_TAX','PINV_SALES_TAX','SRT_SALES_TAX_ADJ','SRT_EXCISE_TAX_ADJ') \n" +
	"\t AND II.`PRODUCT_ID` IS NOT NULL GROUP BY II.`INVOICE_ID`) AS productName,ATE.`CURRENCY_UOM_ID` AS currencyUomId, \n" +
	"\t (SELECT COALESCE(SUM(ATENT.`AMOUNT`)) FROM ACCTG_TRANS_ENTRY ATENT JOIN ACCTG_TRANS ATS WHERE ATENT.`ACCTG_TRANS_ID` = ATS.`ACCTG_TRANS_ID` \n" +
	"\t AND ATENT.`GL_ACCOUNT_ID` NOT IN ('9500000','9000000','9010001','1020106','4240101') AND ATS.`INVOICE_ID` = I.`LINKED_INVOICE_ID`) AS itemSubTotal, \n" +
	"\t CASE WHEN ((SELECT COALESCE(SUM(ATENT.`AMOUNT`)) FROM ACCTG_TRANS_ENTRY ATENT JOIN ACCTG_TRANS ATS WHERE ATENT.`ACCTG_TRANS_ID` = ATS.`ACCTG_TRANS_ID` \n" +
	"\t AND ATENT.`GL_ACCOUNT_ID` = '1020106' AND ATS.`INVOICE_ID` = I.`LINKED_INVOICE_ID`) IS NULL) THEN 0.00 \n" +
	"\t ELSE (SELECT COALESCE(SUM(ATENT.`AMOUNT`)) FROM ACCTG_TRANS_ENTRY ATENT JOIN ACCTG_TRANS ATS WHERE ATENT.`ACCTG_TRANS_ID` = ATS.`ACCTG_TRANS_ID` \n" +
	"\t AND ATENT.`GL_ACCOUNT_ID` = '1020106' AND ATS.`INVOICE_ID` = I.`LINKED_INVOICE_ID`) END AS discountedTotal, \n" +
	"\t (SELECT COALESCE(SUM(amount)) FROM ACCTG_TRANS_ENTRY ACTE WHERE ACTE.`ACCTG_TRANS_ID` = ATE.`ACCTG_TRANS_ID`AND ACTE.`GL_ACCOUNT_ID` = '9500000') AS vatAmount, \n" +
	...
	"\t (SELECT G.`GEO_NAME` FROM GEO G WHERE G.`GEO_ID`=geo) AS country FROM ACCTG_TRANS_ENTRY ATE JOIN ACCTG_TRANS ATS ON ATE.`ACCTG_TRANS_ID` = ATS.`ACCTG_TRANS_ID` \n" +
	"\t JOIN INVOICE I ON ATS.`INVOICE_ID` = I.`INVOICE_ID` \n" + dynamicQuery +
	"\t AND I.PARTY_ID_FROM = 'ZRA001' AND I.`LINKED_INVOICE_ID` IS NOT NULL AND I.`LINKED_SUPPLIER_ID` IS NOT NULL) T \n" +
	"\t WHERE vatAmount IS NOT NULL AND geo <> 'ZMB' AND invoiceTypeId IN ('PURCHASE_INVOICE','PURC_RTN_INVOICE') AND statusId IN ('INVOICE_READY','INVOICE_PAID');");
