...
<div class="screenlet">
	...
	<form method="post" action="<@ofbizUrl>EditCustomTimePeriod</@ofbizUrl>" name="setOrganizationPartyIdForm" id="setOrganizationPartyIdForm">
...
<div class="screenlet">
	<div class="screenlet-title-bar">
		...
		<ul>
			...
			<li>
				<a href="<@ofbizUrl>EditCustomTimePeriod?findOrganizationPartyId=${findOrganizationPartyId?if_exists}</@ofbizUrl>">${uiLabelMap.CommonClearCurrent}</a>
			</li>
		</ul>
		<#else>
	...
	<form method="post" action="<@ofbizUrl>updateCustomTimePeriod</@ofbizUrl>" name="updateCustomTimePeriodForm">
		...
		<table class="basic-table" cellspacing="0">
			...
			<tr>
				...
				<td>
					<select name="parentPeriodId">
						...
						<option value='${allCustomTimePeriod.customTimePeriodId}'<#if isDefault> selected="selected"</#if>>
							...
							<#if allPeriodType?exists>${allPeriodType.description}:</#if>
					...
					<a
						...
						${uiLabelMap.CommonSetAsCurrent}
					</a>
					</#if>
				</td>
				<#-- <td>
					<input type="text" size='12' name="currentCustomTimePeriod" value="${currentCustomTimePeriod.organizationPartyId?if_exists}" />
				</td> -->
				<td>
				...
				<#-- <td>
					<input type="text" size='4' name="periodNum" value="${currentCustomTimePeriod.periodNum?if_exists}" />
				</td> -->
				<td>
					<input type="text" size='10' name="periodName" value="${currentCustomTimePeriod.periodName?if_exists}" />
				</td>
				<td>
...
<div class="screenlet">
	...
	<table class="basic-table" cellspacing="0">
		<tr class="header-row">
			...
			<td>${uiLabelMap.CommonFromDate}</td>
			<td>${uiLabelMap.CommonThruDate}</td>
			<td>${uiLabelMap.FormFieldTitle_isClosed}</td>
			<td style="text-align:center;">${uiLabelMap.FormFieldTitle_actionEnumId}</td>
		</tr>
		<#assign line = 0>
		...
		<tr>
			<form method="post" action='<@ofbizUrl>updateCustomTimePeriod</@ofbizUrl>' name='lineForm${line}'>
				...
				<#-- <td>
					<input type="text" size='12' name="organizationPartyId" value="${customTimePeriod.organizationPartyId?if_exists}" />
				</td> -->
				<td>
					<select name="periodTypeId">
						<#list periodTypes as periodType>
				...
				<#-- <td>
					<input type="text" size='4' name="periodNum" value="${customTimePeriod.periodNum?if_exists}" />
				</td> -->
				<td>
					<input type="text" size='10' name="periodName" value="${customTimePeriod.periodName?if_exists}" />
				</td>
				<td>
					<#assign hasntStarted = false>
					...
					<input type="text" size='13' name="fromDate" value="${customTimePeriod.fromDate?if_exists?string("yyyy-MM-dd")}"<#if hasntStarted> class="alert"</#if> />
				</td>
				<td>
					<#assign hasExpired = false>
					...
					<input type="text" size='13' name="thruDate" value="${customTimePeriod.thruDate?if_exists?string("yyyy-MM-dd")}"<#if hasExpired> class="alert"</#if> />
				</td>
				<td>
					<select name="isClosed">
						<#if (((customTimePeriod.isClosed)!"") == "Y")><option value="Y">${uiLabelMap.CommonYes}</option><option value="N">${uiLabelMap.CommonNo}</option></#if>
						<#if (((customTimePeriod.isClosed)!"") == "N")><option value="N">${uiLabelMap.CommonNo}</option><option value="Y">${uiLabelMap.CommonYes}</option></#if>
					</select>
				</td>
				<td class="button-col">
					<input type="submit" value='${uiLabelMap.CommonUpdate}' class="btn btn-success"/>
					...
					<a
						href='<@ofbizUrl>setCurrentCustomTimePeriod?currentCustomTimePeriodId=${customTimePeriod.customTimePeriodId?if_exists}&amp;findOrganizationPartyId=${findOrganizationPartyId?if_exists}</@ofbizUrl>'>
						${uiLabelMap.CommonSetAsCurrent}
					</a>
				</td>
			</form>
	...
	<div class="screenlet-body">
		<form method="post" action="<@ofbizUrl>createCustomTimePeriod</@ofbizUrl>" name="createCustomTimePeriodForm">
			...
			<div>
				...
				<@htmlTemplate.renderDateTimeField name="fromDate" value="${value!''}" className="date" alert="" title="Format: MM/dd/yyyy" size="15" maxlength="10" id="fromDate" dateType="date-time" shortDateInput=true timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName=""/>
				<span class="label">${uiLabelMap.CommonThruDate}</span>
				<#-- <input type="text" size='14' name='thruDate' /> -->
				<@htmlTemplate.renderDateTimeField name="thruDate" value="${value!''}" className="date" alert="" title="Format: MM/dd/yyyy" size="15" maxlength="10" id="thruDate" dateType="date-time" shortDateInput=true timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName=""/>
				<span class="label">Is Closed</span>
				<select name="isClosed">
					<option value="N" selected>${uiLabelMap.CommonNo}</option>
					<option value="Y">${uiLabelMap.CommonYes}</option>
				</select>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="${uiLabelMap.CommonAdd}" class="btn btn-success"/>
			</div>
		</form>
	</div>
</div>
