...
<div class="screenlet">
  ...
  <div class="screenlet-body">
    <table cellspacing="0" cellpadding="2" class="basic-table hover-bar">
      ...
      <#list items as item>
        <tr>
          <td><a href="/ordermgr/control/returnMain?returnId=${item.returnId}" class="btn-link">${item.returnId}</a> [${item.returnItemSeqId}]</td>
          <td><a href="/catalog/control/EditProductInventoryItems?productId=${item.productId}" class="btn-link">${item.productId}</a> ${item.internalName?if_exists}</td>
          ...
          <td>
            <#if item.issuedItems?has_content>
              <#list item.issuedItems as issuedItem>
                <div><a href="/facility/control/EditInventoryItem?inventoryItemId=${issuedItem.inventoryItemId}" class="btn-link">${issuedItem.inventoryItemId}</a> ${issuedItem.quantity}</div>
              </#list>
            </#if>
          </td>
            ...
            <td>
              <div>
                <form name="issueInventoryItemToShipment_${item_index}" action="<@ofbizUrl>issueInventoryItemToShipment</@ofbizUrl>" method="post">
                  ...
                  <span>
                   <#-- <@htmlTemplate.lookupField formName="issueInventoryItemToShipment_${item_index}" name="inventoryItemId" id="inventoryItemId" 
                    	fieldFormName="LookupInventoryItem?orderId=${item.orderId}&amp;partyId=${item.partyId}&amp;productId=${item.productId}"/> -->
                   <#-- Temporary Fix, We will in this later -->
                   <@htmlTemplate.lookupField formName="issueInventoryItemToShipment_${item_index}" name="inventoryItemId" id="inventoryItemId" 
                    	fieldFormName="LookupInventoryItems"/>
                  </span>
                  <input type="text" size="5" name="quantity"/>
                  <input type="submit" value="${uiLabelMap.CommonSubmit}" class="btn btn-primary"/>
                </form>
              </div>
            </td>
          </#if>
