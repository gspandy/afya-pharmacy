    ...
    <div class="screenlet">
      ...
      <div class="screenlet-body">
        ...
        <#if !(orderId?has_content)>
          <form name="selectOrderForm" method="post" action="<@ofbizUrl>WeightPackageOnly?mode=${requestParameters.mode}</@ofbizUrl>">
            ...
            <table cellspacing="0" class="basic-table">
              ...
              <tr>
                <td colspan="2">&nbsp;</td>
                <td colspan="2">
                  <a href="javascript:document.selectOrderForm.action='<@ofbizUrl>PackOrder?mode=${requestParameters.mode}</@ofbizUrl>';document.selectOrderForm.submit();" class="buttontext">${uiLabelMap.ProductPackOrder}</a>
          ...
          <form name="selectPicklistBinForm" method="post" action="<@ofbizUrl>WeightPackageOnly?mode=${requestParameters.mode}</@ofbizUrl>" style="margin: 0;">
            <input type="hidden" name="facilityId" value="${(facility.facilityId)?if_exists}" />
            <table cellspacing="0" class="basic-table">
              <tr>
              ...
              <tr>
                <td colspan="2">&nbsp;</td>
                <td colspan="1">
                  <a href="javascript:document.selectPicklistBinForm.action='<@ofbizUrl>PackOrder?mode=${requestParameters.mode}</@ofbizUrl>';document.selectPicklistBinForm.submit();" class="buttontext">${uiLabelMap.ProductPackOrder}</a>
        ...
        <#else>
          ...
          <#if !(shipmentPackages?has_content)>
                ...
                <#list packedLines as packedLine>
                  <form name="updateWeightPackageForm_${packedLine.getWeightPackageSeqId()}" method="post" action="<@ofbizUrl>updatePackedLine</@ofbizUrl>">
                    ...
                    <tr>
                      ...
                      <td align="right"><a href="javascript:document.updateWeightPackageForm_${packedLine.getWeightPackageSeqId()}.submit()" class="btn btn-success">${uiLabelMap.CommonUpdate}</a></td>
                      <td align="right"><a href="javascript:document.updateWeightPackageForm_${packedLine.getWeightPackageSeqId()}.action='<@ofbizUrl>deletePackedLine</@ofbizUrl>';document.updateWeightPackageForm_${packedLine.getWeightPackageSeqId()}.submit();" class="btn btn-danger">${uiLabelMap.CommonDelete}</a></div>
                    </tr>
                  </form>
                </#list>
              </table>
              <div align="right">
                <a href="javascript:document.completePackageForm.submit()" class="btn btn-success">${uiLabelMap.ProductComplete}</a>
              </div>
              <form name="completePackageForm" method ="post" action="<@ofbizUrl>completePackage?mode=${requestParameters.mode}</@ofbizUrl>">
                <input type="hidden" name="orderId" value="${orderId?if_exists}"/>
            ...
            <table class="basic-table" cellpadding="2" cellspacing='0'>
              <form name="weightPackageForm" method ="post" action="<@ofbizUrl>setPackageInfo?mode=${requestParameters.mode}</@ofbizUrl>">
                <input type="hidden" name = "shipGroupSeqId" value = "${shipGroupSeqId?if_exists}"/>
                <input type="hidden" name = "facilityId" value = "${(facility.facilityId)?if_exists}"/>
                <input type="hidden" name = "orderId" value = "${orderId?if_exists}"/>
                <#assign packedLines = weightPackageSession.getPackedLines(orderId)/>
          ...
          <#else>
            <table class="basic-table" cellpadding="2" cellspacing='0'> 
             <tr>
              ...
              <form name="completePackForm" method="post" action="<@ofbizUrl>shipNow?mode=${requestParameters.mode}</@ofbizUrl>">
                <input type="hidden" name="orderId" value="${orderId?if_exists}"/>
                <input type="hidden" name="shipGroupSeqId" value="${shipGroupSeqId?if_exists}"/>
                <input type="hidden" name="facilityId" value="${(facility.facilityId)?if_exists}"/>
                <input type="hidden" name="shipmentId" value="${(shipment.shipmentId)?if_exists}"/>
