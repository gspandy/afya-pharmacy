...
<#if security.hasEntityPermission("FACILITY", "_VIEW", session)>
    ...
    <#assign hideGrid = requestParameters.hideGrid?default("N")>
        <#assign shipment = "">
    <#if parameters.shipmentId?exists>
    ...
    <#if (requestParameters.forceComplete?has_content)>
        ...
        <#assign showInput = "Y">
              ...
              <form name="clearPackForm" method="post" action="<@ofbizUrl>ClearPackAllShipment?mode=${requestParameters.mode}</@ofbizUrl>">
            ...
            <form name="incPkgSeq" method="post" action="<@ofbizUrl>SetNextPackageSeq?mode=${requestParameters.mode}&amp;shipmentId=${parameters.shipmentId}</@ofbizUrl>">
              <input type="hidden" name="orderId" value="${orderId?if_exists}"/>
              <input type="hidden" name="shipGroupSeqId" value="${shipGroupSeqId?if_exists}"/>
              <input type="hidden" name="facilityId" value="${facilityId?if_exists}"/>
            </form>
            <form name="clearPackLineForm" method="post" action="<@ofbizUrl>ClearPackLineShipment?mode=${requestParameters.mode}</@ofbizUrl>">
    ...
    <div class="screenlet">
        ...
        <div class="screenlet-body">
              ...
              <#if showInput != "N">
                <hr />
                <form name="singlePackForm" method="post" action="<@ofbizUrl>ProcessPackOrderShipment?mode=${requestParameters.mode}&amp;shipmentId=${shipment}</@ofbizUrl>">
                  ...
                  <table cellpadding="2" cellspacing="0" class="basic-table">
                    <tr>
                      ...
                      <td>
                          ...
                          <input type="button" value="${uiLabelMap.ProductNextPackage}" onclick="javascript:document.incPkgSeq.submit();" class="buttontext"/>
                      </td>
                    </tr>
                  </table>
                </form>
              ...
              <#if showInput != "N" && hideGrid != "Y" && itemInfos?has_content>
                <br />
                <form name="multiPackForm" method="post" action="<@ofbizUrl>ProcessBulkPackOrderShipment?mode=${requestParameters.mode}&amp;shipmentId=${shipment}</@ofbizUrl>">
                  <input type="hidden" name="facilityId" value="${facilityId?if_exists}" />
                  <input type="hidden" name="orderId" value="${orderId?if_exists}" />
                  <input type="hidden" name="shipGroupSeqId" value="${shipGroupSeqId?if_exists}" />
                  ...
                  <table class="basic-table" cellspacing='0'>
                    <tr class="header-row">
                      ...
                      <td align="center">${uiLabelMap.ProductOrderedQuantity}</td>
                      ...
                      <td align="center">${uiLabelMap.ProductPackedWeight}</td>
                    ...
                    <#if (itemInfos?has_content)>
                      ...
                      <#-- <#list itemInfos as orderItem>  -->
                        ...
                        <#assign shippedQuantity = orderReadHelper.getItemToShipQuantity(orderItem,packingSession.shipmentId)?if_exists>
                        ...
                        -->
                        <#assign inputQty = shippedQuantity - packingSession.getPackedQuantity(orderId, orderItem.orderItemSeqId, shipGroupSeqId, itemInfo.productId)>
                        <#--<#assign inputQty = packingSession.getReservedQuantity(orderId, orderItem.orderItemSeqId, shipGroupSeqId)>-->
                        <tr>
                          ...
                          <td>
                              <a href="/catalog/control/EditProduct?productId=${orderProduct.productId?if_exists}${externalKeyParam}" class="btn-link" target="_blank">${(orderProduct.internalName)?if_exists}</a>
                              <#if orderProduct.productId != product.productId>
                          ...
                          <td align="center">${shippedQuantity?default(0)}</td>
                          <td align="center">${packingSession.getPackedQuantity(orderId, orderItem.orderItemSeqId, shipGroupSeqId, itemInfo.productId)}</td>
                          <td>
                          	<input type="text" name="machine_${rowKey}" />
                          </td>
                          <td align="center">
                            <input type="text" size="7" name="qty_${rowKey}" value="${inputQty}" readonly="readonly"/>
                          </td>
                          <td align="center">
                    ...
                    <tr>
                      <td colspan="12" align="right">
                        <input type="submit" value="${uiLabelMap.ProductPackItem}" class="buttontext"/>
                        &nbsp;
                        <input type="button" value="${uiLabelMap.CommonClear} (${uiLabelMap.CommonAll})" class="buttontext" onclick="javascript:document.clearPackForm.submit();"/>
                        &nbsp;
                     <a href="/facility/control/PackInShipment?facilityId=${parameters.facilityId?if_exists}&amp;mode=${requestParameters.mode}&amp;shipmentId=${parameters.shipmentId}" class="buttontext" >${uiLabelMap.CommonGoBack}</a>
                      </td>
              ...
              <!-- complete form -->
              <#if showInput != "N">
                <form name="completePackForm" method="post" action="<@ofbizUrl>CompletePackShipment?mode=${requestParameters.mode}</@ofbizUrl>">
                  <input type="hidden" name="orderId" value="${orderId?if_exists}"/>
                  <input type="hidden" name="shipGroupSeqId" value="${shipGroupSeqId?if_exists}"/>
                  <input type="hidden" name="facilityId" value="${facilityId?if_exists}"/>
                  ...
                  <table class="basic-table" cellpadding="2" cellspacing='0'>
                    <tr>
                        ...
                        <#if packageSeqIds?has_content>
                            <td>
                                <span class="label">${uiLabelMap.ProductPackedWeight}</span>
                                <br />
                      ...
                      <!-- Hiding AdditionalShippingCharge  <td nowrap="nowrap">
                      ...
                      <td align="right">
                        <div>
                          ...
                          <input type="button" value="${buttonName}" class="buttontext" onclick="javascript:document.completePackForm.submit();"/>
                            <#if parameters.externalKey?exists>
                                <a href="ViewShipment?shipmentId=${parameters.shipmentId}&amp;externalKey=${parameters.externalKey?if_exists}" class="buttontext" >View Shipment</a>
                            <#else>
                                <a href="ViewShipment?shipmentId=${parameters.shipmentId}" class="buttontext" >View Shipment</a>
                            </#if>
                        </div>
                      </td>
                    </tr>
              ...
              <#if packedLines?has_content>
                ...
                <table class="basic-table" cellspacing='0'>
                  <tr class="header-row">
                    ...
                    <td align="right">${uiLabelMap.ProductPackedWeight}&nbsp;(${uiLabelMap.ProductPackage})</td>
                  ...
                  <#list packedLines as line>
                    ...
                    <tr>
                      ...
                      <td>
                          <a href="/catalog/control/EditProduct?productId=${line.getProductId()?if_exists}${externalKeyParam}" class="btn-link" target="_blank">${product.internalName?if_exists?default("[N/A]")}</a>
                      </td>
                      <td>${line.getInventoryItemId()}</td>
                      <td>${line.getMachine()}</td>
