       ...
       <#if !(inventoryItem?exists)>
                 ...
                 <table cellspacing="0" class="basic-table">
                  <tr>
                   ...
                   <td>
                     ...
                     <input type="submit" value="${uiLabelMap.ProductGetItem}" class="btn btn-success"/>
                  </td>
                 </tr>
               </table>
            </form>
        ...
        <#else>
           ...
           <#if !(inventoryTransfer?exists)>
                <form method="post" id="transferform" action="<@ofbizUrl>CreateInventoryTransfer</@ofbizUrl>" name="transferform" style="margin: 0;">
            <#else>
            ...
            <tr>
                ...
                <td width="74%">
                    <#if inventoryItem?exists && (inventoryItem.productId)?exists>
                        <a href="/catalog/control/EditProduct?productId=${(inventoryItem.productId)?if_exists}" class="btn-link">${(inventoryItem.productId)?if_exists}</a>
                    </#if>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td width="6%">&nbsp;</td>
                <#if inventoryItem?exists && inventoryItem.inventoryItemTypeId?exists && inventoryItem.inventoryItemTypeId.equals("NON_SERIAL_INV_ITEM")>
                    <td width="74%">
                        ${(inventoryItem.availableToPromiseTotal)?if_exists}&nbsp;
                        /&nbsp;${(inventoryItem.quantityOnHandTotal)?if_exists}
                    </td>
                <#elseif inventoryItem?exists && inventoryItem.inventoryItemTypeId?exists && inventoryItem.inventoryItemTypeId.equals("SERIALIZED_INV_ITEM")>
        ...
        <tr>
           <td width="14%">&nbsp;</td>
            ...
            <td>
					  <@htmlTemplate.renderDateTimeField name="sendDate" value="${nowTimestamp}" className="" alert="" 
					                title="Format: yyyy-MM-dd HH:mm:ss.SSS" size="25" maxlength="30" id="sendDate" dateType="date" shortDateInput=false 
        ...
        <#if !(inventoryTransfer?exists)>
            <tr>
      <script language="JavaScript" type="text/javascript">
         function getFa(facTo,locationSeqIdTo) {
          var facTo = facTo.value;
} 
      </script>
                <td width="14%">&nbsp;</td>
                ...
                <td width="6%">&nbsp;</td>
                <td width="74%">
               <input type="label" name="lbl" id="lbl"  style="color:red;width:60%;display: none;font-weight:bold" value="Transfer To Facility is required"/>
                    <div>
                        <select name="facilityIdTo" id="facTo" onChange="javascript:getFa(facTo,locationSeqIdTo);">
                            <#list facilities as nextFacility>
                            <#if nextFacility.facilityId!=parameters.facilityId>
                            <option value="${nextFacility.facilityId}">${(nextFacility.facilityName)?if_exists} [${(nextFacility.facilityId)?if_exists}]</option>
                            </#if>
                            </#list>
                        </select>
           ...
           <#--   <tr>
                <td width="14%">&nbsp;</td>
                <td width="6%" align="right" nowrap="nowrap"><span class="label">Location</span></td>
                <td width="6%">&nbsp;</td>
                <td width="74%">
                     <div>
                        <select name="locationSeqIdTo" id="locationSeqIdTo" onChange="javascript:getFa(facTo,locationSeqIdTo);">
                          <#assign relatedLocations = delegator.findByAnd("FacilityLocation", {"facilityId" : parameters.facilityId})> 
                           <#list relatedLocations as relatedLocation>
                              <option value="${relatedLocation.locationSeqId}">${(relatedLocation.locationSeqId)?if_exists} [${(relatedLocation.locationSeqId)?if_exists}]</option>
                           </#list>
                        </select>
                        <br />
                    </div>
            ...
            <tr>
             ...
             <#--   <input type="text" name="comments"  maxlength="250" /> -->
                 <textarea cols="40" rows="4" name="comments" maxlength="250"></textarea>
                </td>
            </tr>
            <tr>
                <td width="14%">&nbsp;</td>
                ...
                <td width="74%">
                <#if inventoryItem?exists && inventoryItem.inventoryItemTypeId?exists && inventoryItem.inventoryItemTypeId.equals("NON_SERIAL_INV_ITEM")>
                    <input type="text"  size="5" name="xferQty" value="${(inventoryItem.quantityOnHandTotal)?if_exists}" />
                <#elseif inventoryItem?exists && inventoryItem.inventoryItemTypeId?exists && inventoryItem.inventoryItemTypeId.equals("SERIALIZED_INV_ITEM")>
        ...
        <tr>
            ...
            <#if !(inventoryTransfer?exists)>
                <td colspan="1"><input type="submit" value="${uiLabelMap.ProductTransfer}" class="btn btn-success" onClick="javascript:getFacility(facTo,lbl,locationSeqIdTo);" /></td>
            <#else>
                <td colspan="1"><input type="submit" value="${uiLabelMap.CommonUpdate}" class="btn btn-success"/></td>
            </#if>
        </tr>
        </table>
    <script language="JavaScript" type="text/javascript">
        function getFacility(facTo,lbl,locationSeqIdTo) {
           var facTo = facTo.value;
           var lbl = lbl.value;
           var locationSeqIdTo = locationSeqIdTo.value;
           var cform = document.transferform;
         
             if(facTo != ""){
               cform.action = "<@ofbizUrl>CreateInventoryTransfer</@ofbizUrl>";
               }
           else
              {
