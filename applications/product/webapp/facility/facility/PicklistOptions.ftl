...
<div class="screenlet">
  ...
  <form method="post" name="selectFactors" action="<@ofbizUrl>PicklistOptions?mode=${requestParameters.mode}</@ofbizUrl>">
  ...
  <!--<div class="screenlet-body">
    <div align ='right'>
      <a class="buttontext" href="<@ofbizUrl>ReviewOrdersNotPickedOrPacked?facilityId=${facilityId}?mode=${requestParameters.mode}</@ofbizUrl>">${uiLabelMap.FormFieldTitle_reviewOrdersNotPickedOrPacked}</a>
    </div> -->
        ...
        <tr>
          ...
          <td>
            <#if orderReadyToPickInfoList?has_content>
              <form method="post" action="<@ofbizUrl>createPicklistFromOrders?mode=${requestParameters.mode}</@ofbizUrl>">
                <input type="hidden" name="facilityId" value="${facilityId}"/>
                ...
                <input type="submit" value="${uiLabelMap.ProductCreatePicklist}" class="btn btn-success"/>
              </form>
            <#else>
              &nbsp;
            </#if>
      ...
      <#if pickMoveInfoList?has_content>
        ...
        <#list pickMoveInfoList as pickMoveInfo>
          ...
          <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                <td>
                    <form name="viewGroupDetail_${pickMoveInfo_index}" action="<@ofbizUrl>PicklistOptions?mode=${requestParameters.mode}</@ofbizUrl>" method="post">
              ...
              <#if ((requestParameters.groupByShippingMethod?exists && requestParameters.groupByShippingMethod == "Y") || (requestParameters.groupByWarehouseArea?exists && requestParameters.groupByWarehouseArea == "Y") || (requestParameters.groupByNoOfOrderItems?exists && requestParameters.groupByNoOfOrderItems == "Y"))>
                  <#if groupName1?has_content>
                    <a href="javascript:document.viewGroupDetail_${pickMoveInfo_index}.submit()" class="btn-link">${groupName1}</a>
                  </#if>
                </td>
                <td>
                  <#if groupName2?has_content>
                    <a href="javascript:document.viewGroupDetail_${pickMoveInfo_index}.submit()" class="btn-link">${groupName2}</a>
                  </#if>
                </td>
                <td>
                  <#if groupName3?has_content>
                    <a href="javascript:document.viewGroupDetail_${pickMoveInfo_index}.submit()" class="btn-link">${groupName3}</a></td>
                  </#if>
              <#else>
                  <a href="javascript:document.viewGroupDetail_${pickMoveInfo_index}.submit()" class="btn-link">${groupName?if_exists}</a>
              </#if>
                </td>
            <td>
              <#if !((requestParameters.groupByShippingMethod?exists && requestParameters.groupByShippingMethod == "Y") || (requestParameters.groupByWarehouseArea?exists && requestParameters.groupByWarehouseArea == "Y") || (requestParameters.groupByNoOfOrderItems?exists && requestParameters.groupByNoOfOrderItems == "Y"))>
            ...
            <td>
             <!-- <#if orderReadyToPickInfoList?has_content>
                <form method="post" action="<@ofbizUrl>printPickSheets?mode=${requestParameters.mode}</@ofbizUrl>">
      ...
      <table cellspacing="0" class="basic-table">
        ...
        <#list toPickList as toPick>
          ...
          <#list oiasgal as oiasga>
            ...
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              <td><a href="/ordermgr/control/orderview?orderId=${oiasga.orderId}${externalKeyParam}&amp;externalLoginKey=${externalLoginKey}" class="btn-link" target="_blank">${oiasga.orderId}</a></td>
