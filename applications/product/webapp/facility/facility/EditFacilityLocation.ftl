...
<#if facilityId?exists>
    ...
    <tr>
        ...
        <#if locationSeqId?exists>
          <td><input type="submit" value="${uiLabelMap.CommonUpdate}" class="btn btn-success" /></td>
        <#else>
          <td><input type="submit" value="${uiLabelMap.CommonSave}" class="btn btn-success" /></td>
        </#if>
    </tr>
  </table>
  </form>
  ...
  <div class="screenlet">
    ...
    <div class="screenlet-body">
        ...
        <tr class="header-row">
            ...
            <td>${uiLabelMap.FormFieldTitle_minimumStock}</td>
            <td>${uiLabelMap.FormFieldTitle_moveQuantity}</td>
        ...
        <#list productFacilityLocations?if_exists as productFacilityLocation>
            ...
            <tr>
                <td><#if product?exists>${(product.internalName)?if_exists}</#if>[${productFacilityLocation.productId}]</td>
                    <form method="post" action="<@ofbizUrl>updateProductFacilityLocation</@ofbizUrl>" id="lineForm_${productFacilityLocation_index}">
                        ...
                        <td><input type="text" size="10" name="minimumStock" value="${(productFacilityLocation.minimumStock)?if_exists}"/></td>
                        <td><input type="text" size="10" name="moveQuantity" value="${(productFacilityLocation.moveQuantity)?if_exists}"/></td>
                        <td><input type="submit" value="${uiLabelMap.CommonUpdate}" class="btn btn-success" /></td>
                        <td><a href="javascript:document.getElementById('lineForm_${productFacilityLocation_index}').action='/facility/control/deleteProductFacilityLocation';
                                document.getElementById('lineForm_${productFacilityLocation_index}').submit();" class="btn btn-mini btn-danger">${uiLabelMap.CommonDelete}</a></td>
                    </form>
            </tr>
        </#list>
        </table>
