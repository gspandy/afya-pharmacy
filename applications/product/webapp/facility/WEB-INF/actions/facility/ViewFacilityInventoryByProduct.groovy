...
if (action) {
    ...
    if (offsetQOHQty) {
        try {
            ...
            QOHQty = new BigDecimal(offsetQOHQty);
        } catch (NumberFormatException nfe) {
          request.setAttribute("_ERROR_MESSAGE_","Invalid QOH minus Min Stock less than ");
          return "error";
        }
    }
    if (offsetATPQty) {
        try {
            ...
            searchParameterString = searchParameterString + "&offsetATPQty=" + offsetATP;
             ATPQty = new BigDecimal(offsetATPQty);
        } catch (NumberFormatException nfe) {
                  request.setAttribute("_ERROR_MESSAGE_","Invalid ATP minus Min Stock less than ");
                            return "error";
    ...
    prods.each { oneProd ->
        ...
        if (resultMap) {
            oneInventory.totalAvailableToPromise = Double.parseDouble(resultMap.totalAvailableToPromise);
            oneInventory.totalQuantityOnHand = Double.parseDouble(resultMap.totalQuantityOnHand);
