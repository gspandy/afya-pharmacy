...
if (inventoryTransfer && inventoryTransfer.statusId) {
 ...
 if(!"IXF_COMPLETE".equals(parameters.statusId))
        {
		EntityCondition genericsCondition = EntityCondition.makeCondition( EntityCondition.makeCondition("statusTypeId", EntityOperator.EQUALS, "INVENTORY_XFER_STTS"),EntityOperator.AND, 
				 EntityCondition.makeCondition("statusId", EntityOperator.NOT_EQUAL, "IXF_CANCELLED") );
        statusItems = delegator.findList("StatusItem", genericsCondition, null, ['sequenceId'], null, false);
   }
   else
