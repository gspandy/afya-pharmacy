...
orderHeader = delegator.findOne("OrderHeader", [orderId : orderId], false);
	 if(orderHeader !=  null){
		if(!"SALES_ORDER".equals(orderHeader.orderTypeId) && "SALES_SHIPMENT".equals(shipment.getString("shipmentTypeId"))){
		request.setAttribute("_ERROR_MESSAGE_", "This is Sales Shipment");
		return "error";
		}
		else if(!"PURCHASE_ORDER".equals(orderHeader.orderTypeId) && "PURCHASE_SHIPMENT".equals(shipment.getString("shipmentTypeId"))){
		request.setAttribute("_ERROR_MESSAGE_", "This is purchase Shipment");
		return "error";
		}
		}
	}
...
if (orderItemShipGroupAssocs) {
    ...
    orderItemShipGroupAssocs.each { orderItemShipGroupAssoc ->
        ...
        if (shipGroupSeqId) {
            orderShipmentCondition = EntityCondition.makeCondition([orderId : orderItemShipGroupAssoc.orderId, orderItemSeqId : orderItemShipGroupAssoc.orderItemSeqId, shipGroupSeqId : orderItemShipGroupAssoc.shipGroupSeqId,shipmentId : shipment.shipmentId]);
        } else {
            orderShipmentCondition = EntityCondition.makeCondition([orderId : orderItemShipGroupAssoc.orderId, orderItemSeqId : orderItemShipGroupAssoc.orderItemSeqId,shipmentId : shipment.shipmentId]);
        }
        
        //plans = delegator.findList("OrderShipment", orderShipmentCondition, null, null, null, false);
        
        plans = delegator.findByAnd("OrderShipment", UtilMisc.toMap("orderId",orderItemShipGroupAssoc.orderId,"orderItemSeqId",orderItemShipGroupAssoc.orderItemSeqId));
