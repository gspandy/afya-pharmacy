...
if(orderId){
orderItemShip = delegator.findList("OrderItemShipGroup", EntityCondition.makeCondition([orderId : orderId]), null, null, null, false);
 GenericValue value = EntityUtil.getFirst(orderItemShip);
 maySplit = value.maySplit;
if ("Y".equals(maySplit)) {
        request.setAttribute("_ERROR_MESSAGE_", "Order is marked as Split. Cannot do Verify pick");
		return "error";
}
}
if(picklistBinId){
...
GenericValue gv = EntityUtil.getFirst(orderItemShip);
 maySplit = gv.maySplit;
if ("Y".equals(maySplit)) {
        request.setAttribute("_ERROR_MESSAGE_", "Order is marked as Split. Cannot do Verify pick");
		return "error";
}
...
if(picklistBinSplit.size()>0){
 ...
 order = orderValue.primaryOrderId;
