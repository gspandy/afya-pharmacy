...
if (shipmentId) {
	...
	orderItemShipgrp = delegator.findList("OrderItemShipGroup",EntityCondition.makeCondition([orderId : shipment.primaryOrderId]),null,null,null,false);
	if(orderItemShipgrp.size()>0){
	GenericValue genericValue = EntityUtil.getFirst(orderItemShipgrp);
	maySplit = genericValue.maySplit;
	if("N".equals(maySplit)) {
     if (shipment && shipment.primaryOrderId) {
   ...
   for(GenericValue items : shipmentItem){
    if (shipment && shipment.primaryOrderId) {
        orderItemBillingList = delegator.findList("OrderItemBilling", EntityCondition.makeCondition([orderId : shipment.primaryOrderId,quantity: items.quantity]), null, ['invoiceId'], null, false);
       GenericValue invValue = EntityUtil.getFirst(orderItemBillingList);
		invoiceId = invValue.invoiceId;
		List invoiceIds = [];
...
if(orderId){
orderShip = delegator.findList("OrderShipment", EntityCondition.makeCondition([orderId : orderId]), null, null, null, false);
if(orderShip.size()>0){
GenericValue value = EntityUtil.getFirst(orderShip);
 orderId = value.orderId;
orderItemShip = delegator.findList("OrderItemShipGroup",EntityCondition.makeCondition("orderId",EntityOperator.EQUALS,orderId),null,null,null,false);
GenericValue gValue = EntityUtil.getFirst(orderItemShip);
maySplit = gValue.maySplit;
if("N".equals(maySplit)) {
        request.setAttribute("_ERROR_MESSAGE_", "Orders are available in Shipment. Please Pack through Manual Shipment");
		return "error";
}
}
}
if(picklistBinId){
picklistBinSplit = delegator.findList("PicklistBin", EntityCondition.makeCondition([picklistBinId : picklistBinId]), null, null, null, false);
if(picklistBinSplit.size()>0){
 GenericValue value = EntityUtil.getFirst(picklistBinSplit);
 orderId = value.primaryOrderId;
 orderShip = delegator.findList("OrderShipment",EntityCondition.makeCondition("orderId",EntityOperator.EQUALS,orderId),null,null,null,false);
 if(orderShip.size()>0){
 GenericValue orderValue = EntityUtil.getFirst(orderShip);
 order = orderValue.orderId;
orderItemShip = delegator.findList("OrderItemShipGroup",EntityCondition.makeCondition("orderId",EntityOperator.EQUALS,order),null,null,null,false);
GenericValue generic = EntityUtil.getFirst(orderItemShip);
 maySplit = generic.maySplit;
 if("N".equals(maySplit)) {
        request.setAttribute("_ERROR_MESSAGE_", "Orders are available in Shipment. Please Pack through Manual Shipment");
		return "error";
}
...
if(orderId){
itemIssuance = delegator.findList("ItemIssuance", EntityCondition.makeCondition([orderId : orderId]), null, null, null, false);
if(itemIssuance.size()>0){
GenericValue value = EntityUtil.getFirst(itemIssuance);
 orderId = value.orderId;
orderItemShip = delegator.findList("OrderItemShipGroup",EntityCondition.makeCondition("orderId",EntityOperator.EQUALS,orderId),null,null,null,false);
GenericValue gValue = EntityUtil.getFirst(orderItemShip);
maySplit = gValue.maySplit;
...
if(picklistBinSplit.size()>0){
 GenericValue value = EntityUtil.getFirst(picklistBinSplit);
 orderId = value.primaryOrderId;
 itemIssuance = delegator.findList("ItemIssuance",EntityCondition.makeCondition("orderId",EntityOperator.EQUALS,orderId),null,null,null,false);
 if(itemIssuance.size()>0){
 GenericValue orderValue = EntityUtil.getFirst(itemIssuance);
 order = orderValue.orderId;
orderItemShip = delegator.findList("OrderItemShipGroup",EntityCondition.makeCondition("orderId",EntityOperator.EQUALS,order),null,null,null,false);
GenericValue generic = EntityUtil.getFirst(orderItemShip);
 maySplit = generic.maySplit;
