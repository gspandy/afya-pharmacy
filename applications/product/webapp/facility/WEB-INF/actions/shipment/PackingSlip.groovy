...
shipmentPackages.each { shipmentPackage ->
    ...
    contents.each { content ->
        ...
        List shipmentPackageList = delegator.findByAnd("ShipmentPackage", UtilMisc.toMap("shipmentId", shipmentItem.shipmentId, "shipmentPackageSeqId", content.shipmentPackageSeqId));
        line.packedWeight = UtilValidate.isNotEmpty(shipmentPackageList)? shipmentPackageList.get(0).weight : "";
        line.machine = UtilValidate.isNotEmpty(shipmentPackageList)? shipmentPackageList.get(0).machine : "";
        line.shipmentPackageSeqId = content.shipmentPackageSeqId.replaceFirst("^0+", "").trim();
        GenericValue uomGv = delegator.findOne("Uom", ["uomId":product.quantityUomId], false)
        line.uom = uomGv.description;
        ...
        context.quantityRequested = quantityRequestedByProduct.get(product.productId);
        context.quantityShipped = quantityShippedByProduct.get(product.productId);
    }
    packages.add(lines);
}
