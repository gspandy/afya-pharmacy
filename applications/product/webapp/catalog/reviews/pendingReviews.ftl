            ...
            <form method='post' action='<@ofbizUrl>updateProductReview</@ofbizUrl>' name="selectAllForm">
                ...
                <#else>
                  ...
                  <tr id="review_tableRow_${rowCount}" valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                      ...
                      <td>${review.getRelatedOne("Product").internalName?if_exists}<br /><a class="btn-link" href="<@ofbizUrl>EditProduct?productId=${review.productId}</@ofbizUrl>">${review.productId}</a></td>
                      <td>
