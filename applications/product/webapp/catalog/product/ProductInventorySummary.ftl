...
<div class="screenlet">
  <#if product?exists>
    <div class="screenlet-title-bar">
      <ul>
        <li class="h3">${uiLabelMap.ProductInventorySummary}</li>
        <#if parameters.showAllFacilities?exists>
          <li><a href="EditProductInventoryItems?productId=${productId}">${uiLabelMap.ProductShowProductFacilities}</a></li>
        <#else>
          <li><a href="EditProductInventoryItems?productId=${productId}&amp;showAllFacilities=Y">${uiLabelMap.ProductShowAllFacilities}</a></li>
        </#if>
    </div>
    <div class="screenlet-body">
        <table cellspacing="0" class="basic-table">
            <tr class="header-row">
                <td style="text-align:center"><b>${uiLabelMap.ProductFacility}</b></td>
            ...
            <#list quantitySummaryByFacility.values() as quantitySummary>
                <#if quantitySummary.facilityId?exists>
                    ...
                    <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                        <td><a href="/facility/control/ReceiveInventory?facilityId=${facilityId}&amp;productId=${productId}${externalKeyParam}&amp;externalLoginKey=${externalLoginKey}" class="btn">${uiLabelMap.ProductInventoryReceive}</a>
                        ${(facility.facilityName)?if_exists} [${facilityId?default("[No Facility]")}]</td>
