...
<form name="keywordsearchform" id="keywordSearchForm" method="post" action="<@ofbizUrl>keywordsearch?VIEW_SIZE=25&amp;PAGING=Y</@ofbizUrl>">
  <fieldset>
    ...
    <div>
      <input type="checkbox" name="SEARCH_CONTAINS" id="keywordSearchCointains" style="margin-bottom: 6px;" value="N" <#if requestParameters.SEARCH_CONTAINS?if_exists == "N">checked="checked"</#if> />
      <label style="display:inline" for="keywordSearchCointains">${uiLabelMap.CommonNoContains}</label>
      <td>&nbsp;</td>
      <input type="radio" name="SEARCH_OPERATOR" id="keywordSearchOperatorOr" style="margin-bottom: 6px;" value="OR" <#if requestParameters.SEARCH_OPERATOR?if_exists != "AND">checked="checked"</#if> />
      <label style="display:inline" for="keywordSearchOperatorOr">${uiLabelMap.CommonAny}</label>
      <td>&nbsp;</td>
      <input type="radio" name="SEARCH_OPERATOR" id="keywordSearchOperatorAnd" style="margin-bottom: 6px;" value="AND" <#if requestParameters.SEARCH_OPERATOR?if_exists == "AND">checked="checked"</#if> />
      <label style="display:inline" for="keywordSearchOperatorAnd">${uiLabelMap.CommonAll}</label>
      <td>&nbsp;</td>
    </div>
    <div>
      <input type="submit" name="find" value="${uiLabelMap.CommonFind}" class="btn btn-primary" />
    </div>
    </fieldset>
</form>
<form name="advancedsearchform" id="advancedSearchForm" method="post" action="<@ofbizUrl>advancedsearch</@ofbizUrl>">
  <fieldset>
    ...
    <div>
    <a href="javascript:document.getElementById('advancedSearchForm').submit()" class="btn btn-primary">${uiLabelMap.ProductAdvancedSearch}</a>
    </div>
  </fieldset>
</form>
