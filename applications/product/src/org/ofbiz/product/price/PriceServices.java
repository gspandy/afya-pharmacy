...
public class PriceServices {
    ...
    public static Map<String, Object> calculateProductPrice(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if (defaultPriceValue != null) {
            ...
            if ("ProductPrice".equals(defaultPriceValue.getEntityName()) && UtilValidate.isNotEmpty(defaultPriceValue.getString("customPriceCalcService"))) {
                ...
                if (UtilValidate.isNotEmpty(customMethod) && UtilValidate.isNotEmpty(customMethod.getString("customMethodName"))) {
                    ...
                    inMap.put("customerId", customerId);
        ...
        List<GenericValue> customerPrices = null;
		try {
			customerPrices = dispatcher.getDelegator().findByAnd("CustomerProduct", UtilMisc.toMap("partyId",partyId,"productId",productId));
			customerPrices=EntityUtil.filterByDate(customerPrices,new Timestamp(new java.util.Date().getTime()), "availableFromDate", "availableThruDate", true);
		} catch (GenericEntityException e2) {
			// TODO Auto-generated catch block
			e2.printStackTrace();
		}
		 if(UtilValidate.isNotEmpty(customerPrices)){
 			defaultPrice = EntityUtil.getFirst(customerPrices).getBigDecimal("lastPrice");
 		}
        
        if (defaultPrice!=null && listPrice == null) {
    ...
    public static Map<String, Object> addGeneralResults(Map<String, Object> result, GenericValue competitivePriceValue, GenericValue specialPromoPriceValue, GenericValue productStore,
        ...
        BigDecimal basePrice=(BigDecimal) result.get("price");
