...
public class VerifyPickServices {
    ...
    public static Map<String, Object> completeVerifiedPick(DispatchContext dctx, Map<String, ? extends Object> context) throws GenericEntityException {
       ...
       EntityCondition cond = EntityCondition.makeCondition("orderId", EntityOperator.EQUALS, orderId);
       List<GenericValue> orderShipment = delegator.findList("OrderShipment", cond, null, null, null, false);
    
        try {
        	if(orderShipment.size() == 0 ){
            shipmentId = pickSession.complete(orderId, locale);
        	}
        	else if(orderShipment.size() != 0){
        		pickSession.checkVerifiedQty(orderId, locale);
        		pickSession.issueItemsToShipment(orderShipment.get(0).getString("shipmentId"), locale);
        	}
