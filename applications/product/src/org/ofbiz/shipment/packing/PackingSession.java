...
import org.ofbiz.shipment.verify.VerifyPickSessionRow;
...
public class PackingSession implements java.io.Serializable {
    ...
    public void addOrIncreaseLine(String orderId, String orderItemSeqId, String shipGroupSeqId, String productId, BigDecimal quantity, int packageSeqId, BigDecimal weight, boolean update, String machine) throws GeneralException {
        ...
        if (reservations.size() == 1) {
            ...
            this.createPackLineItem(checkCode, res, orderId, orderItemSeqId, shipGroupSeqId, productId, quantity, weight, packageSeqId, machine);
        } else {
            ...
            if (qtyRemain.compareTo(BigDecimal.ZERO) == 0) {
                for (Map.Entry<GenericValue, BigDecimal> entry : toCreateMap.entrySet()) {
                    ...
                    this.createPackLineItem(2, res, orderId, orderItemSeqId, shipGroupSeqId, productId, qty, weight, packageSeqId, machine);
                }
            } else {
    ...
    public void addOrIncreaseLine(String orderId, String orderItemSeqId, String shipGroupSeqId, BigDecimal quantity, int packageSeqId, String machine) throws GeneralException {
        this.addOrIncreaseLine(orderId, orderItemSeqId, shipGroupSeqId, null, quantity, packageSeqId, BigDecimal.ZERO, false, machine);
    }

    public void addOrIncreaseLine(String productId, BigDecimal quantity, int packageSeqId, String machine) throws GeneralException {
        this.addOrIncreaseLine(null, null, null, productId, quantity, packageSeqId, BigDecimal.ZERO, false, machine);
    }
    ...
    protected void createPackLineItem(int checkCode, GenericValue res, String orderId, String orderItemSeqId, String shipGroupSeqId, String productId, BigDecimal quantity, BigDecimal weight, int packageSeqId, String machine) throws GeneralException {
        ...
        switch (checkCode) {
            ...
            case 2:
                ...
                packLines.add(new PackingSessionLine(orderId, orderItemSeqId, shipGroupSeqId, productId, invItemId, quantity, weight, packageSeqId, machine));
                break;
        }
    ...
    public BigDecimal getReservedQuantity(String orderId, String orderItemSeqId, String shipGroupSeqId) throws GenericEntityException{
    	Map<String, Object> invLookup = FastMap.newInstance();
    	invLookup.put("orderId", orderId);
    	invLookup.put("orderItemSeqId", orderItemSeqId);
    	invLookup.put("shipGroupSeqId", shipGroupSeqId);
    	invLookup.put("shipmentId", this.shipmentId);
    	List<GenericValue> reservations = this.getDelegator().findByAnd("OrderItemShipGrpInvRes", invLookup, UtilMisc.toList("quantity DESC"));
        BigDecimal total = BigDecimal.ZERO;
    	for(GenericValue reservation : reservations) {
            total=total.add(reservation.getBigDecimal("quantity"));
    	}
    	return total;
    }
    ...
    protected void checkReservations(boolean ignore) throws GeneralException {
        ...
        for (PackingSessionLine line : this.getLines()) {
            ...
            if (ignore && packedQty.compareTo(reservedQty) != 0) {
                BigDecimal cancelQuantity = reservedQty.subtract(packedQty);
                List<GenericValue> itemIssuanceGvs = getItemIssuances(line.getOrderId(), line.getOrderItemSeqId(), line.getShipGroupSeqId());
                for (GenericValue itemIssuanceGv: itemIssuanceGvs){
                    _dispatcher.runSync("cancelOrderItemIssuanceFromSalesShipment",UtilMisc.toMap("itemIssuanceId",itemIssuanceGv.getString("itemIssuanceId"),"cancelQuantity",cancelQuantity,"userLogin",userLogin));
                }
                //cancelOrderItemIssuanceFromSalesShipment
            }
        }
    ...
    protected void issueItemToShipment() throws GeneralException {
        this.getShipmentId();
        List<PackingSessionLine> processedLines = FastList.newInstance();
        for (PackingSessionLine line : this.getLines()) {
            if (this.checkLine(processedLines, line)) {
    ...
    public void createPackage() throws GeneralException {
        this.getShipmentId();
        for (int i = 0; i < packageSeq; i++) {
            String shipmentPackageSeqId = UtilFormatOut.formatPaddedNumber(i + 1, 5);
            ...
            pkgCtx.put("weight", getPackageWeight(i + 1));
            pkgCtx.put("weightUomId", getWeightUomId());
            pkgCtx.put("machine", packLines.get(i).getMachine());
            pkgCtx.put("userLogin", userLogin);
            Map<String, Object> newPkgResp = this.getDispatcher().runSync("createShipmentPackage", pkgCtx);
    ...
    protected void applyItemToPackages() throws GeneralException {
        this.getShipmentId();
        for (PackingSessionLine line : this.getLines()) {
            line.applyLineToPackage(shipmentId, userLogin, getDispatcher());
        }
    ...
    protected void updateShipmentRouteSegment() throws GeneralException {
        this.getShipmentId();
        BigDecimal shipmentWeight = getTotalWeight();
        if (shipmentWeight.compareTo(BigDecimal.ZERO) <= 0) return;
        List<GenericValue> shipmentRouteSegments = getDelegator().findByAnd("ShipmentRouteSegment", UtilMisc.toMap("shipmentId", this.getShipmentId()));
    ...
    protected void setShipmentToPack() throws GeneralException {
        this.getShipmentId();
        Map<String, Object> packedCtx = UtilMisc.toMap("shipmentId", shipmentId, "statusId", "SHIPMENT_PACKED", "userLogin", userLogin);
        Map<String, Object> packedResp = this.getDispatcher().runSync("updateShipment", packedCtx);
        if (packedResp != null && ServiceUtil.isError(packedResp)) {
