...
public class ProductionRunServices {
    ...
    public static Map cancelProductionRun(DispatchContext ctx, Map context) {
        ...
        List<GenericValue> acctgRecords = new ArrayList();
        ...
        if (currentStatusId.equals("PRUN_CREATED") || currentStatusId.equals("PRUN_DOC_PRINTED") || currentStatusId.equals("PRUN_SCHEDULED")
                || currentStatusId.equals("PRUN_COMPLETED")) {
            try {
                ...
                acctgRecords.addAll(delegator.findList("AcctgTrans", EntityConditionList.makeCondition(UtilMisc.toList(EntityCondition.makeCondition("workEffortId", EntityOperator.EQUALS, productionRunId),
                        EntityCondition.makeCondition("isPosted", EntityOperator.NOT_EQUAL, "Y"))),null,null,null,false));
                ...
                for (int i = 0; i < tasks.size(); i++) {
                    ...
                    acctgRecords.addAll(delegator.findList("AcctgTrans", EntityConditionList.makeCondition(UtilMisc.toList(EntityCondition.makeCondition("workEffortId",
...
//                    List components = delegator.findByAnd("WorkEffortGoodStandard", UtilMisc.toMap("workEffortId", taskId, "workEffortGoodStdTypeId", "PRUNT_PROD_NEEDED", "statusId", "WEGS_CREATED"));
                    List components = delegator.findByAnd("WorkEffortGoodStandard", UtilMisc.toMap("workEffortId", taskId, "workEffortGoodStdTypeId", "PRUNT_PROD_NEEDED"));
                ...
                //Pradyumna: Not sure how to handle the cancellation of Production Run that is related to an Order Item Fulfillment

                List<GenericValue> records = null;
                if(currentStatusId.equals("PRUN_COMPLETED")) {

                    try {
                        for (int i = 0; i < tasks.size(); i++) {
                            oneTask = (GenericValue) tasks.get(i);
                            taskId = oneTask.getString("workEffortId");
                            records = delegator.findByAnd("WorkEffortInventoryAssign", UtilMisc.toMap("workEffortId", taskId));
                            serviceContext.clear();
                            serviceContext.put("userLogin", userLogin);

                            for (GenericValue inv : records) {
                                serviceContext.put("inventoryItemId", inv.getString("inventoryItemId"));
                                serviceContext.put("availableToPromiseDiff", inv.getBigDecimal("quantity"));
                                serviceContext.put("quantityOnHandDiff", inv.getBigDecimal("quantity"));
                                serviceContext.put("workEffortId", taskId);
                                serviceContext.put("description","Production Run Task cancelled");
                                dispatcher.runSync("createInventoryItemDetail", serviceContext);
                                inv.remove();
                            }
                            //Check if any Stoppages or Electric Consumption
                            delegator.removeByAnd("FixedAssetMeter",UtilMisc.toMap("workEffortId", taskId));
                            delegator.removeByAnd("WorkEffortFixedAssetAssign",UtilMisc.toMap("workEffortId", taskId));
                            delegator.removeByAnd("CostComponent",UtilMisc.toMap("workEffortId", taskId));
                        }
                            records = delegator.findByAnd("WorkEffortInventoryProduced", UtilMisc.toMap("workEffortId", productionRunId));
                            serviceContext.clear();
                            serviceContext.put("userLogin", userLogin);
                            for (GenericValue invProduced : records) {
                                List<GenericValue> inventoryDetails = delegator.findByAnd("InventoryItemDetail",
                                        UtilMisc.toMap("workEffortId", productionRunId));

                                for(GenericValue invDetail : inventoryDetails){
                                    serviceContext.put("inventoryItemId", invDetail.getString("inventoryItemId"));
                                    serviceContext.put("availableToPromiseDiff", invDetail.getBigDecimal("availableToPromiseDiff").negate());
                                    serviceContext.put("quantityOnHandDiff", invDetail.getBigDecimal("quantityOnHandDiff").negate());
                                    serviceContext.put("workEffortId", productionRunId);
                                    serviceContext.put("description","Production Run cancelled");
                                    dispatcher.runSync("createInventoryItemDetail",serviceContext);
                                }
                                invProduced.remove();
                            }
                        ...
                        serviceContext.clear();
                        serviceContext.put("userLogin", userLogin);
                        for (GenericValue acctgTrans : acctgRecords) {
                            serviceContext.put("fromAcctgTransId", acctgTrans.getString("acctgTransId"));
                            serviceContext.put("revert", "Y");
                            dispatcher.runSync("copyAcctgTransAndEntries",serviceContext);
                        }
                        //Check if any Stoppages or Electric Consumption
                        delegator.removeByAnd("FixedAssetMeter",UtilMisc.toMap("workEffortId", productionRunId));
                        delegator.removeByAnd("WorkEffortFixedAssetAssign",UtilMisc.toMap("workEffortId", productionRunId));
    ...
    public static Map createProductionRun(DispatchContext ctx, Map context) {
        ...
        if (workEffortName == null) {
            ...
            workEffortName = prdName + "-" + wefName + "-" + UtilDateTime.nowDateString();
        }
    ...
    public static Map changeProductionRunStatus(DispatchContext ctx, Map context) {
        ...
        if (currentStatusId.equals("PRUN_DOC_PRINTED") && (statusId == null || statusId.equals("PRUN_RUNNING"))) {
            ...
            if (productionRun.getEstimatedStartDate() == null)
                serviceContext.put("actualStartDate", UtilDateTime.nowTimestamp());
            else
                serviceContext.put("actualStartDate", productionRun.getEstimatedStartDate());
        ...
        if (currentStatusId.equals("PRUN_RUNNING") && (statusId == null || statusId.equals("PRUN_COMPLETED"))) {
            ...
            if (productionRun.getEstimatedCompletionDate() == null)
                serviceContext.put("actualCompletionDate", UtilDateTime.nowTimestamp());
            else
                serviceContext.put("actualCompletionDate", productionRun.recalculateEstimatedCompletionDate());
            serviceContext.put("userLogin", userLogin);
            Map resultService = null;
            try {
                resultService = dispatcher.runSync("updateWorkEffort", serviceContext);
    ...
    public static Map getWorkEffortCosts(DispatchContext ctx, Map context) {
        ...
        String costCompoTypeIdPrefix = (String) context.get("costCompoTypeIdPrefix");
        if (UtilValidate.isEmpty(costCompoTypeIdPrefix))
            costCompoTypeIdPrefix = "ACTUAL_";
        ...
        try {
            ...
            List<EntityExpr> conds = UtilMisc.toList(EntityCondition.makeCondition("workEffortId", EntityOperator.EQUALS, workEffortId), EntityCondition.makeCondition("costComponentTypeId", EntityOperator.LIKE, costCompoTypeIdPrefix + "%"));

            EntityCondition entityCondition = EntityCondition.makeCondition(conds);
            List costComponents = EntityUtil.filterByDate(delegator.findList("CostComponent", entityCondition, null, null, null, false));
            ...
            while (costComponentsIt.hasNext()) {
                ...
                if (!((costCompoTypeIdPrefix + "MAT_COST").equals(costComponent.getString("costComponentTypeId")))) {
    ...
    public static Map getProductionRunCost(DispatchContext ctx, Map context) {
        ...
        try {
            ...
            BigDecimal totalCost = ZERO;
            Map outputMap = dispatcher.runSync("getWorkEffortCosts", UtilMisc.<String, Object>toMap("userLogin", userLogin, "workEffortId", workEffortId, "costCompoTypeIdPrefix", costCompoTypeIdPrefix));
            ...
            while (tasksIt.hasNext()) {
                ...
                outputMap = dispatcher.runSync("getWorkEffortCosts", UtilMisc.<String, Object>toMap("userLogin", userLogin, "workEffortId", task.getString("workEffortId"), "costCompoTypeIdPrefix", costCompoTypeIdPrefix));
    ...
    public static Map<String, Object> createProductionRunTaskCosts(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            try {
                createMaterialCost(dispatcher, productionRunTaskId, userLogin, locale);
            } catch (Throwable e) {
    ...
    private static void createMaterialCost(LocalDispatcher dispatcher, String productionRunTaskId, GenericValue userLogin, Locale locale) throws Throwable {
        ...
        if ("SERIALIZED_INV_ITEM".equals(inventoryItemTypeId)) {
            try {
                ...
                for (int i = 0; i < numOfItems; i++) {
                    ...
                    serviceContext.put("facilityId", productionRun.getGenericValue().getString("facilityId"));
                    serviceContext.put("datetimeReceived", datetimeReceived);
                    serviceContext.put("datetimeManufactured", datetimeReceived);
    ...
    public static Map productionRunDeclareAndProduce(DispatchContext ctx, Map context) {
        ...
        ProductionRun productionRun = new ProductionRun(productionRunId, delegator, dispatcher);
        String currentStatusId = productionRun.getGenericValue().getString("currentStatusId");
        if ("PRUN_CANCELLED".equals(currentStatusId)) {
            return ServiceUtil.returnError("Production Run is cancelled. Cannot declare and do Stock in.");
        }
    ...
    public static Map productionRunTaskReturnMaterial(DispatchContext ctx, Map context) {
        ...
        try {
            List<GenericValue> records = delegator.findByAnd("WorkEffortAndInventoryAssign", UtilMisc.toMap("workEffortId", productionRunTaskId, "productId", productId));
            Iterator issuances = records.iterator();
           ...
           /* Iterator returns = (delegator.findByAnd("WorkEffortAndInventoryProduced", UtilMisc.toMap("workEffortId", productionRunTaskId, "productId", productId))).iterator();
            ...
            if (quantity.compareTo(totalIssued) > 0) {
                return ServiceUtil.returnError("Production Run Task with id [" + productionRunTaskId + "] cannot return more items [" + quantity + "] than the ones currently allocated" +
                        " [" + (totalIssued) + "]");
            }

            List<GenericValue> storeList = new ArrayList<GenericValue>();
            issuances = records.iterator();

            // Creating a Map to capture the quantity and the inventory that has to be returned.
            Map<GenericValue, BigDecimal> quantityRateMap = new HashMap<GenericValue, BigDecimal>();
            while (issuances.hasNext()) {
                GenericValue issuance = (GenericValue) issuances.next();
                String inventoryItemId = issuance.getString("inventoryItemId");
                List<GenericValue> inventories = delegator.findByAnd("WorkEffortInventoryAssign",
                        UtilMisc.toMap("workEffortId", productionRunTaskId, "inventoryItemId", inventoryItemId));
                for (GenericValue inventory : inventories) {
                    BigDecimal issued = issuance.getBigDecimal("quantity");
                    BigDecimal returnQty = null;
                    if (issued.compareTo(quantity) ==1) {
                        returnQty = issued.subtract(quantity);
                        inventory.set("quantity", returnQty.doubleValue());
                        storeList.add(inventory);
                        quantityRateMap.put(inventory, quantity);
                        break;
                    } else {
                        inventory.set("quantity", BigDecimal.ZERO.doubleValue());
                        storeList.add(inventory);
                        quantityRateMap.put(inventory, issued);
                        quantity = quantity.subtract(issued);
                    }
                }
            }
            ...
            ProductionRun productionRun = new ProductionRun(productionRunTaskId, delegator, dispatcher);
            List<GenericValue> productionRunTasks = productionRun.getProductionRunRoutingTasks();
            for (GenericValue each : productionRunTasks) {
                delegator.removeByAnd("CostComponent", UtilMisc.toMap("workEffortId", each.getString("workEffortId"), "costComponentTypeId", "ACTUAL_MAT_COST"));
            }
            try {
                createMaterialCost(dispatcher, productionRunTaskId, userLogin, locale);
            } catch (Throwable t) {
            ...
            for (GenericValue inv : quantityRateMap.keySet()) {
                GenericValue inventory = delegator.findOne("InventoryItem", true, "inventoryItemId", inv.get("inventoryItemId"));
                Map inventoryResult = dispatcher.runSync("productionRunTaskProduce", UtilMisc.<String, Object>toMap("workEffortId", productionRunTaskId,
                        "productId", productId,
                        "unitCost", inventory.getBigDecimal("unitCost"),
                        "currencyUomId", inventory.getString("currencyUomId"),
                        "quantity", quantityRateMap.get(inv),
    ...
    public static Map approveRequirement(DispatchContext ctx, Map context) {
        ...
        try {
            ...
            if (requirementTypeId.equals("PRODUCT_REQUIREMENT")) {
                boolean isValidSupplier = checkIfValidSupplierExistsForRequirement(requirement);
                if(!isValidSupplier){
                    return ServiceUtil.returnError(UtilProperties.getMessage(resource, "ManufacturingRequirementNotValidSupplier", locale));
                }
                result = dispatcher.runSync("updateRequirement", UtilMisc.<String, Object>toMap("requirementId", requirementId, "statusId", "REQ_APPROVED", "requirementTypeId", requirement.getString("requirementTypeId"), "quantity", quantity, "productId", requirement.getString("productId"), "userLogin", userLogin));
                try {
                    dispatcher.runSync("autoAssignRequirementToSupplier", UtilMisc.toMap("requirementId", requirementId, "userLogin", userLogin));
                }catch(Exception e){
                    e.printStackTrace();
                }
            } else if (!(requirementTypeId.equals("PRODUCT_REQUIREMENT"))) {
    ...
    private static boolean checkIfValidSupplierExistsForRequirement(GenericValue requirement) {
        Timestamp requiredBy = requirement.getTimestamp("requiredByDate");
        String productId = requirement.getString("productId");
        List<EntityCondition> conditions = new ArrayList<EntityCondition>();
        if(UtilValidate.isNotEmpty(requiredBy)){
        	conditions.add(EntityCondition.makeCondition("availableFromDate",EntityOperator.LESS_THAN_EQUAL_TO,requiredBy));
        	conditions.add(EntityConditionList.makeCondition(EntityCondition.makeCondition("availableThruDate",EntityOperator.GREATER_THAN_EQUAL_TO,requiredBy)
                ,EntityOperator.OR,EntityCondition.makeCondition("availableThruDate",null)));
        }
        conditions.add(EntityCondition.makeCondition("productId",productId));
        try {
            List<GenericValue> requirements = requirement.getDelegator().findList("SupplierProduct",EntityCondition.makeCondition(conditions),null,null,null,false);
    ...
    public static Map createProductionRunForMktgPkg(DispatchContext ctx, Map context) throws GenericEntityException {
        Map result = new HashMap();
        Delegator delegator = ctx.getDelegator();
        LocalDispatcher dispatcher = ctx.getDispatcher();
        Locale locale = (Locale) context.get("locale");
        ...
        try {
            ...
            if (isImmediatelyFulfilled) {
                ...
                if (shipmentId != null) {
                    List<GenericValue> shipmentItemGvs = delegator.findByAnd("ShipmentItem", UtilMisc.toMap("shipmentId", shipmentId, "shipmentItemSeqId",
                            shipmentItemSeqId));
                    if (UtilValidate.isNotEmpty(shipmentItemGvs)) {
                        quantity = shipmentItemGvs.get(0).getBigDecimal("quantity");
                    }
                }
                existingAtp = existingAtp.subtract(quantity);
            }
    ...
    public static Map quickChangeProductionRunStatus(DispatchContext ctx, Map context) {
        ...
        ProductionRun productionRun = new ProductionRun(productionRunId, dispatcher.getDelegator(), dispatcher);
        GenericValue currentStatus = productionRun.getCurrentStatus();

        try {
            if (currentStatus != null) {
                GenericValue checkStatusValidChange = dispatcher.getDelegator().findOne("StatusValidChange", false, UtilMisc.toMap("statusId", currentStatus.getString("statusId"),
                        "statusIdTo", statusId));
                if (checkStatusValidChange == null) {
                    ServiceUtil.returnError("Changing the status from " + currentStatus.getString("description") + " to Close is not allowed.");
                } else {
                    return ServiceUtil.returnError("Changing the status to " + statusId + " is not allowed.");
                }
            }
        } catch (GenericEntityException gee) {
            gee.printStackTrace();
        ...
        try {
            ...
            if (statusId.equals("PRUN_CLOSED")) {
                ...
                updateProductPriceAverageCost(ctx, serviceContext);
    ...
    private static void updateProductPriceAverageCost(DispatchContext ctx, Map context) {
        ...
        String productionRunId = (String) context.get("workEffortId");
        ProductionRun productionRun = new ProductionRun(productionRunId, delegator, dispatcher);
        GenericValue pRun = productionRun.getGenericValue();
        GenericValue product = productionRun.getProductProduced();
        BigDecimal quantityProduced = new BigDecimal(pRun.getDouble("quantityProduced"));
        BigDecimal actualTotalCost = pRun.getBigDecimal("actualTotalCost");
        BigDecimal estimatedTotalCost = pRun.getBigDecimal("estimatedTotalCost");
        BigDecimal actualAvgCost = BigDecimal.ZERO;
        BigDecimal estimatedAvgCost = BigDecimal.ZERO;


        if (null != actualTotalCost && quantityProduced.compareTo(BigDecimal.ZERO) > 0) {
            actualAvgCost = actualTotalCost.divide(quantityProduced, decimals, rounding);
            estimatedAvgCost = (actualTotalCost.divide(quantityProduced, decimals, rounding));
        }


        try {
            List<GenericValue> pPrice = EntityUtil.filterByDate(delegator.findByAnd("ProductPrice", UtilMisc.toMap("productId", product.getString("productId"), "productPriceTypeId",
                    "AVERAGE_COST", "productPricePurposeId", "PURCHASE")));
            Map resultService = null;
            Map serviceContext = new HashMap();
            if (UtilValidate.isNotEmpty(pPrice)) {
                GenericValue prodPrice = pPrice.get(0);

                BigDecimal lastPrice = ((BigDecimal) prodPrice.get("price")) == null ? BigDecimal.ZERO : (BigDecimal) prodPrice.get("price");
                BigDecimal priceAvg = (lastPrice.add(actualAvgCost)).divide(new BigDecimal(2), decimals, rounding);
                serviceContext.put("price", priceAvg);
                serviceContext.put("productId", prodPrice.getString("productId"));
                serviceContext.put("productPriceTypeId", prodPrice.getString("productPriceTypeId"));
                serviceContext.put("productPricePurposeId", prodPrice.getString("productPricePurposeId"));
                serviceContext.put("currencyUomId", prodPrice.getString("currencyUomId"));
                serviceContext.put("productStoreGroupId", prodPrice.getString("productStoreGroupId"));
                serviceContext.put("fromDate", prodPrice.getTimestamp("fromDate"));
                serviceContext.put("userLogin", userLogin);
                resultService = dispatcher.runSync("updateProductPrice", serviceContext);
            } else {
                serviceContext.put("productId", product.getString("productId"));
                serviceContext.put("productPriceTypeId", "AVERAGE_COST");
                serviceContext.put("productPricePurposeId", "PURCHASE");
                serviceContext.put("currencyUomId", pRun.getString("moneyUomId"));
                serviceContext.put("fromDate", UtilDateTime.nowTimestamp());
                serviceContext.put("price", actualAvgCost);
                serviceContext.put("productStoreGroupId", "_NA_");
                serviceContext.put("userLogin", userLogin);
                resultService = dispatcher.runSync("createProductPrice", serviceContext);
            }
            serviceContext.clear();
            serviceContext.put("workEffortId", pRun.get("workEffortId"));
            serviceContext.put("actualAvgCost", actualAvgCost);
            serviceContext.put("estimatedAvgCost", estimatedAvgCost);
            serviceContext.put("userLogin", userLogin);
            resultService = dispatcher.runSync("updateWorkEffort", serviceContext);
            ...
            Debug.logError(e, "Problem while updating the production run cost", module);
