...
public class MrpServices {
    ...
    public static Map initMrpEvents(DispatchContext ctx, Map context) {
        ...
        try {
            condition = EntityCondition.makeCondition(EntityCondition.makeCondition("facilityId", EntityOperator.EQUALS,
                    facilityId));

            listResult = delegator.findList("MrpEvent", condition, null, null, null, false);
            // int numOfRecordsRemoved = delegator.removeByCondition("MrpEvent",
            // null);
        } catch (GenericEntityException e) {
            Debug.logError(e,"Error : findList(\"MrpEvent\", null, null, null, nullR, false)", module);
        ...
        try {
            listResult = delegator.findByAnd("Requirement", UtilMisc.toMap("requirementTypeId", "PRODUCT_REQUIREMENT",
                    "statusId", "REQ_PROPOSED", "facilityId", facilityId));
        } catch (GenericEntityException e) {
            return ServiceUtil.returnError("Problem, we can not find all the items of MrpEvent, for more detail look at the log");
        }
        if (listResult != null) {
        ...
        listResult = null;
        try {
            listResult = delegator.findByAnd("Requirement", UtilMisc.toMap("requirementTypeId", "INTERNAL_REQUIREMENT",
                    "statusId", "REQ_PROPOSED", "facilityId", facilityId));
        } catch (GenericEntityException e) {
            return ServiceUtil.returnError("Problem, we can not find all the items of MrpEvent, for more detail look at the log");
        }
        ...
        while (iteratorResult.hasNext()) {
            ...
            parameters = UtilMisc.toMap("mrpId", mrpId, "productId", productId, "eventDate", requiredByDate, "mrpEventTypeId",
                    "SALES_ORDER_SHIP");
            try {
                InventoryEventPlannedServices.createOrUpdateMrpEvent(parameters, eventQuantityTmp, facilityId,
        ...
        resultList = null;
        iteratorResult = null;
        parameters = UtilMisc.toMap("requirementTypeId", "PRODUCT_REQUIREMENT", "statusId", "REQ_APPROVED", "facilityId",
                facilityId);
        try {
        ...
        try {
            List facilityContactMechs = EntityUtil.filterByDate(delegator.findByAnd("FacilityContactMech",
            ...
            Set fieldsToSelect = UtilMisc.toSet("orderId", "orderItemSeqId", "productId", "quantity", "cancelQuantity",
                    "oiEstimatedDeliveryDate");
            resultList = delegator.findList("OrderHeaderItemAndShipGroup",
                    EntityCondition.makeCondition(searchConditions, EntityOperator.AND), fieldsToSelect,
        ...
        while (iteratorResult.hasNext()) {
            ...
            if (!newOrderId.equals(orderId)) {
                ...
                orderId = newOrderId;
                try {
                    orderDeliverySchedule = delegator.findByPrimaryKey("OrderDeliverySchedule",
            ...
            try {
                orderItemDeliverySchedule = delegator.findByPrimaryKey("OrderDeliverySchedule",
                        UtilMisc.toMap("orderId", orderId, "orderItemSeqId", genericResult.getString("orderItemSeqId")));
            } catch (GenericEntityException e) {
            }
            ...
            parameters = UtilMisc.toMap("mrpId", mrpId, "productId", productId, "eventDate", estimatedShipDate, "mrpEventTypeId",
                    "PUR_ORDER_RECP", "facilityId", facilityId);
            try {
                InventoryEventPlannedServices.createOrUpdateMrpEvent(parameters, shipGroupQuantity, facilityId,
                        genericResult.getString("orderId") + "-" + genericResult.getString("orderItemSeqId"), false, delegator);
            } catch (GenericEntityException e) {
        ...
        resultList = null;
        iteratorResult = null;
        parameters = UtilMisc.toMap("workEffortGoodStdTypeId", "PRUNT_PROD_NEEDED", "statusId", "WEGS_CREATED", "facilityId",
                facilityId);
        try {
            ...
            while (iteratorResult.hasNext()) {
                ...
                List<GenericValue> consumedInventoryItems = delegator.findByAnd("WorkEffortAndInventoryAssign",
                        UtilMisc.toMap("workEffortId", genericResult.getString("workEffortId"), "productId", productId));
                ...
                parameters = UtilMisc.toMap("mrpId", mrpId, "productId", productId, "eventDate", estimatedShipDate,
                        "mrpEventTypeId", "MANUF_ORDER_REQ");
                String eventName = (UtilValidate.isEmpty(genericResult.getString("workEffortParentId")) ? genericResult
                        .getString("workEffortId") : genericResult.getString("workEffortParentId") + "-"
                        + genericResult.getString("workEffortId"));
                InventoryEventPlannedServices.createOrUpdateMrpEvent(parameters, eventQuantityTmp, facilityId, eventName, false,
                        delegator);
            }
        } catch (GenericEntityException e) {
            return ServiceUtil
                    .returnError("MRP was unable to initialize events for production run components: " + e.getMessage());
        }
        ...
        resultList = null;
        iteratorResult = null;
        parameters = UtilMisc.toMap("workEffortGoodStdTypeId", "PRUN_PROD_DELIV", "statusId", "WEGS_CREATED", "workEffortTypeId",
        ...
        try {
            ...
            while (iteratorResult.hasNext()) {
                ...
                parameters = UtilMisc.toMap("mrpId", mrpId, "productId", productId, "eventDate", estimatedShipDate,
                        "mrpEventTypeId", "MANUF_ORDER_RECP");
                InventoryEventPlannedServices.createOrUpdateMrpEvent(parameters, eventQuantityTmp, facilityId,
                        genericResult.getString("workEffortId"), false, delegator);
            }
        } catch (GenericEntityException e) {
            return ServiceUtil.returnError("MRP was unable to initialize events for products produced by production runs: "
                    + e.getMessage());
        }
        ...
        while (iteratorResult.hasNext()) {
            ...
            try {
                EntityFieldMap ecl = EntityCondition.makeCondition(UtilMisc.toMap("mrpId", mrpId, "productId", productId),
                        EntityOperator.AND);
            ...
            parameters = UtilMisc.toMap("mrpId", mrpId, "productId", productId, "eventDate", now, "mrpEventTypeId",
                    "REQUIRED_MRP");
            try {
                InventoryEventPlannedServices.createOrUpdateMrpEvent(parameters, BigDecimal.ZERO, facilityId, null, false,
                        delegator);
            } catch (GenericEntityException e) {
        ...
        while (iteratorResult.hasNext()) {
            ...
            if (customTimePeriod != null) {
                if (customTimePeriod.getDate("thruDate") != null
                    ...
                    try {
                        salesForecastDetails = delegator.findByAnd("SalesForecastDetail",
                                UtilMisc.toMap("salesForecastId", genericResult.getString("salesForecastId")));
                    } catch (GenericEntityException e) {
                        return ServiceUtil
                                .returnError("Problem, we can not find SalesForecastDetails, for more detail look at the log");
                    }
                    ...
                    while (sfdIter.hasNext()) {
                        ...
                        parameters = UtilMisc.toMap("mrpId", mrpId, "productId", productId, "eventDate",
                                customTimePeriod.getDate("fromDate"), "mrpEventTypeId", "SALES_FORECAST");
                        try {
                            InventoryEventPlannedServices.createOrUpdateMrpEvent(parameters, eventQuantityTmp, null,
                                    genericResult.getString("salesForecastDetailId"), false, delegator);
                        } catch (GenericEntityException e) {
    ...
    /**
     * Find the quantity on hand of products for MRP. <li>PreConditions : none</li>
     * <li>Result : We get the quantity of product available in the stocks.</li>
     * 
     * @param product
     *            the product for which the Quantity Available is required
     * @return the sum of all the totalAvailableToPromise of the inventoryItem
     *         related to the product, if the related facility is Mrp available
     *         (not yet implemented!!)
     */
    ...
    public static BigDecimal findProductMrpQoh(String mrpId, String productId, String facilityId, LocalDispatcher dispatcher,
            Delegator delegator) {
        Map resultMap = null;
        try {
            if (facilityId == null) {
                ...
                resultMap = dispatcher.runSync("getInventoryAvailableByFacility",
        ...
        try {
            if (UtilValidate.isNotEmpty(productId) && UtilValidate.isNotEmpty(errorMessage)) {
                GenericValue inventoryEventError = delegator.makeValue("MrpEvent", UtilMisc.toMap("productId", productId,
                        "mrpId", mrpId, "eventDate", eventDate, "mrpEventTypeId", "ERROR", "eventName", errorMessage));
                delegator.createOrStore(inventoryEventError);
            }
        } catch (GenericEntityException e) {
            Debug.logError(e, "Error calling logMrpError for productId [" + productId + "] and errorMessage [" + errorMessage
                    + "]", module);
        }
    }

    /**
     * Process the bill of material (bom) of the product to insert components in
     * the MrpEvent table. Before inserting in the entity, test if there is the
     * record already existing to add quantity rather to create a new one.
     * 
     * @param product
     * @param eventQuantity
     *            the product quantity needed
     * @param startDate
     *            the startDate of the productionRun which will used to produce
     *            the product
     * @param routingTaskStartDate
     *            Map with all the routingTask as keys and startDate of each of
     *            them
     * @return None
     */

    public static void processBomComponent(String mrpId, GenericValue product, String facilityId, BigDecimal eventQuantity,
            Timestamp startDate, Map routingTaskStartDate, List listComponent) {
        // TODO : change the return type to boolean to be able to test if all is
        // ok or if it have had a exception
        ...
        if (UtilValidate.isNotEmpty(listComponent)) {
            ...
            while (listComponentIter.hasNext()) {
                ...
                Timestamp eventDate = (routingTask == null || !routingTaskStartDate.containsKey(routingTask))
                        ? startDate
                ...
                if (EntityUtil.isValueActive(productComponent, eventDate)) {
                    ...
                    try {
                        InventoryEventPlannedServices.createOrUpdateMrpEvent(parameters, componentEventQuantity.negate(),
                                facilityId, product.get("productId") + ": " + eventDate, false, delegator);
                    } catch (GenericEntityException e) {
                        Debug.logError(
                                "Error : findByPrimaryKey(\"MrpEvent\", parameters) =" + parameters + "--" + e.getMessage(),
                                module);
                        logMrpError(mrpId, node.getProduct().getString("productId"),
    ...
    /**
     * Launch the MRP. <li>PreConditions : none</li> <li>Result : The date when
     * we must order or begin to build the products and subproducts we need are
     * calclated</li>
     * 
     * <li>INPUT : parameters to get from the context :</li>
     * <ul>
     * <li>String mrpName</li>
     * </ul>
     * 
     * <li>OUTPUT : Result to put in the map :</li>
     * <ul>
     * <li>none</li>
     * </ul>
     * 
     ...
     * @param context
     *            Map containing the input parameters, productId routingId,
     *            quantity, startDate.
    ...
    public static Map executeMrp(DispatchContext ctx, Map context) {
        ...
        if (UtilValidate.isEmpty(facilityId)) {
            try {
                GenericValue facilityGroup = delegator.findByPrimaryKey("FacilityGroup",
                        UtilMisc.toMap("facilityGroupId", facilityGroupId));
        ...
        parameters = UtilMisc.toMap("mrpId", mrpId, "reInitialize", Boolean.TRUE, "defaultYearsOffset", defaultYearsOffset,
                "userLogin", userLogin);
        parameters.put("facilityId", facilityId);
        ...
        do {
            ...
            if (bomLevel == 0) {
                filterByConditions = EntityCondition.makeCondition(
                        EntityCondition.makeCondition("billOfMaterialLevel", EntityOperator.EQUALS, null), EntityOperator.OR,
                        EntityCondition.makeCondition("billOfMaterialLevel", EntityOperator.EQUALS, Long.valueOf(bomLevel)));
            } else {
                filterByConditions = EntityCondition.makeCondition("billOfMaterialLevel", EntityOperator.EQUALS,
                        Long.valueOf(bomLevel));
            }
            filterByConditions = EntityCondition.makeCondition(filterByConditions,
                    EntityCondition.makeCondition(UtilMisc.toMap("facilityId", facilityId)));
            try {
                listInventoryEventForMRP = delegator.findList("MrpEventView", filterByConditions, null,
                        UtilMisc.toList("productId", "eventDate"), null, false);
            } catch (GenericEntityException e) {
                return ServiceUtil.returnError("MRP Error retieving MRP event for the bom level: " + bomLevel + ". Error: "
                        + e.getMessage());
            }
                ...
                while (iteratorListInventoryEventForMRP.hasNext()) {
                    ...
                    if (!productId.equals(oldProductId)) {
                        BigDecimal positiveEventQuantity = eventQuantity.compareTo(BigDecimal.ZERO) > 0
                                ? eventQuantity
                                : eventQuantity.negate();
                        // It's a new product, so it's necessary to read the
                        // MrpQoh
                        try {
                            product = inventoryEventForMRP.getRelatedOneCache("Product");
                            productFacility = EntityUtil.getFirst(product.getRelatedByAndCache("ProductFacility",
                                    UtilMisc.toMap("facilityId", facilityId)));
                        } catch (GenericEntityException e) {
                            return ServiceUtil
                                    .returnError("Problem, can not find the product for a event, for more detail look at the log");
                        }
                        stockTmp = findProductMrpQoh(mrpId, product, facilityId, dispatcher, delegator);
                        try {
                            InventoryEventPlannedServices.createOrUpdateMrpEvent(UtilMisc.toMap("mrpId", mrpId, "productId",
                                    product.getString("productId"), "mrpEventTypeId", "INITIAL_QOH", "eventDate", now), stockTmp,
                                    facilityId, null, false, delegator);
                        } catch (GenericEntityException e) {
                            return ServiceUtil.returnError("Problem running createOrUpdateMrpEvent");
                        }
                        // days to ship is only relevant for sales order to plan
                        // for preparatory days to ship. Otherwise MRP will push
                        // event dates for manufacturing parts
                        // as well and cause problems
                        daysToShip = 0;
                        if (productFacility != null) {
                            reorderQuantity = (productFacility.getBigDecimal("reorderQuantity") != null ? productFacility
                                    .getBigDecimal("reorderQuantity") : BigDecimal.ONE.negate());
                            minimumStock = (productFacility.getBigDecimal("minimumStock") != null ? productFacility
                                    .getBigDecimal("minimumStock") : BigDecimal.ZERO);
                            if ("SALES_ORDER_SHIP".equals(inventoryEventForMRP.getString("mrpEventTypeId"))) {
                                daysToShip = (productFacility.getLong("daysToShip") != null ? productFacility.getLong(
                                        "daysToShip").intValue() : 0);
                            }
                        } else {
                            minimumStock = BigDecimal.ZERO;
                        ...
                        try {
                            serviceResponse = dispatcher.runSync("getManufacturingComponents", UtilMisc.<String, Object> toMap(
                                    "productId", product.getString("productId"), "quantity", positiveEventQuantity,
                                    "excludeWIPs", Boolean.FALSE, "userLogin", userLogin));
                        } catch (Exception e) {
                            return ServiceUtil.returnError("An error occurred exploding the product ["
                    ...
                    if (stockTmp.compareTo(minimumStock) < 0) {
                        ...
                        try {
                            serviceResponse = dispatcher.runSync("getManufacturingComponents", UtilMisc.<String, Object> toMap(
                                    "productId", product.getString("productId"), "quantity", proposedOrder.getQuantity(),
                                    "excludeWIPs", Boolean.FALSE, "userLogin", userLogin));
                        } catch (Exception e) {
                            return ServiceUtil.returnError("An error occurred exploding the product ["
                                    + product.getString("productId") + "]");
                        ...
                        // calculate the ProposedOrder requirementStartDate and
                        // update the requirementStartDate object property.
                        Map routingTaskStartDate = proposedOrder.calculateStartDate(daysToShip, routing, delegator, dispatcher,
                                userLogin);
                        if (isBuilt) {
                            // process the product components
                            processBomComponent(mrpId, product, facilityId, proposedOrder.getQuantity(),
                                    proposedOrder.getRequirementStartDate(), routingTaskStartDate, components);
                        }
                        ...
                        if (UtilValidate.isEmpty(productFacility) && !isBuilt) {
                            logMrpError(mrpId, productId, now, "No ProductFacility record for [" + facilityId
                        ...
                        Map eventMap = UtilMisc.toMap("productId", product.getString("productId"), "mrpId", mrpId, "eventDate",
                                eventDate,
                                "mrpEventTypeId", (isBuilt ? "PROP_MANUF_O_RECP" : "PROP_PUR_O_RECP"));
                        try {
                            InventoryEventPlannedServices.createOrUpdateMrpEvent(eventMap, proposedOrder.getQuantity(),
                                    (String) context.get("facilityId"), eventName, (proposedOrder.getRequirementStartDate()
                                            .compareTo(now) < 0), delegator);
                        } catch (GenericEntityException e) {
                            return ServiceUtil.returnError("Problem running createOrUpdateMrpEvent");
                        }
                        //
