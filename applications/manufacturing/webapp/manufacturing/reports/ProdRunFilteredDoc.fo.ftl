...
<#if productionRunList?has_content>
        ...
        <fo:flow flow-name="xsl-region-body" font-family="Helvetica">
            <fo:block font-size="14pt" margin-bottom="0.2in"> ${mainHeader} </fo:block>
            ...
            <fo:table>
                ...
                <fo:table-column />
                <fo:table-column />
                <#if parameters.viewReportFor == "ON_TIME_RUN">
                <fo:table-column />
                 </#if>
                <fo:table-header>
                    <fo:table-row font-weight="bold">
                        ...
                        <fo:table-cell border-bottom="thin solid grey"><fo:block>Planned Quantity</fo:block></fo:table-cell>
                        <fo:table-cell border-bottom="thin solid grey"><fo:block>${uiLabelMap.ManufacturingQuantityProduced}</fo:block></fo:table-cell>
                        <fo:table-cell border-bottom="thin solid grey"><fo:block>${uiLabelMap.CommonStatus}</fo:block></fo:table-cell>
                        <fo:table-cell border-bottom="thin solid grey"><fo:block>Estd Completion Date</fo:block></fo:table-cell>
                        <#if parameters.viewReportFor == "ON_TIME_RUN">
                        <fo:table-cell border-bottom="thin solid grey"><fo:block>Actual Completion Date</fo:block></fo:table-cell>
                        </#if>
                ...
                <fo:table-body>
                    ...
                    <#list productionRunList as productRun>
                        <fo:table-row>
                            ...
                            <fo:table-cell padding="2pt" background-color="${rowColor}">
                                <fo:block>${productRun.plannedQuantity?if_exists}</fo:block>
                            </fo:table-cell>
                            <fo:table-cell padding="2pt" background-color="${rowColor}">
                                <fo:block>${productRun.quantityProduced?if_exists}</fo:block>
                            </fo:table-cell>
                            <fo:table-cell padding="2pt" background-color="${rowColor}">
                                <fo:block>${productRun.status?if_exists}</fo:block>
                            </fo:table-cell>
                            <fo:table-cell padding="2pt" background-color="${rowColor}">
                                <fo:block>${productRun.estimatedCompletionDate?if_exists}</fo:block>
                            </fo:table-cell>
                            <#if parameters.viewReportFor == "ON_TIME_RUN">
                            <fo:table-cell padding="2pt" background-color="${rowColor}">
                                <fo:block>${productRun.actualCompletionDate?if_exists}</fo:block>
                            </fo:table-cell>
                            </#if>
                        </fo:table-row>
