...
<div class="screenlet">
  ...
  <div class="screenlet-body">
    ...
    <#if !(productAssoc?exists)>
        ...
        <table class="basic-table" cellspacing="0">
          ...
          <tr>
            ...
            <td>&nbsp;</td>
            <td>
  				<@htmlTemplate.renderDateTimeField name="fromDate" value="${value!''}" className="" alert="" 
                title="Format: yyyy-MM-dd HH:mm:ss.SSS" size="25" maxlength="30" id="fromDate" dateType="date" shortDateInput=false 
                timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" 
                hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName="editProductAssocForm"/>
        ...
        <table class="basic-table" cellspacing="0">
          <#if productId?has_content>
            ...
            <tr>
              <td align="right">${uiLabelMap.ProductProductId}</td>
              <td>&nbsp;</td>
              <td>${product.internalName} [${product.productId}]</td>
            </tr>
          <#else>
            <tr>
              <td align="right">${uiLabelMap.ProductProductId}</td>
          ...
          <#if productIdTo?has_content>
            <#assign product = delegator.findOne("Product",{"productId":productIdTo},false)>
            <tr>
              <td align="right">${uiLabelMap.ManufacturingProductIdTo}</td>
              <td>&nbsp;</td>
              <td>${product.internalName} [${product.productId}]</td>
    ...
    <tr>
        ...
        <td>&nbsp;</td>
        <td width="74%">
		<@htmlTemplate.renderDateTimeField name="thruDate" value="${value!''}" className="" alert="" 
                title="Format: yyyy-MM-dd HH:mm:ss.SSS" size="25" maxlength="30" id="thruDate" dateType="date" shortDateInput=false 
                timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" 
                hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName="editProductAssocForm"/>
        </td>
    </tr>
    ...
    <tr>
        <td colspan="2">&nbsp;</td>
        <td><input type="submit" class="btn btn-success" <#if !(productAssoc?exists)>value="${uiLabelMap.CommonAdd}"<#else>value="${uiLabelMap.CommonUpdate}"</#if>/></td>
    </tr>
    </table>
    </form>
  </div>
  ...
  <div class="screenlet-body">
    ...
    <#assign curProductAssocType = assocFromProduct.getRelatedOneCache("ProductAssocType")>
      <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
        <td><a href="<@ofbizUrl>EditProductBom?productId=${(assocFromProduct.productIdTo)?if_exists}&amp;productAssocTypeId=${(assocFromProduct.productAssocTypeId)?if_exists}#components</@ofbizUrl>" class="btn-link">${(assocFromProduct.productIdTo)?if_exists}</a></td>
        <td><#if listToProduct?exists><a href="<@ofbizUrl>EditProductBom?productId=${(assocFromProduct.productIdTo)?if_exists}&amp;productAssocTypeId=${(assocFromProduct.productAssocTypeId)?if_exists}#components</@ofbizUrl>" class="btn-link">${(listToProduct.internalName)?if_exists}</a></#if>&nbsp;</td>
        ...
        <td>
        <a href="<@ofbizUrl>UpdateProductBom?UPDATE_MODE=DELETE&amp;productId=${productId}&amp;productIdTo=${(assocFromProduct.productIdTo)?if_exists}&amp;productAssocTypeId=${(assocFromProduct.productAssocTypeId)?if_exists}&amp;fromDate=${assocFromProduct.getString("fromDate")}&amp;useValues=true</@ofbizUrl>" class="btn btn-mini btn-danger">${uiLabelMap.CommonDelete}</a>
        </td>
        <td>
...
<div class="screenlet">
  ...
  <div class="screenlet-body">
      <table class="basic-table" cellspacing="0">
        ...
        <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
            <td><a href="<@ofbizUrl>EditProductBom?productId=${(assocToProduct.productId)?if_exists}&amp;productAssocTypeId=${(assocToProduct.productAssocTypeId)?if_exists}#components</@ofbizUrl>" class="btn-link">${(assocToProduct.productId)?if_exists}</a></td>
<!--                <td><#if listToProduct?exists><a href="<@ofbizUrl>EditProduct?productId=${(assocToProduct.productId)?if_exists}</@ofbizUrl>" class="buttontext">${(listToProduct.internalName)?if_exists}</a></#if></td> -->
            <td><#if listToProduct?exists><a href="<@ofbizUrl>EditProductBom?productId=${(assocToProduct.productId)?if_exists}&amp;productAssocTypeId=${(assocToProduct.productAssocTypeId)?if_exists}#components</@ofbizUrl>" class="btn-link">${(listToProduct.internalName)?if_exists}</a></#if></td>
