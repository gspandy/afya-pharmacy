...
String exceptionDateStart = parameters.exceptionDateStart != null ? parameters.exceptionDateStart : request.getAttribute("exceptionDateStart");
/*exceptionDateStart = ObjectType.simpleTypeConvert(exceptionDateStart, "Timestamp", null, null);*/
calendarExceptionWeek = [];
if (UtilValidate.isNotEmpty(exceptionDateStart)) {
	DateFormat formater = UtilDateTime.SIMPLE_DATE_FORMAT;
	java.sql.Date exceptionDateStartSql= new java.sql.Date(formater.parse(exceptionDateStart).getTime());
    calendarExceptionWeek = delegator.findOne("TechDataCalendarExcWeek",false,UtilMisc.toMap("calendarId",calendarId,"exceptionDateStart",exceptionDateStartSql));
	if (UtilValidate.isNotEmpty(calendarExceptionWeek)) {
