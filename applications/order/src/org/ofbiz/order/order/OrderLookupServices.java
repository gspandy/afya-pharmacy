	...
	public static Map findOrders(DispatchContext dctx, Map context) {
		...
		if (saleChannelList != null) {
			...
			while (i.hasNext()) {
				...
				orExprs.add(EntityCondition.makeCondition("salesChannelEnumId", EntityOperator.EQUALS,
						salesChannelEnumId));
			}
			conditions.add(EntityCondition.makeCondition(orExprs, EntityOperator.OR));
		}
		...
		/* No role based restrictions for Find Orders (createdBy condition commented) */
		
		/*else {
			// Fixed for order approval Based
			String partyId = userLogin.getString("partyId");
			List orderCreators = new ArrayList();
			orderCreators.add(partyId);			
			List<GenericValue> partyRole;
			try {
				...
				if (partyRole.size()>0) {
					List<GenericValue> partyRelationships = delegator.findList("PartyRelationship",
							EntityCondition.makeCondition("partyIdTo", partyId),UtilMisc.toSet("partyIdFrom"), null, null, false);
			...
			conditions.add(EntityCondition.makeCondition("createdBy", EntityOperator.IN, orderCreators));
		}*/
		...
		if (UtilValidate.isNotEmpty(minDate)) {
			...
			try {
				...
				if (converted != null) {
					conditions.add(EntityCondition.makeCondition(dateField, EntityOperator.GREATER_THAN_EQUAL_TO,
							converted));
				}

			} catch (GeneralException e) {
				error = ServiceUtil.returnError("Please enter the valid From date");
				return error;
			}
		...
		if (UtilValidate.isNotEmpty(maxDate)) {
			...
			try {
				...
				if (converted != null) {
					conditions.add(EntityCondition.makeCondition("orderDate", EntityOperator.LESS_THAN_EQUAL_TO,
							converted));
				}

			} catch (GeneralException e) {
				error = ServiceUtil.returnError("Please enter the valid Thru date");
				return error;
			}
		...
		if (UtilValidate.isNotEmpty(userLoginId) && UtilValidate.isEmpty(partyId)) {
			...
			try {
				...
				if (ul == null) {
					error = ServiceUtil.returnError("User Login Id : " + userLoginId + " Not Found");
					return error;
				}
			} catch (GenericEntityException e) {
		...
		if (UtilValidate.isNotEmpty(gatewayAvsResult) || UtilValidate.isNotEmpty(gatewayScoreResult)) {
			...
			dve.addViewLink("OPP", "PGR", Boolean.FALSE,
					UtilMisc.toList(new ModelKeyMap("orderPaymentPreferenceId", "orderPaymentPreferenceId")));
		}
		...
		if (correspondingPoId != null || subscriptionId != null || productId != null || budgetId != null
				|| quoteId != null) {
		...
		if (UtilValidate.isNotEmpty(productId)) {
			...
			if (productId.startsWith("%") || productId.startsWith("*") || productId.endsWith("%")
					|| productId.endsWith("*")) {
				...
				if (product != null) {
					...
					if (isVirtual != null && "Y".equals(isVirtual)) {
						...
						Map varLookup = null;
						try {
							varLookup = dispatcher.runSync("getAllProductVariants",
		...
		if (UtilValidate.isNotEmpty(cardNumber)) {
			...
			dve.addViewLink("OP", "CC", Boolean.FALSE,
		...
		if (UtilValidate.isNotEmpty(accountNumber)) {
			...
			dve.addViewLink("OP", "EF", Boolean.FALSE,
					UtilMisc.toList(new ModelKeyMap("paymentMethodId", "paymentMethodId")));
		...
		if (shipmentId != null || inventoryItemId != null || softIdentifier != null || serialNumber != null) {
			...
			if (softIdentifier != null || serialNumber != null) {
				...
				dve.addViewLink("II", "IV", Boolean.FALSE,
						UtilMisc.toList(new ModelKeyMap("inventoryItemId", "inventoryItemId")));
			}
		}
		...
		if (UtilValidate.isNotEmpty(hasBackOrders)) {
			...
			if ("Y".equals(hasBackOrders)) {
				...
				conditions.add(EntityCondition.makeCondition("quantityNotAvailable", EntityOperator.GREATER_THAN,
						BigDecimal.ZERO));
				...
				orExpr.add(EntityCondition
						.makeCondition("quantityNotAvailable", EntityOperator.EQUALS, BigDecimal.ZERO));
