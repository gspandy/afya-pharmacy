 ...
 * <p/>
 * <p>
 * NOTE: in the current scheme order adjustments are never included in tax or shipping, but order item adjustments ARE
 * included in tax and shipping calcs unless they are tax or shipping adjustments or the includeInTax or
...
public class OrderReadHelper {
    ...
    public OrderReadHelper(GenericValue orderHeader, List<GenericValue> adjustments, List<GenericValue> orderItems) {
        ...
        if (this.orderHeader != null && !this.orderHeader.getEntityName().equals("OrderHeader")) {
            try {
                this.orderHeader =
                        orderHeader.getDelegator().findByPrimaryKey("OrderHeader",
    ...
    public static BigDecimal getOrderItemDeductible(BigDecimal netAmount, GenericValue orderItem) {
        BigDecimal deductibleAmount = orderItem.getBigDecimal("deductibleAmount") == null ? BigDecimal.ZERO : orderItem.getBigDecimal("deductibleAmount");
        if (deductibleAmount.compareTo(BigDecimal.ZERO) == 0) {
            BigDecimal deductiblePercentage = orderItem.getBigDecimal("deductiblePercentage") == null ? BigDecimal.ZERO : orderItem.getBigDecimal("deductiblePercentage");
    ...
    public static BigDecimal getOrderItemCopay(BigDecimal netAmount, GenericValue orderItem) {
        BigDecimal copayAmount = orderItem.getBigDecimal("copayAmount") == null ? BigDecimal.ZERO : orderItem.getBigDecimal("copayAmount");
        if (copayAmount.compareTo(BigDecimal.ZERO) == 0) {
            BigDecimal copayPercentage = orderItem.getBigDecimal("copayPercentage") == null ? BigDecimal.ZERO : orderItem.getBigDecimal("copayPercentage");
            if (orderItem.getString("computeBy") != null && orderItem.getString("computeBy").equalsIgnoreCase("GROSS")) {
    ...
    public static BigDecimal getOrderItemCorporateCopay(String primaryPayer, BigDecimal netAmount, GenericValue orderItem) {
        BigDecimal copayAmount = orderItem.getBigDecimal("copayAmount") == null ? BigDecimal.ZERO : orderItem.getBigDecimal("copayAmount");
        if (copayAmount.compareTo(BigDecimal.ZERO) == 0) {
            if (orderItem.getBigDecimal("copayPercentage") == null && primaryPayer!= null && "Patient".equalsIgnoreCase(primaryPayer)) {
                if (orderItem.getString("computeBy") != null && orderItem.getString("computeBy").equalsIgnoreCase("GROSS"))
                    copayAmount = getOrderItemQuantity(orderItem).multiply(orderItem.getBigDecimal("unitPrice")).setScale(scale, rounding);
                else if (orderItem.getString("computeBy") != null && orderItem.getString("computeBy").equalsIgnoreCase("NET"))
                    copayAmount = netAmount.setScale(scale, rounding);
                else
                    copayAmount = netAmount.setScale(scale, rounding);
            } else if (orderItem.getBigDecimal("copayPercentage") == null && primaryPayer!= null && "Corporate".equalsIgnoreCase(primaryPayer)) {
    ...
    public List<GenericValue> getPaymentPreferences() {
        if (paymentPrefs == null) {
            try {
                paymentPrefs =
                        orderHeader.getRelated("OrderPaymentPreference", UtilMisc.toList("orderPaymentPreferenceId"));
            } catch (GenericEntityException e) {
                Debug.logError(e, module);
            }
        }
        ...
        while (ppit.hasNext()) {
            ...
            try {
                List<EntityExpr> exprs =
                        UtilMisc.toList(EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "PMNT_RECEIVED"),
                ...
                List<EntityExpr> conds =
                        UtilMisc.toList(
                                EntityCondition.makeCondition("paymentTypeId", EntityOperator.EQUALS, "CUSTOMER_PAYMENT"),
            ...
            while (payit.hasNext()) {
                ...
                if (payment.get("amount") != null) {
                    chargedToPaymentPref =
                            chargedToPaymentPref.add(payment.getBigDecimal("amount")).setScale(scale + 1, rounding);
                }
            }

            // if chargedToPaymentPref > 0
            if (chargedToPaymentPref.compareTo(ZERO) > 0) {
                // key of the resulting map is paymentMethodId or paymentMethodTypeId if the paymentMethodId is not
                // available
                String paymentMethodKey =
                        paymentPref.getString("paymentMethodId") != null ? paymentPref.getString("paymentMethodId")
    ...
    public Map<String, BigDecimal> getReturnedTotalsByPaymentMethod() {
        ...
        while (ppit.hasNext()) {
            ...
            try {
                returnItemResponses =
                        orderHeader.getDelegator().findByAnd("ReturnItemResponse",
                                UtilMisc.toMap("orderPaymentPreferenceId", paymentPref.getString("orderPaymentPreferenceId")));
            } catch (GenericEntityException e) {
                Debug.logError(e, module);
            }
    ...
    public Long getOrderTermNetDays() {
        List<GenericValue> orderTerms =
                EntityUtil.filterByAnd(getOrderTerms(), UtilMisc.toMap("termTypeId", "FIN_PAYMENT_TERM"));
        if (UtilValidate.isEmpty(orderTerms)) {
            return null;
        } else if (orderTerms.size() > 1) {
            Debug.logWarning("Found " + orderTerms.size() + " FIN_PAYMENT_TERM order terms for orderId ["
                    + getOrderId() + "], using the first one ", module);
        }
        ...
        if (shippingCms != null) {
            ...
            while (i.hasNext()) {
                ...
                if (ocm != null) {
                    try {
                        GenericValue addr =
                                ocm.getDelegator().findByPrimaryKey("PostalAddress",
    ...
    public Timestamp getLatestShipAfterDate() {
        try {
            List<GenericValue> groups =
                    orderHeader.getRelated("OrderItemShipGroup", UtilMisc.toList("shipAfterDate DESC"));
            if (groups.size() > 0) {
                GenericValue group = groups.get(0);
    ...
    /**
     * Returns the OrderPaymentPreference.maxAmount for the billing account associated with the order, or 0 if there is
     * no billing account or no max amount set
     */
    ...
    public GenericValue getPartyFromRole(String roleTypeId) {
        ...
        try {
            GenericValue orderRole =
                    EntityUtil.getFirst(orderHeader.getRelatedByAnd("OrderRole", UtilMisc.toMap("roleTypeId", roleTypeId)));

            if (orderRole != null) {
                partyObject =
                        delegator.findByPrimaryKey("Person", UtilMisc.toMap("partyId", orderRole.getString("partyId")));

                if (partyObject == null) {
                    partyObject =
                            delegator.findByPrimaryKey("PartyGroup",
                                    UtilMisc.toMap("partyId", orderRole.getString("partyId")));
                }
            }
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
    ...
    public String getAffiliateId() {
        try {
            GenericEntity distributorRole =
                    EntityUtil
                            .getFirst(orderHeader.getRelatedByAnd("OrderRole", UtilMisc.toMap("roleTypeId", "AFFILIATE")));
    ...
    public BigDecimal getShippingTotal() {
        return OrderReadHelper.calcOrderAdjustments(getOrderHeaderAdjustments(), getOrderItemsSubTotal(), false, false,
                true);
    }

    public BigDecimal getHeaderTaxTotal() {
        return OrderReadHelper.calcOrderAdjustments(getOrderHeaderAdjustments(), getOrderItemsSubTotal(), false, true,
                false);
    }
    ...
    public Set<String> getItemFeatureSet(GenericValue item) {
        ...
        if (item.get("productId") != null) {
            try {
                featureAppls =
                        item.getDelegator().findByAndCache("ProductFeatureAppl",
                                UtilMisc.toMap("productId", item.getString("productId")));
                List<EntityExpr> filterExprs =
                        UtilMisc.toList(EntityCondition.makeCondition("productFeatureApplTypeId", EntityOperator.EQUALS,
                                "STANDARD_FEATURE"));
                filterExprs.add(EntityCondition.makeCondition("productFeatureApplTypeId", EntityOperator.EQUALS,
                        "REQUIRED_FEATURE"));
        ...
        try {
            additionalFeatures =
                    item.getRelatedByAnd("OrderAdjustment", UtilMisc.toMap("orderAdjustmentTypeId", "ADDITIONAL_FEATURE"));
        } catch (GenericEntityException e) {
    ...
    public Map<String, BigDecimal> getFeatureIdQtyMap(String shipGroupSeqId) {
        ...
        if (validItems != null) {
            ...
            while (i.hasNext()) {
                ...
                try {
                    additionalFeatures =
                            item.getRelatedByAnd("OrderAdjustment",
            ...
            while (i.hasNext()) {
                ...
                if (product != null) {
                    if (ProductWorker.shippingApplies(product)) {
                        shippableTotal =
                                shippableTotal.add(
                                        OrderReadHelper.getOrderItemSubTotal(item, getAdjustments(), false, true)).setScale(
                                        scale, rounding);
                    }
                }
            }
    ...
    public BigDecimal getShippableWeight(String shipGroupSeqId) {
        ...
        if (validItems != null) {
            ...
            while (i.hasNext()) {
                GenericValue item = i.next();
                shippableWeight =
                        shippableWeight.add(this.getItemWeight(item).multiply(getOrderItemQuantity(item))).setScale(scale,
                                rounding);
            }
        }
                    ...
                    try {
                        ...
                        if (UtilValidate.isNotEmpty(virtualId)) {
                            GenericValue virtual =
                                    delegator.findByPrimaryKeyCache("Product", UtilMisc.toMap("productId", virtualId));
                            if (virtual != null) {
    ...
    /**
     * Get the total payment preference amount by payment type. Specify null to get amount for all preference types.
     * TODO: filter by status as well?
     */
    ...
    /**
     * Get the total payment received amount by payment type. Specify null to get amount over all types. This method
     * works by going through all the PaymentAndApplications for all order Invoices that have status PMNT_RECEIVED.
     */
    public BigDecimal getOrderPaymentReceivedTotalByType(String paymentMethodTypeId) {
        ...
        try {
            ...
            // get the payments of the desired type for these invoices TODO: in models where invoices can have many
            // orders,
            ...
            if (paymentMethodTypeId != null) {
                conditions.add(EntityCondition.makeCondition("paymentMethodTypeId", EntityOperator.EQUALS,
                        paymentMethodTypeId));
            }
            EntityConditionList<EntityExpr> ecl = EntityCondition.makeCondition(conditions, EntityOperator.AND);
            List<GenericValue> payments =
                    orderHeader.getDelegator().findList("PaymentAndApplication", ecl, null, null, null, true);
    ...
    public BigDecimal getItemSize(GenericValue item) {
        ...
        if (product != null) {
            if (ProductWorker.shippingApplies(product)) {
                ...
                size =
                        sizeInfo[0].multiply(new BigDecimal("2")).add(sizeInfo[1].multiply(new BigDecimal("2")))
                                .add(sizeInfo[2]);
            }
        }
                    ...
                    try {
                        List<GenericValue> virtuals =
                                delegator.findByAnd("ProductAssoc", UtilMisc.toMap("productIdTo",
                                        product.getString("productId"), "productAssocTypeId", "PRODUCT_VARIANT"), UtilMisc
                                        .toList("-fromDate"));
    ...
    public String getOrderEmailString() {
        ...
        try {
            Map<String, Object> ocFields =
                    UtilMisc.toMap("orderId", orderHeader.get("orderId"), "contactMechPurposeTypeId", "ORDER_EMAIL");
    ...
    /**
     * Gets the amount open on the order that is not covered by the relevant OrderPaymentPreferences. This works by
     * adding up the amount allocated to each unprocessed OrderPaymentPreference and the amounts received and refunded
     * as payments for the settled ones.
     */
        ...
        for (Iterator<GenericValue> iter = prefs.iterator(); iter.hasNext(); ) {
            ...
            if ("PAYMENT_CANCELLED".equals(pref.get("statusId")) || "PAYMENT_DECLINED".equals(pref.get("statusId"))) {
                ...
                List<GenericValue> responses =
                        pref.getRelatedByAnd("PaymentGatewayResponse", UtilMisc.toMap("transCodeEnumId", "PGT_CAPTURE"));
                ...
                responses =
                        pref.getRelatedByAnd("PaymentGatewayResponse", UtilMisc.toMap("transCodeEnumId", "PGT_REFUND"));
                for (Iterator<GenericValue> respIter = responses.iterator(); respIter.hasNext(); ) {
                    GenericValue response = respIter.next();
                    BigDecimal amount = response.getBigDecimal("amount");
                    if (amount != null) {
    ...
    public int hasSurvey() {
        ...
        try {
            surveys =
                    delegator.findByAnd("SurveyResponse", UtilMisc.toMap("orderId", orderHeader.getString("orderId")));
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
        }
        int size = 0;
    ...
    public List<GenericValue> getOrderItemAndShipGroupAssoc() {
        if (orderItemAndShipGrp == null) {
            try {
                orderItemAndShipGrp =
                        orderHeader.getDelegator().findByAnd("OrderItemAndShipGroupAssoc",
                                UtilMisc.toMap("orderId", orderHeader.getString("orderId")));
            } catch (GenericEntityException e) {
                Debug.logWarning(e, module);
            }
    ...
    public List<GenericValue> getOrderItemAndShipGroupAssoc(String shipGroupSeqId) {
        List<EntityExpr> exprs =
                UtilMisc.toList(EntityCondition.makeCondition("shipGroupSeqId", EntityOperator.EQUALS, shipGroupSeqId));
        return EntityUtil.filterByAnd(getOrderItemAndShipGroupAssoc(), exprs);
    }

    public List<GenericValue> getValidOrderItems() {
        List<EntityExpr> exprs =
                UtilMisc.toList(EntityCondition.makeCondition("statusId", EntityOperator.NOT_EQUAL, "ITEM_CANCELLED"),
    ...
    public boolean getPastEtaOrderItems(String orderId) {
        /*
         * List exprs = UtilMisc.toList(EntityCondition.makeCondition("statusId", EntityOperator.EQUALS,
         * "ITEM_APPROVED")); List itemsApproved = EntityUtil.filterByAnd(getOrderItems(), exprs); Iterator i =
         * itemsApproved.iterator(); while (i.hasNext()) { GenericValue item = (GenericValue) i.next(); Timestamp
         * estimatedDeliveryDate = (Timestamp) item.get("estimatedDeliveryDate"); if (estimatedDeliveryDate != null &&
         * UtilDateTime.nowTimestamp().after(estimatedDeliveryDate)) { return true; } } return false; }
         */
        Delegator delegator = orderHeader.getDelegator();
        GenericValue orderDeliverySchedule = null;
        try {
            orderDeliverySchedule =
                    delegator.findByPrimaryKey("OrderDeliverySchedule",
    ...
    public boolean getPartiallyReceivedItems() {
         ...
         * List exprs = UtilMisc.toList(EntityCondition.makeCondition("statusId", EntityOperator.EQUALS,
         * "ITEM_APPROVED")); List itemsApproved = EntityUtil.filterByAnd(getOrderItems(), exprs); Iterator i =
         * itemsApproved.iterator(); while (i.hasNext()) { GenericValue item = (GenericValue) i.next(); int
         * shippedQuantity = (int) getItemShippedQuantity(item); BigDecimal orderedQuantity = (BigDecimal)
         * item.get("quantity"); if (shippedQuantity != orderedQuantity.intValue() && shippedQuantity > 0) { return
         * true; } } return false; }
         */
        List<GenericValue> items = getOrderItems();
        Iterator<GenericValue> i = items.iterator();
    ...
    public GenericValue getOrderItem(String orderItemSeqId) {
        List<EntityExpr> exprs =
                UtilMisc.toList(EntityCondition.makeCondition("orderItemSeqId", EntityOperator.EQUALS, orderItemSeqId));
    ...
    public List<GenericValue> getValidDigitalItems() {
        ...
        List<EntityExpr> exprs =
                UtilMisc.toList(EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "ITEM_APPROVED"),
    ...
    public List<GenericValue> getOrderItemPriceInfos(GenericValue orderItem) {
        ...
        if (this.orderItemPriceInfos == null) {
            ...
            try {
                orderItemPriceInfos =
                        delegator.findByAnd("OrderItemPriceInfo", UtilMisc.toMap("orderId", orderHeader.get("orderId")));
            } catch (GenericEntityException e) {
                Debug.logWarning(e, module);
            }
        }
    ...
    public List<GenericValue> getOrderItemShipGroupAssocs(GenericValue orderItem) {
        if (orderItem == null)
            return null;
        try {
            return orderHeader.getDelegator().findByAnd(
                    "OrderItemShipGroupAssoc",
                    UtilMisc.toMap("orderId", orderItem.getString("orderId"), "orderItemSeqId",
                            orderItem.getString("orderItemSeqId")), UtilMisc.toList("shipGroupSeqId"));
        } catch (GenericEntityException e) {
            Debug.logWarning(e, module);
        }
        return null;
        ...
        if (this.orderItemShipGrpInvResList == null) {
            ...
            try {
                orderItemShipGrpInvResList =
                        delegator.findByAnd("OrderItemShipGrpInvRes", UtilMisc.toMap("orderId", orderItem.get("orderId")));
            } catch (GenericEntityException e) {
                Debug.logWarning(e, "Trouble getting OrderItemShipGrpInvRes List", module);
            }
        }
        return EntityUtil.filterByAnd(orderItemShipGrpInvResList,
                UtilMisc.toMap("orderItemSeqId", orderItem.getString("orderItemSeqId")));
    }
    ...
    public List<GenericValue> getOrderItemIssuances(GenericValue orderItem, String shipmentId) {
        ...
        if (this.orderItemIssuances == null) {
            ...
            try {
                orderItemIssuances =
                        delegator.findByAnd("ItemIssuance", UtilMisc.toMap("orderId", orderItem.get("orderId")));
            } catch (GenericEntityException e) {
        ...
        if (this.orderReturnItems == null) {
            try {
                this.orderReturnItems =
                        delegator.findByAnd("ReturnItem", UtilMisc.toMap("orderId", orderHeader.getString("orderId")));
                GenericValue returnItem = EntityUtil.getFirst(this.orderReturnItems);
                if (returnItem != null) {
                    GenericValue returnHeader = returnItem.getRelatedOne("ReturnHeader");
                    if ("RETURN_REQUESTED".equals(returnHeader.getString("statusId"))) {
    ...
    /**
     * Get the quantity returned per order item. In other words, this method will count the ReturnItems related to each
     * OrderItem.
     *
    ...
    /**
     * Get the total quantity of returned items for an order. This will count only the ReturnItems that are directly
     * correlated to an OrderItem.
     */
    public BigDecimal getOrderReturnedQuantity() {
        ...
        List<EntityExpr> orderItemExprs =
                UtilMisc.toList(EntityCondition.makeCondition("returnItemTypeId", EntityOperator.EQUALS, "RET_PROD_ITEM"));
        ...
        orderItemExprs.add(EntityCondition.makeCondition("returnItemTypeId", EntityOperator.EQUALS,
                "RET_PROD_FEATR_ITEM"));
        ...
        while (i.hasNext()) {
            ...
            if (returnedItem.get("returnQuantity") != null) {
                returnedQuantity =
                        returnedQuantity.add(returnedItem.getBigDecimal("returnQuantity")).setScale(scale, rounding);
            }
        }
    ...
    public BigDecimal getOrderReturnedTotalByTypeBd(String returnTypeId, boolean includeAll) {
        ...
        if (!includeAll) {
            ...
            returnedItems.addAll(EntityUtil.filterByAnd(returnedItemsBase, UtilMisc.toList(EntityCondition
                    .makeCondition("statusId", EntityOperator.NOT_EQUAL, "RETURN_CANCELLED"))));
        }
        ...
        while (i.hasNext()) {
            ...
            if ((returnedItem.get("returnPrice") != null) && (returnedItem.get("returnQuantity") != null)) {
                returnedAmount =
                        returnedAmount.add(returnedItem.getBigDecimal("returnPrice")
                                .multiply(returnedItem.getBigDecimal("returnQuantity")).setScale(scale, rounding));
            }
            ...
            returnedAmount =
                    returnedAmount.add(getReturnAdjustmentTotal(orderHeader.getDelegator(), itemAdjustmentCondition));
        ...
        while (returnHeaderIterator.hasNext()) {
            ...
            Map<String, Object> returnHeaderAdjFilter =
                    UtilMisc.<String, Object>toMap("returnId", returnId, "returnItemSeqId", "_NA_");
            returnedAmount =
                    returnedAmount.add(getReturnAdjustmentTotal(orderHeader.getDelegator(), returnHeaderAdjFilter))
                            .setScale(scale, rounding);
        }
    ...
    /**
     * Gets the total returned over all return types. Specify true to include all return statuses except cancelled.
     * Specify false to include only COMPLETED and RECEIVED returns.
     */
    ...
    public BigDecimal getOrderNonReturnedTaxAndShipping() {
        ...
        // then go through all order items and for the quantity not returned calculate it's portion of the item, and of
        // the
        // entire order
        ...
        BigDecimal orderTaxNotReturned =
                this.getHeaderTaxTotal().multiply(orderFactorNotReturned).setScale(scale, rounding);
        BigDecimal orderShippingNotReturned =
                this.getShippingTotal().multiply(orderFactorNotReturned).setScale(scale, rounding);

        return totalTaxNotReturned.add(totalShippingNotReturned).add(orderTaxNotReturned).add(orderShippingNotReturned)
                .setScale(scale, rounding);
    }
    ...
    public BigDecimal getBillingAccountReturnedTotalByTypeBd(String returnTypeId) {
        ...
        try {
            for (Iterator<GenericValue> iter = returnedItems.iterator(); iter.hasNext(); ) {
                ...
                returnedAmount =
                        returnedAmount.add(returnItemResponse.getBigDecimal("responseAmount")).setScale(scale, rounding);
            }
        } catch (GenericEntityException e) {
            Debug.logError(e, e.getMessage(), module);
        }
    ...
    public BigDecimal getReturnedCreditTotalWithBillingAccountBd() {
        return getOrderReturnedCreditTotalBd().add(getBillingAccountReturnedRefundTotalBd()).subtract(
    ...
    public BigDecimal getReturnedRefundTotalWithBillingAccountBd() {
        return getOrderReturnedRefundTotalBd().add(getBillingAccountReturnedCreditTotalBd()).subtract(
    ...
    public BigDecimal getItemPickedQuantityBd(GenericValue orderItem) {
        ...
        try {
            picked =
                    orderHeader.getDelegator().findList("PicklistAndBinAndItem", pickedConditions, null, null, null, false);
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            this.orderHeader = null;
        }
    ...
    public BigDecimal getItemShippedQuantity(GenericValue orderItem) {
        ...
        quantityShipped.setScale(scale, rounding);
    ...
    public BigDecimal getOrderItemAdjustmentsTotal(GenericValue orderItem, boolean includeOther, boolean includeTax,
    ...
    public Set<String> getProductPromoCodesEntered() {
        ...
        try {
            for (GenericValue orderProductPromoCode : delegator.findByAndCache("OrderProductPromoCode",
    ...
    public List<GenericValue> getAvailableOrderHeaderAdjustments() {
        ...
        if (orderHeaderAdjustments != null) {
            ...
            while (orderAdjIterator.hasNext()) {
                ...
                try {
                    count =
                            orderHeader.getDelegator().findCountByCondition(
                                    "ReturnAdjustment",
    ...
    public List<GenericValue> getGroupedSubTotal() {
        Delegator delegator = this.getOrderHeader().getDelegator();
        List<GenericValue> records = FastList.newInstance();
        try {
            List<GenericValue> apportionAdjs =
                    delegator.findList(
                            "OrderAdjustmentView",
                            EntityCondition.makeCondition(
                                    EntityCondition.makeCondition(UtilMisc.toMap("orderId", this.getOrderId())),
                                    EntityCondition.makeCondition("orderItemSeqId", EntityOperator.EQUALS, null)), null, null,
                            null, false);
            records = delegator.findByAnd("OrderAdjustmentGrouped", UtilMisc.toMap("orderId", this.getOrderId()));

            for (GenericValue each : apportionAdjs) {
                for (GenericValue groupedTax : records) {
                    if (groupedTax.getString("comments").equals(each.getString("comments"))) {
                        BigDecimal amount = groupedTax.getBigDecimal("amount").subtract(each.getBigDecimal("amount"));
                        amount = amount.setScale(OrderServices.taxDecimals, OrderServices.taxRounding);
                        groupedTax.set("amount", amount);
                    }
                }
            }
