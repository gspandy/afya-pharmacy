...
public class OrderListState {
	...
	protected Map<String, Object> orderTypeState;
	protected Map<Object, Object> orderFilterState;
	protected int orderListSize;

	// parameter to ID maps
	protected static final Map<Object, Object> parameterToOrderStatusId;
	protected static final Map<Object, Object> parameterToOrderTypeId;
	protected static final Map<Object, Object> parameterToFilterId;
	static {
	...
	protected OrderListState() {
		...
		Boolean isSalesManger = OrderMgrUtil.isSalesManger(((GenericValue)Infrastructure.getRequest().getSession().getAttribute("userLogin")).getString("partyId"));
		Boolean isSalesReprensentative = OrderMgrUtil.isSalesReprensentative(((GenericValue)Infrastructure.getRequest().getSession().getAttribute("userLogin")).getString("partyId"));
		Boolean isPurchaseManger = OrderMgrUtil.isPurchaseManger((((GenericValue)Infrastructure.getRequest().getSession().getAttribute("userLogin")).getString("partyId")));
		Boolean isPurchaseReprensentative = OrderMgrUtil.isPurchaseReprensentative(((GenericValue)Infrastructure.getRequest().getSession().getAttribute("userLogin")).getString("partyId"));
		
		if(isSalesManger || isSalesReprensentative)
			orderTypeState.put("view_SALES_ORDER", "Y");
		if(isPurchaseManger || isPurchaseReprensentative)
			orderTypeState.put("view_PURCHASE_ORDER", "Y");
	}
	...
	private void changePaginationState(String viewSizeParam, String viewIndexParam) {
		try {
			...
			Debug.logWarning("Values of " + VIEW_SIZE_PARAM + " [" + viewSizeParam + "] and " + VIEW_INDEX_PARAM + " ["
					+ viewIndexParam + "] must both be Integers. Not paginating order list.", module);
		}
	}
	...
	Boolean isSalesManger = OrderMgrUtil.isSalesManger(((GenericValue)Infrastructure.getRequest().getSession().getAttribute("userLogin")).getString("partyId"));
	Boolean isPurchaseManger = OrderMgrUtil.isPurchaseManger((((GenericValue)Infrastructure.getRequest().getSession().getAttribute("userLogin")).getString("partyId")));
	if(isSalesManger && isPurchaseManger)
		return true;
	else
		return false;
	}
	...
	public List getOrders(String facilityId, Timestamp filterDate, Delegator delegator, HttpServletRequest request,
		...
		if (filterDate != null) {
			...
			andExprs.add(EntityCondition.makeCondition("orderDate", EntityOperator.GREATER_THAN_EQUAL_TO,
					UtilDateTime.getDayStart(filterDate)));
			andExprs.add(EntityCondition.makeCondition("orderDate", EntityOperator.LESS_THAN_EQUAL_TO,
		...
		for (Iterator iter = orderStatusState.keySet().iterator(); iter.hasNext();) {
			...
			if (!hasStatus(status))
				continue;
			statusConditions.add(EntityCondition.makeCondition("statusId", EntityOperator.EQUALS,
					parameterToOrderStatusId.get(status)));
		}
		...
		for (Iterator iter = orderTypeState.keySet().iterator(); iter.hasNext();) {
			...
			if (!hasType(type))
				continue;
			typeConditions.add(EntityCondition.makeCondition("orderTypeId", EntityOperator.EQUALS,
					parameterToOrderTypeId.get(type)));
		}
    ...
    /* Fixed for order Approval Role Based */
		...
		List orderCreators = new ArrayList();
		...
		EntityCondition condition = EntityCondition.makeCondition(UtilMisc.toList(EntityCondition.makeCondition(
				...
				EntityCondition.makeCondition("roleTypeId", EntityOperator.IN, UtilMisc.toList("SALES_MGR","MANAGER")))));
		
		List<GenericValue> partyRole =  delegator.findList("PartyRole", condition, UtilMisc.toSet("partyId"), null, null, false);				
		if (partyRole.size()>0) {
		...
		List<GenericValue> userLogins = delegator.findList("UserLogin",
				EntityCondition.makeCondition("partyId", EntityOperator.IN, orderCreators),
				UtilMisc.toSet("userLoginId"), null, null, false);

		orderCreators.clear();
		for(GenericValue ul : userLogins){
			orderCreators.add(ul.getString("userLoginId"));
		}
		
		//allConditions.add(EntityCondition.makeCondition("createdBy", EntityOperator.IN, orderCreators));

		/*  End For order Approval Role Based*/
		...
		if("admin".equals(partyId)){
			...
			iterator = delegator.find("OrderHeader", queryConditionsList, null, null,UtilMisc.toList("orderDate DESC"), options);
		    	...
		    	allConditions.add(EntityCondition.makeCondition("createdBy", EntityOperator.IN, orderCreators));
				EntityCondition createdByList = EntityCondition.makeCondition(allConditions, EntityOperator.AND);
				iterator = delegator.find("OrderHeader", createdByList, null, null,UtilMisc.toList("orderDate DESC"), options);
		    }
