...
public class OrderReturnServices {
    ...
    private static Map sendReturnNotificationScreen(DispatchContext dctx, Map context, String emailType) {
        ...
        if (UtilValidate.isNotEmpty(productStoreId)) {
            ...
            if (productStoreEmail != null && emailAddress != null) {
                ...
                Map bodyParameters = UtilMisc.toMap("returnHeader", returnHeader, "returnItems", returnItems, "uiLabelMap",
                ...
                try {
                    ...
                    return ServiceUtil.returnError(UtilProperties.getMessage(resource_error, "OrderProblemSendingEmail",
                            locale));
                }
                ...
                if (sendResp != null && ServiceUtil.isError(sendResp)) {
                    ...
                    return ServiceUtil.returnError(
                            UtilProperties.getMessage(resource_error, "OrderProblemSendingEmail", locale), null, null,
                            sendResp);
                }
    ...
    public static Map<String, Object> autoCancelReplacementOrders(DispatchContext dctx,
        ...
        try {
            returnHeaders = delegator.findList("ReturnHeader", EntityCondition.makeCondition(
                            EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "RETURN_ACCEPTED"),
                            EntityOperator.AND,
                            EntityCondition.makeCondition("returnHeaderTypeId", EntityOperator.EQUALS, "CUSTOMER_RETURN")), null,
        ...
        for (GenericValue returnHeader : returnHeaders) {
            ...
            String daysTillCancelStr = UtilProperties.getPropertyValue("order.properties",
                ...
                if (cancelDate.equals(nowDate) || nowDate.after(cancelDate)) {
                    try {
                        List<GenericValue> returnItems = delegator.findList("ReturnItem", EntityCondition.makeCondition(
                                ...
                                EntityOperator.AND, EntityCondition.makeCondition("returnTypeId", EntityOperator.EQUALS,
                        ...
                        for (GenericValue returnItem : returnItems) {
                            ...
                            if (returnItemResponse != null) {
                                ...
                                GenericValue orderHeader = delegator.findOne("OrderHeader",
                                        UtilMisc.toMap("orderId", replacementOrderId), false);
    ...
    public static Map getReturnableQuantity(DispatchContext dctx, Map context) {
       ...
       /* try {
            GenericValue productStore = orderItem.getRelatedOne("OrderHeader").getRelatedOne("ProductStore");
            if(productStore!=null && "Y".equals(productStore.getString("pricesIncludeTax"))){
                    ...
                    unitPrice = unitPrice.divide(new BigDecimal(1).add(unitPrice.divide(PERCENT_SCALE, salestaxCalcDecimals, salestaxRounding)),salestaxCalcDecimals, salestaxRounding);
                    result.put("returnablePrice",unitPrice);
            }else{
                result.put("returnablePrice", orderItem.getBigDecimal("unitPrice"));
            }
        } catch (GenericEntityException e) {
    ...
    public static Map getReturnableItems(DispatchContext dctx, Map context) {
        ...
        if (orderHeader != null) {
            ...
            EntityConditionList whereConditions = EntityCondition.makeCondition(
                    UtilMisc.toList(
		 ...
		 */
            ...
            try {
                orderItemQuantitiesIssued = delegator.findList("OrderItemQuantityReportGroupByItem", whereConditions,
                        UtilMisc.toSet("orderId", "orderItemSeqId", "quantityIssued"), UtilMisc.toList("orderItemSeqId"),
                        null, false);
            } catch (GenericEntityException e) {
                Debug.logError(e, module);
                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
            ...
            if (orderItemQuantitiesIssued != null) {
                ...
                while (i.hasNext()) {
                    ...
                    if (serviceResult.containsKey(ModelService.ERROR_MESSAGE)) {
                        ...
                        // Don't add the OrderItem to the map of returnable OrderItems if there isn't any returnable
                        // quantity.
                        ...
                        BigDecimal quantity =  (BigDecimal)serviceResult.get("returnableQuantity");
                        BigDecimal unitPrice =  (BigDecimal)serviceResult.get("returnablePrice");
                        returnInfo.put("returnableQuantity", quantity);
                        returnInfo.put("returnablePrice", unitPrice);
                             ...
                             GenericValue productStore = orderHeader.getRelatedOne("ProductStore");
                                 ...
                                 if (UtilValidate.isNotEmpty(itemAdjustments)) {
                                     ...
                                     while (itemAdjustmentsIt.hasNext()) {
                                             ...
                                             if ((productStore != null && "N".equals(productStore.getString("pricesIncludeTax")))
                                                     || !"SALES_TAX".equals(itemAdjustment.getString("orderAdjustmentTypeId"))) {
                                             ...
                                             adjustmentAmount = adjustmentAmount.add((BigDecimal) itemAdjustment.get("amount"));
                ...
                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error, "OrderErrorNoOrderItemsFound",
                        locale));
            }
        } else {
            return ServiceUtil.returnError(UtilProperties.getMessage(resource_error, "OrderErrorUnableToFindOrderHeader",
                    locale));
        }
                ...
                if (currentStatus != null && "RETURN_ACCEPTED".equals(currentStatus)) {
                    try {
                        dispatcher.runSync("updateReturnHeader", UtilMisc.<String, Object>toMap("returnId", returnId,
                                "statusId", "RETURN_RECEIVED", "userLogin", userLogin));
                    } catch (GenericServiceException e) {
                        Debug.logError(e, module);
                        return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
                    ...
                    try {
                        dispatcher.runSync("updateReturnHeader", UtilMisc.<String, Object>toMap("returnId", returnId,
                                "statusId", "RETURN_COMPLETED", "userLogin", userLogin));
                    } catch (GenericServiceException e) {
                        Debug.logError(e, module);
                        return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
                                "OrderErrorUnableToCreateReturnStatusHistory", locale));
    ...
    public static Map processCreditReturn(DispatchContext dctx, Map context) {
        ...
        BigDecimal adjustments = getReturnAdjustmentTotal(delegator,
                UtilMisc.toMap("returnId", returnId, "returnTypeId", "RTN_CREDIT"));
        ...
        if (returnHeader != null && (UtilValidate.isNotEmpty(returnItems) || adjustments.compareTo(ZERO) > 0)) {
            ...
            // if both billingAccountId and finAccountId are supplied, look for productStore.storeCreditAccountEnumId
            // preference
            if (finAccountId != null && billingAccountId != null) {
                Debug.logWarning(
                        "FinAccount and BillingAccount both are supplied for storing credit, priority will be given to ProductStore preference. Default is FinAccount.",
                        module);
            ...
            if (finAccountId == null && billingAccountId == null) {
                ...
                try {
                    billingAccounts = delegator.findByAnd("BillingAccountRoleAndAddress",
                            UtilMisc.toMap("partyId", fromPartyId, "roleTypeId", "BILL_TO_CUSTOMER"));
                } catch (GenericEntityException e) {
                    return ServiceUtil.returnError(e.getMessage());
                }
                ...
                // if no billing account with negative balance is found, look for productStore.storeCreditAccountEnumId
                // settings
                    ...
                    if (productStore != null && productStore.getString("storeCreditAccountEnumId") != null
                        ...
                        if (UtilValidate.isNotEmpty(billingAccounts)) {
                            ...
                            if (ServiceUtil.isError(results)) {
                                Debug.logError("Error creating BillingAccount: " + results.get(ModelService.ERROR_MESSAGE),
                                        module);
                                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
                                        "OrderErrorWithCreateBillingAccount", locale)
                        ...
                        try {
                            finAccounts = delegator.findByAnd("FinAccountAndRole", UtilMisc.toMap("partyId", fromPartyId,
                        ...
                        if (finAccountId == null) {
                            ...
                            try {
                                finAccountRoleResult = dispatcher.runSync("createFinAccountRole", UtilMisc.toMap(
                                        "finAccountId", finAccountId, "partyId", fromPartyId, "roleTypeId", "OWNER",
                                        "userLogin", userLogin));
                            } catch (GenericServiceException e) {
            ...
            if (finAccountId != null) {
                ...
                try {
                    finAccountTransResult = dispatcher.runSync("createFinAccountTrans", UtilMisc.toMap("finAccountId",
                            finAccountId, "finAccountTransTypeId", "DEPOSIT", "partyId", toPartyId, "amount", creditTotal,
                            "reasonEnumId", "FATR_REFUND", "userLogin", userLogin));
                } catch (GenericServiceException e) {
            ...
            payment.set("partyIdFrom", toPartyId); // if you receive a return FROM someone, then you'd have to give a return
            // TO that person
            ...
            payment.set("statusId", "PMNT_CONFIRMED"); // set the status to confirmed so nothing else can happen to the
            // payment
            if (billingAccountId != null) {
            ...
            try {
                ...
                if (ServiceUtil.isError(serviceResults)) {
                    return ServiceUtil
                            .returnError("Could not create ReturnItemResponse record", null, null, serviceResults);
                }
            } catch (GenericServiceException e) {
            ...
            for (Iterator itemsIter = returnItems.iterator(); itemsIter.hasNext(); ) {
                GenericValue item = (GenericValue) itemsIter.next();
                Map returnItemMap = UtilMisc.toMap("returnItemResponseId", itemResponseId, "returnId",
                        item.get("returnId"), "returnItemSeqId", item.get("returnItemSeqId"), "statusId",
                        "RETURN_COMPLETED", "userLogin", userLogin);
                // store the item changes (attached responseId)
                try {
                    serviceResults = dispatcher.runSync("updateReturnItem", returnItemMap);
                    if (ServiceUtil.isError(serviceResults)) {
                        return ServiceUtil.returnError(
                                UtilProperties.getMessage(resource_error, "OrderProblemStoringReturnItemUpdates", locale),
            ...
            if (billingAccountId != null) {
                ...
                try {
                    serviceResults = dispatcher
                            .runSync("createPaymentApplicationsFromReturnItemResponse", UtilMisc.<String, Object>toMap(
                                    "returnItemResponseId", itemResponseId, "userLogin", userLogin));
                    if (ServiceUtil.isError(serviceResults)) {
                        return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
     ...
     */
    public static BigDecimal getBillingAccountBalance(String billingAccountId, DispatchContext dctx)
            throws GenericEntityException {
        Delegator delegator = dctx.getDelegator();
        GenericValue billingAccount = delegator.findByPrimaryKey("BillingAccount",
        ...
        EntityConditionList whereConditions = EntityCondition.makeCondition(UtilMisc.toList(
                ...
                EntityCondition.makeCondition("statusId", EntityOperator.NOT_IN,
                        UtilMisc.toList("ORDER_CANCELLED", "ORDER_REJECTED")),
        ...
        List orderPaymentPreferenceSums = delegator.findList("OrderPurchasePaymentSummary", whereConditions,
     ...
     */
    private static Map createBillingAccountFromReturn(GenericValue returnHeader, List returnItems,
        ...
        try {
            ...
            // find the minimum storeCreditValidDays of all the ProductStores associated with all the Orders on the Return,
            // skipping null ones
            ...
            Map input = UtilMisc.toMap("accountLimit", BigDecimal.ZERO, "description", "Credit Account for Return #"
            ...
            input = UtilMisc.toMap("billingAccountId", billingAccountId, "partyId", returnHeader.get("fromPartyId"),
            ...
            if (ServiceUtil.isError(roleResults)) {
                Debug.logError("Error with createBillingAccountRole: " + roleResults.get(ModelService.ERROR_MESSAGE),
                        module);
                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
            ...
            return ServiceUtil.returnError(UtilProperties.getMessage(resource_error, "OrderProblemsCreatingBillingAccount",
                    locale));
        } catch (GenericServiceException e) {
        ...
        if (UtilValidate.isEmpty(orderPayPrefs)) {
            ...
            for (GenericValue returnItemResponse : returnItemResponses) {
                ...
                if ("RETURN_RECEIVED".equals(returnHeader.getString("statusId"))) {
                    ...
                    try {
                        serviceResult = dispatcher.runSync("processRefundReturn",
                                UtilMisc.toMap("returnId", returnId, "returnTypeId", returnTypeId, "userLogin", userLogin));
                    } catch (GenericServiceException e) {
    ...
    public static Map processRefundReturn(DispatchContext dctx, Map context) {
        ...
        BigDecimal adjustments = getReturnAdjustmentTotal(delegator,
                UtilMisc.toMap("returnId", returnId, "returnTypeId", returnTypeId));
            ...
            while (itemByOrderIt.hasNext()) {
                ...
                try {
                    ...
                    List exprs = UtilMisc.toList(
                            EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "PAYMENT_SETTLED"),
                    ...
                    // Check for replacement order
                    if (UtilValidate.isEmpty(orderPayPrefs)) {
                        List<GenericValue> orderItemAssocs = delegator.findByAnd("OrderItemAssoc",
                                UtilMisc.toMap("toOrderId", orderId, "orderItemAssocTypeId", "REPLACEMENT"));
                        if (UtilValidate.isNotEmpty(orderItemAssocs)) {
                            ...
                            orderHeader = delegator.findByPrimaryKey("OrderHeader",
                                    UtilMisc.toMap("orderId", originalOrderId));
                            orderPayPrefs = orderHeader.getRelated("OrderPaymentPreference", UtilMisc.toList("-maxAmount"));
                            orderPayPrefs = EntityUtil.filterByOr(orderPayPrefs, exprs);
                ...
                // Determine the fall-through refund paymentMethodId from the PartyAcctgPreference of the owner of the
                // productStore for the order
                ...
                if (UtilValidate.isEmpty(productStore) || UtilValidate.isEmpty(productStore.get("payToPartyId"))) {
                    ...
                    try {
                        acctgPreferencesResult = dispatcher.runSync("getPartyAccountingPreferences", UtilMisc.toMap(
                                "organizationPartyId", productStore.get("payToPartyId"), "userLogin", userLogin));
                    } catch (GenericServiceException e) {
                        Debug.logError(e,
                                "Error retrieving PartyAcctgPreference for partyId " + productStore.get("payToPartyId"),
                                module);
                        return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
                    ...
                    if (UtilValidate.isNotEmpty(orgAcctgPref)) {
                        try {
                            ...
                            Debug.logError(
                                    "Error retrieving related refundPaymentMethod from PartyAcctgPreference for partyId "
                ...
                // We break the OPPs down this way because we need to process the refunds to payment methods in a particular
                // order
                ...
                // getOrderPaymentPreferenceTotalByType has been called because getReceivedPaymentTotalsByPaymentMethod does
                // not
                // return payments captured from Billing Account.This is because when payment is captured from Billing
                // Account
                // then no entry is maintained in Payment entity.
                BigDecimal receivedPaymentTotalsByBillingAccount = orderReadHelper
                        .getOrderPaymentPreferenceTotalByType("EXT_BILLACT");

			/*
             * Go through the OrderPaymentPreferences and determine how much remains to be refunded for each. Then group
			 * these refund amounts and orderPaymentPreferences by paymentMethodTypeId. That is, the intent is to get
			 * the refundable amounts per orderPaymentPreference, grouped by payment method type.
			 */
                ...
                while (oppit.hasNext()) {
                    ...
                    String orderPayPrefKey = orderPayPref.getString("paymentMethodId") != null ? orderPayPref
                            .getString("paymentMethodId") : orderPayPref.getString("paymentMethodTypeId");
                    ...
                    if (receivedPaymentTotalsByPaymentMethod.containsKey(orderPayPrefKey)) {
                        orderPayPrefReceivedTotal = orderPayPrefReceivedTotal.add(
                                (BigDecimal) receivedPaymentTotalsByPaymentMethod.get(orderPayPrefKey)).setScale(decimals,
                                rounding);
                    }
                    ...
                    if (refundedTotalsByPaymentMethod.containsKey(orderPayPrefKey)) {
                        orderPayPrefRefundedTotal = orderPayPrefRefundedTotal.add(
                                (BigDecimal) refundedTotalsByPaymentMethod.get(orderPayPrefKey)).setScale(decimals,
                                rounding);
                    }
                ...
                EntityConditionList pmtConditionList = EntityCondition.makeCondition(UtilMisc.toList(EntityCondition
                                .makeCondition("paymentMethodTypeId", EntityOperator.NOT_IN, orderedRefundPaymentMethodTypes)),
                        EntityOperator.AND);
                List otherPaymentMethodTypes = new ArrayList();
                try {
                    otherPaymentMethodTypes = delegator.findList("PaymentMethodType", pmtConditionList, null, null, null,
                            true);
                } catch (GenericEntityException e) {
                ...
                orderedRefundPaymentMethodTypes.addAll(EntityUtil.getFieldListFromEntityList(otherPaymentMethodTypes,
                        "paymentMethodTypeId", true));

                // Iterate through the specified sequence of paymentMethodTypes, refunding to the correct
                ...
                while (orpmtit.hasNext() && amountLeftToRefund.compareTo(ZERO) == 1) {
                    ...
                    if (prefSplitMap.containsKey(paymentMethodTypeId)) {
                        ...
                        while (pmtppit.hasNext() && amountLeftToRefund.compareTo(ZERO) == 1) {
                            ...
                            GenericValue orderPaymentPreference = (GenericValue) orderPaymentPrefDetails
                                    .get("orderPaymentPreference");
                            BigDecimal orderPaymentPreferenceAvailable = (BigDecimal) orderPaymentPrefDetails
                                    .get("availableTotal");

                            // Refund up to the maxAmount for the paymentPref, or whatever is left to refund if that's less
                            // than the maxAmount
                            ...
                            String returnItemStatusId = "RETURN_COMPLETED"; // generally, the return item will be considered
                            // complete after this
                            ...
                            if (electronicTypes.contains(paymentMethodTypeId)) {
                                try {
                                    ...
                                    serviceResult = dispatcher.runSync("refundPayment", UtilMisc.<String, Object>toMap(
                                            "orderPaymentPreference", orderPaymentPreference, "refundAmount",
                                    ...
                                    if (ServiceUtil.isError(serviceResult) || ServiceUtil.isFailure(serviceResult)) {
                                        Debug.logError(
                                                "Error in refund payment: " + ServiceUtil.getErrorMessage(serviceResult),
                                                module);
                                        continue;
                                    }
                                ...
                                try {
                                    ...
                                    serviceResult = dispatcher.runSync("refundBillingAccountPayment", UtilMisc
                                            .<String, Object>toMap("orderPaymentPreference", orderPaymentPreference,
                                                    "refundAmount", amountToRefund.setScale(decimals, rounding),
                                                    "userLogin", userLogin));
                                    if (ServiceUtil.isError(serviceResult) || ServiceUtil.isFailure(serviceResult)) {
                                        Debug.logError(
                                                "Error in refund payment: "
                                                        + ServiceUtil.getErrorMessage(serviceResult), module);
                                        continue;
                                    }
                                    paymentId = (String) serviceResult.get("paymentId");
                                } catch (GenericServiceException e) {
                                ...
                                try {
                                    Map input = UtilMisc.toMap("userLogin", userLogin, "amount", amountLeftToRefund,
                                    ...
                                    paymentId = (String) serviceResult.get("paymentId");
                                    returnItemStatusId = "RETURN_MAN_REFUND"; // however, in this case we should flag it
                                    // as a manual refund
                                } catch (GenericServiceException e) {
                                    return ServiceUtil.returnError(e.getMessage());
                                }
                            ...
                            try {
                                ...
                                if (ServiceUtil.isError(serviceResults)) {
                                    return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
                                                    "OrderProblemsCreatingReturnItemResponseEntity", locale), null, null,
                                            serviceResults);
                                }
                            } catch (GenericServiceException e) {
                            ...
                            while (itemsIter.hasNext()) {
                                ...
                                Map returnItemMap = UtilMisc.toMap("returnItemResponseId", responseId, "returnId",
                                        item.get("returnId"), "returnItemSeqId", item.get("returnItemSeqId"), "statusId",
                                        returnItemStatusId, "userLogin", userLogin);
                                // Debug.log("Updating item status", module);
                                try {
                                    serviceResults = dispatcher.runSync("updateReturnItem", returnItemMap);
                                    if (ServiceUtil.isError(serviceResults)) {
                                        return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
                                                        "OrderProblemUpdatingReturnItemReturnItemResponseId", locale), null, null,
                                                serviceResults);
                                    }
                                } catch (GenericServiceException e) {
                            ...
                            try {
                                serviceResults = dispatcher.runSync("createPaymentApplicationsFromReturnItemResponse",
                                        UtilMisc.<String, Object>toMap("returnItemResponseId", responseId, "userLogin",
                                                userLogin));
                                if (ServiceUtil.isError(serviceResults)) {
                                    return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
                                                    "OrderProblemUpdatingReturnItemReturnItemResponseId", locale), null, null,
    ...
    public static Map refundBillingAccountPayment(DispatchContext dctx, Map context) {
        ...
        // if the original order was paid with a billing account, then go find the billing account from the order and
        // associate this refund with that billing account
        ...
        if ("EXT_BILLACT".equals(paymentPref.getString("paymentMethodTypeId"))) {
            ...
            if (UtilValidate.isNotEmpty(billingAccount.getString("billingAccountId"))) {
                try {
                    Map paymentApplResult = dispatcher.runSync("createPaymentApplication", UtilMisc.<String, Object>toMap(
                            "paymentId", paymentId, "billingAccountId", billingAccount.getString("billingAccountId"),
    ...
    public static Map createPaymentApplicationsFromReturnItemResponse(DispatchContext dctx, Map context) {
        ...
        // the strategy for this service is to get a list of return invoices via the return items -> return item billing
        // relationships
        ...
        try {
            GenericValue response = delegator.findByPrimaryKey("ReturnItemResponse",
                    UtilMisc.toMap("returnItemResponseId", responseId));
            if (response == null) {
            ...
            for (Iterator itemIter = items.iterator(); itemIter.hasNext(); ) {
                ...
                for (Iterator billIter = billings.iterator(); billIter.hasNext(); ) {
                    ...
                    // put the invoice in the map if it doesn't already exist (a very loopy way of doing group by invoiceId
            ...
            Map invoiceTotals = FastMap.newInstance(); // key is invoiceId, value is the sum of all billings for that
            // invoice
            ...
            for (Iterator iter = returnInvoices.values().iterator(); iter.hasNext(); ) {
                ...
                for (Iterator billIter = billings.iterator(); billIter.hasNext(); ) {
                    GenericValue billing = (GenericValue) billIter.next();
                    runningTotal = runningTotal.add(billing.getBigDecimal("amount")
                            .multiply(billing.getBigDecimal("quantity")).setScale(decimals, rounding));
                }
            ...
            for (Iterator iter = returnInvoices.values().iterator(); iter.hasNext(); ) {
                ...
                BigDecimal amountApplied = responseAmount.multiply(invoiceTotal).divide(grandTotal, decimals, rounding)
                ...
                if (paymentId != null) {
                    ...
                    if (Debug.verboseOn()) {
                        Debug.logInfo(
                                "Created PaymentApplication for response with amountApplied " + amountApplied.toString(),
                                module);
                    }
                }
            }
    ...
    public static Map processReplacementReturn(DispatchContext dctx, Map context) {
        ...
        if (returnHeader != null && UtilValidate.isNotEmpty(returnItems)) {
            ...
            /*
			 * OrderShipmentPreference is a deprecated entity List shipmentPrefs = new ArrayList(); List orderSp = null;
			 * try { orderSp = orderHeader.getRelated("OrderShipmentPreference"); } catch (GenericEntityException e) {
			 * Debug.logError(e, module); } if (orderSp != null) { Iterator orderSpi = orderSp.iterator(); while
			 * (orderSpi.hasNext()) { GenericValue v = (GenericValue) orderSpi.next();
			 * shipmentPrefs.add(GenericValue.create(v)); } orderMap.put("orderShipmentPreferences", shipmentPrefs); }
			 */
                ...
                List orderItemShipGroupIds = new ArrayList(); // this is used to store the ship group ids of the groups
                // already added to the orderItemShipGroupInfo list
                        ...
                        if (orderItem != null) {
                            ...
                            if (quantity != null && unitPrice != null) {
                                ...
                                if ("CUSTOMER_RETURN".equals(returnHeaderTypeId)) {
                                    try {
                                        if (UtilValidate.isNotEmpty(product)) {
                                            GenericValue refurbItemAssoc = EntityUtil.getFirst(EntityUtil
                                                    .filterByDate(product.getRelated("MainProductAssoc",
                                    ...
                                    if (UtilValidate.isNotEmpty(refurbItem)) {
                                        ...
                                        try {
                                            Map invReqResult = dispatcher.runSync("isStoreInventoryAvailable", UtilMisc
                                                    .toMap("productStoreId", orderHeader.get("productStoreId"),
                                                            "productId", refurbItem.getString("productId"), "product",
                                                            refurbItem, "quantity", quantity));
                                            if (ServiceUtil.isError(invReqResult)) {
                                                Debug.logError(
                                                        "Error calling isStoreInventoryAvailable service, result is: "
                                                                + invReqResult, module);
                                            } else {
                                            ...
                                            Debug.logError(e,
                                                    "Fatal error calling inventory checking services: " + e.toString(),
                                                    module);
                                        }
                                ...
                                GenericValue newItem = delegator.makeValue("OrderItem",
                                        UtilMisc.toMap("orderItemSeqId", UtilFormatOut.formatPaddedNumber(itemCount++, 5)));
                                if (UtilValidate.isEmpty(refurbItem)) {
                                    ...
                                    newItem.set("itemDescription", ProductContentWrapper.getProductContentAsText(
                                            refurbItem, "PRODUCT_NAME", locale, null));
                                }
                                ...
                                try {
                                    orderItemShipGroupAssoc = EntityUtil.getFirst(orderItem
                                            .getRelated("OrderItemShipGroupAssoc"));
                                    if (orderItemShipGroupAssoc != null) {
                                        if (!orderItemShipGroupIds.contains(orderItemShipGroupAssoc
                                                .getString("shipGroupSeqId"))) {
                                            GenericValue orderItemShipGroup = orderItemShipGroupAssoc
                                                    .getRelatedOne("OrderItemShipGroup");
                                        ...
                                        GenericValue newOrderItemShipGroupAssoc = delegator.makeValue(
                                                "OrderItemShipGroupAssoc", UtilMisc.toMap("orderItemSeqId",
                                                        newItem.getString("orderItemSeqId"), "shipGroupSeqId",
                                                        orderItemShipGroupAssoc.getString("shipGroupSeqId"), "quantity",
                                                        quantity));
                                ...
                                // Create an association between the replacement order item and the order item of the
                                // original order
                                GenericValue newOrderItemAssoc = delegator.makeValue("OrderItemAssoc", UtilMisc.toMap(
                                        "orderId", orderHeader.getString("orderId"), "orderItemSeqId",
                                        orderItem.getString("orderItemSeqId"), "shipGroupSeqId", "_NA_",
                                        "toOrderItemSeqId", newItem.getString("orderItemSeqId"), "toShipGroupSeqId",
                                    ...
                                    if (UtilValidate.isNotEmpty(repairItems)) {
                                        ...
                                        while (repairItemIt.hasNext()) {
                                            ...
                                            if (UtilValidate.isNotEmpty(repairItemProduct)) {
                                                ...
                                                newItem.set("itemDescription", ProductContentWrapper
                                                        .getProductContentAsText(repairItemProduct, "PRODUCT_NAME", locale,
                                                                null));
                                                newItem.set("statusId", "ITEM_CREATED");
                                                orderItems.add(newItem);
                                                additionalItemTotal = additionalItemTotal.add(repairQuantity
                                                        .multiply(repairUnitPrice));
                                                ...
                                                // Create an association between the repair order item and the order item of
                                                // the original order
                                                newOrderItemAssoc = delegator.makeValue("OrderItemAssoc", UtilMisc.toMap(
                                                        "orderId", orderHeader.getString("orderId"), "orderItemSeqId",
                                                        orderItem.getString("orderItemSeqId"), "shipGroupSeqId", "_NA_",
                                                        "toOrderItemSeqId", newItem.getString("orderItemSeqId"),
                                                        "toShipGroupSeqId", "_NA_", "orderItemAssocTypeId", "REPLACEMENT"));
                                                orderItemAssocs.add(newOrderItemAssoc);
                                            }
                                        }
                                    }
                ...
                // we'll assume new order is under same terms as original. note orderTerms is a required parameter of
                // storeOrder
                try {
                    orderMap.put("orderTerms", orderHeader.getRelated("OrderTerm"));
                } catch (GenericEntityException e) {
                    Debug.logError(e,
                            "Cannot create replacement order because order terms for original order are not available",
                            module);
                }
                ...
                try {
                    ...
                    Debug.logError(e,
                            "Cannot create replacement order because order roles for original order are not available",
                            module);
                }
                ...
                if (createdOrderId != null) {
                    if ("RETURN_ACCEPTED".equals(returnHeader.get("statusId"))
                            && "RTN_WAIT_REPLACE_RES".equals(returnTypeId)) {
                        Map serviceResult = null;
                        try {
                            serviceResult = dispatcher.runSync("changeOrderStatus", UtilMisc.toMap("orderId",
                                    createdOrderId, "statusId", "ORDER_HOLD", "userLogin", userLogin));
                        } catch (GenericServiceException e) {
                            Debug.logError(e, "Service invocation error, status changes were not updated for order #"
                                    + createdOrderId, module);
                            return ServiceUtil.returnError(e.getMessage());
                        }
                        if (ServiceUtil.isError(serviceResult)) {
                            return ServiceUtil.returnError(ServiceUtil.getErrorMessage(serviceResult));
                    ...
                    try {
                        ...
                        if (ServiceUtil.isError(createReturnItemResponseResult)) {
                            return ServiceUtil.returnError("Could not create ReturnItemResponse record", null, null,
                                    createReturnItemResponseResult);
                        }
                    ...
                    while (updateReturnItemIter.hasNext()) {
                        ...
                        try {
                            ...
                            if (ServiceUtil.isError(updateReturnItemResult)) {
                                return ServiceUtil.returnError("Could not update ReturnItem record", null, null,
                                        updateReturnItemResult);
                            }
                        } catch (GenericServiceException e) {
     ...
     *
     * @param returnItems
     * @param returnItemsByOrderId
     * @param totalByOrder
     * @param delegator
     * @param returnId
     * @param returnTypeId
     */
    public static void groupReturnItemsByOrder(List returnItems, Map returnItemsByOrderId, Map totalByOrder,
                                               Delegator delegator, String returnId, String returnTypeId) {
            ...
            if (orderId != null) {
                if (returnItemsByOrderId != null) {
                    ...
                    if (totalByOrder != null) {
                        ...
                        BigDecimal newTotal = existingTotal.add(thisTotal).add(
                                getReturnAdjustmentTotal(delegator, condition));
        ...
        if ((totalByOrder != null) && (totalByOrder.keySet() != null)) {
            ...
            while (orderIterator.hasNext()) {
                ...
                Map condition = UtilMisc.toMap("returnId", returnId, "returnItemSeqId",
                        org.ofbiz.common.DataModelConstants.SEQ_ID_NA, "returnTypeId", returnTypeId);
                BigDecimal existingTotal = ((BigDecimal) totalByOrder.get(orderId)).add(getReturnAdjustmentTotal(delegator,
                        condition));
    ...
    public static Map checkPaymentAmountForRefund(DispatchContext dctx, Map context) {
        ...
        try {
            serviceResult = dispatcher.runSync("getReturnAmountByOrder",
                    org.ofbiz.base.util.UtilMisc.toMap("returnId", returnId));
        } catch (GenericServiceException e) {
            ...
            while (orderIterator.hasNext()) {
                ...
                if (returnAmount == null) {
                    ...
                    if (returnAmount.subtract(grandTotal).compareTo(new BigDecimal("0.01")) > 0) {
                        Debug.logError("Order [" + orderId + "] refund amount[ " + returnAmount + "] exceeds order total ["
                                + grandTotal + "]", module);
                        return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
    ...
    public static Map<String, Object> createReturnAdjustment(DispatchContext dctx, Map<String, Object> context) {
        ...
        if (orderAdjustmentId != null) {
            try {
                ...
                returnHeader = delegator.findByPrimaryKey("ReturnHeader", UtilMisc.toMap("returnId", returnId));
                String returnHeaderTypeId = ((returnHeader != null) && (returnHeader.getString("returnHeaderTypeId") != null)) ? returnHeader
                        .getString("returnHeaderTypeId") : "CUSTOMER_RETURN";
                returnItemTypeMap = delegator.findByPrimaryKey(
                        "ReturnItemTypeMap",
                        UtilMisc.toMap("returnHeaderTypeId", returnHeaderTypeId, "returnItemMapKey",
                                orderAdjustment.get("orderAdjustmentTypeId")));
                ...
                if ((returnItemSeqId != null) && !("_NA_".equals(returnItemSeqId))) {
                    ...
                    if (UtilValidate.isNotEmpty(orderAdjustment.getString("orderItemSeqId"))
                        ...
                        returnItem = EntityUtil.getFirst(delegator.findByAnd("ReturnItem", UtilMisc.toMap("returnId",
                                returnId, "orderId", orderAdjustment.getString("orderId"), "orderItemSeqId",
        ...
        // if orderAdjustmentTypeId is empty, ie not found from orderAdjustmentId, then try to get returnAdjustmentTypeId
        ...
        if (returnItem != null) { // returnAdjustment for returnItem
            if (needRecalculate(returnAdjustmentTypeId)) {
                Debug.logInfo(
                        "returnPrice:" + returnItem.getBigDecimal("returnPrice") + ",returnQuantity:"
                ...
                BigDecimal returnTotal = returnItem.getBigDecimal("returnPrice").multiply(
                ...
                amount = getAdjustmentAmount("RET_SALES_TAX_ADJ".equals(returnAdjustmentTypeId) || "RET_EXCISE_TAX_ADJ".equals(returnAdjustmentTypeId), returnTotal, orderTotal,
        ...
        try {
            ...
            return ServiceUtil
                    .returnError(UtilProperties.getMessage(resource, "OrderCreateReturnAdjustmentFailed", locale));
        }
    }
    ...
    public static Map<String, Object> updateReturnAdjustment(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            returnAdjustment = delegator.findByPrimaryKey("ReturnAdjustment",
                    UtilMisc.toMap("returnAdjustmentId", context.get("returnAdjustmentId")));
            if (returnAdjustment != null) {
                returnItem = delegator.findByPrimaryKey(
                        "ReturnItem",
                        UtilMisc.toMap("returnId", returnAdjustment.get("returnId"), "returnItemSeqId",
                                returnAdjustment.get("returnItemSeqId")));
                returnAdjustmentTypeId = returnAdjustment.getString("returnAdjustmentTypeId");
            }

            // calculate the returnAdjustment amount
            if (returnItem != null) { // returnAdjustment for returnItem
                BigDecimal originalReturnPrice = (context.get("originalReturnPrice") != null) ? ((BigDecimal) context
                        .get("originalReturnPrice")) : returnItem.getBigDecimal("returnPrice");
                BigDecimal originalReturnQuantity = (context.get("originalReturnQuantity") != null) ? ((BigDecimal) context
                        .get("originalReturnQuantity")) : returnItem.getBigDecimal("returnQuantity");

                if (needRecalculate(returnAdjustmentTypeId)) {
                    BigDecimal returnTotal = returnItem.getBigDecimal("returnPrice").multiply(
                            returnItem.getBigDecimal("returnQuantity"));
                    BigDecimal originalReturnTotal = originalReturnPrice.multiply(originalReturnQuantity);
                    amount = getAdjustmentAmount("RET_SALES_TAX_ADJ".equals(returnAdjustmentTypeId) || "RET_EXCISE_TAX_ADJ".equals(returnAdjustmentTypeId), returnTotal,
                            originalReturnTotal, returnAdjustment.getBigDecimal("amount"));
                } else {
                    amount = (BigDecimal) context.get("amount");
                }
            } else { // returnAdjustment for returnHeader
            ...
            if (UtilValidate.isNotEmpty(amount)) {
                ...
                Debug.logInfo("Update ReturnAdjustment with Id:" + context.get("returnAdjustmentId") + " to amount "
                        + amount + " successfully.", module);
                result = ServiceUtil.returnSuccess(UtilProperties.getMessage(resource, "OrderUpdateReturnAdjustment",
    ...
    public static boolean needRecalculate(String returnAdjustmentTypeId) {
        return "RET_PROMOTION_ADJ".equals(returnAdjustmentTypeId) || "RET_DISCOUNT_ADJ".equals(returnAdjustmentTypeId)
                || "RET_SALES_TAX_ADJ".equals(returnAdjustmentTypeId) || "RET_EXCISE_TAX_ADJ".equals(returnAdjustmentTypeId);
     ...
     */
    public static Map filterServiceContext(DispatchContext dctx, String serviceName, Map context)
            throws GenericServiceException {
     ...
     */
    public static BigDecimal getAdjustmentAmount(boolean isSalesTax, BigDecimal returnTotal, BigDecimal originalTotal,
                                                 BigDecimal amount) {
    ...
    public static Map generateDebitCreditNoteXML(DispatchContext dctx, Map serviceContext) {
        LocalDispatcher dispatcher = dctx.getDispatcher();
        Delegator delegator = dctx.getDelegator();
        GenericValue userLogin = (GenericValue) serviceContext.get("userLogin");
        List<String> returnIds = (List) serviceContext.get("returnIdList");
        ...
        for (String returnId : returnIds) {
            try {
                File f = new File(basePath, "Order_" + returnId + ".xml");
                ...
                String returnType = returnHeader.getString("returnHeaderTypeId");
                if ("CUSTOMER_RETURN".equals(returnType)) {
                    billFromVendor = delegator.findOne("PartyGroup",
                            UtilMisc.toMap("partyId", returnHeader.getString("fromPartyId")), false);
                    organizationPartyId = returnHeader.getString("toPartyId");
                    voucherType = "Credit Note";
                } else if ("VENDOR_RETURN".equals(returnType)) {
                    billFromVendor = delegator.findOne("PartyGroup",
                            UtilMisc.toMap("partyId", returnHeader.getString("toPartyId")), false);
                    organizationPartyId = returnHeader.getString("fromPartyId");
                    voucherType = "Debit Note";
                }
                BigDecimal grandTotal = computeGrandTotal(returnHeader);
                ...
                dve.addMemberEntity("RA", "ReturnAdjustment");
                dve.addAliasAll("RA", null);
                dve.addAlias("RA", "overrideGlAccountId", "overrideGlAccountId", "overrideGlAccountId", false, true, null);
                dve.addAlias("RA", "amount", "amount", "amount", false, false, "sum");
                dve.addRelation("one", "Override", "GlAccount",
                        UtilMisc.toList(new ModelKeyMap("overrideGlAccountId", "glAccountId")));
                EntityListIterator distinctGlAccountIter = delegator.findListIteratorByCondition(dve,
                        EntityCondition.makeCondition("returnId", returnId), null, null,
                        UtilMisc.toList("-sourcePercentage"), null);
                List<GenericValue> distinctGlAccounts = distinctGlAccountIter.getCompleteList();
                distinctGlAccountIter.close();

                List<Map> returnItemAdjs = new ArrayList<Map>();
                Map<String, Map> returnItemMap = new HashMap();
                for (GenericValue returnAdj : distinctGlAccounts) {
                    GenericValue glAccount = returnAdj.getRelatedOne("OverrideGlAccount");
                    if (glAccount == null) continue;
                    Map results = computeVatAssessableAmount(returnAdj);
                    List<Map> taxObjectAllocations = (List<Map>) results.get("taxObjectAllocations");
                    BigDecimal vatAssessableAmount = (BigDecimal) results.get("vatAssessableAmount");
                    Map<String, Object> adjustmentMap = UtilMisc.toMap(UtilMisc.toMap("rateOfTax",
                            returnAdj.getBigDecimal("sourcePercentage"), "dutyLedger", glAccount, "amount",
                            returnAdj.getBigDecimal("amount"), "vatAssessableAmount", vatAssessableAmount));
                    adjustmentMap.put("subCategories", taxObjectAllocations);
                    returnItemAdjs.add(adjustmentMap);
                    for (Map m : taxObjectAllocations) {
                        if ("RET_SALES_TAX_ADJ".equals(returnAdj.getString("returnAdjustmentTypeId")))
                            returnItemMap.put((String) m.get("returnItemSeqId"), m);
                    }
                }

                // InventoryItemList
                List<Map> returnItems = new ArrayList<Map>();
                for (Map returnItem : returnItemMap.values()) {
                    Map inventoryMap = new HashMap(returnItem);
                    String taxClassification = (String) returnItem.get("taxClassification");
                    if ("CST @ 2% Against Form C".equalsIgnoreCase(taxClassification)) {
                        taxClassification = "Interstate Sales @ 2% Against Form C";
                    }
                    List<GenericValue> purchaseOrSalesGlAccounts = delegator.findByAnd("GlAccount", UtilMisc.toMap(
                            "parentGlAccountId", "CUSTOMER_RETURN".equals(returnType) ? "3000" : "4000",
                            "taxClassId", taxClassification));
                    if (purchaseOrSalesGlAccounts.size() > 0) {
                        GenericValue purchaseOrSalesAccountLedger = EntityUtil.getFirst(purchaseOrSalesGlAccounts);
                        inventoryMap.put("purchaseOrSalesAccountLedger", purchaseOrSalesAccountLedger);
                    }
                    returnItems.add(inventoryMap);
                }
                Collections.sort(returnItemAdjs, new Comparator<Map>() {
                    @Override
                ...
                context.put("returnItems", new SimpleList(returnItems));
                context.put("returnItemAdjs", new SimpleList(returnItemAdjs));
                context.put("returnHeader", new SimpleHash(returnHeader, ObjectWrapper.DEFAULT_WRAPPER));
                ...
                Template template = new Template("DebitCreditNote.ftl", new FileReader(
                        new File(System.getProperty("ofbiz.home") + File.separator
                                + "applications/order/config/DebitCreditNote.ftl")), cfg);
    ...
    private static Map computeVatAssessableAmount(GenericValue returnAdj) {
        Map context = new HashMap();
        Delegator delegator = returnAdj.getDelegator();
        BigDecimal vatAssessableAmount = BigDecimal.ZERO;
        List<Map> taxObjectAllocations = FastList.newInstance();
        try {
            String overrideGlAccountId = returnAdj.getString("overrideGlAccountId");
            GenericValue glAccount = returnAdj.getRelatedOne("OverrideGlAccount");
            List<GenericValue> lineItems = delegator
                    .findByAnd("ReturnAdjustment", UtilMisc.toMap("returnId", returnAdj.getString("returnId"),
                            "overrideGlAccountId", overrideGlAccountId));
            for (GenericValue each : lineItems) {
                GenericValue returnItem = each.getRelatedOne("ReturnItem");
                BigDecimal lineTotal = returnItem.getBigDecimal("returnQuantity").multiply(
                        returnItem.getBigDecimal("returnPrice"));
                vatAssessableAmount = vatAssessableAmount.add(lineTotal);
                String taxAuthGeoId = returnAdj.getString("taxAuthGeoId");
                String taxAuthPartyId = returnAdj.getString("taxAuthPartyId");
                GenericValue taxAuthority = delegator.findOne("TaxAuthority", true, "taxAuthGeoId", taxAuthGeoId, "taxAuthPartyId", taxAuthPartyId);
                String taxClassification = taxAuthority.getString("taxClassificationName");
                GenericValue productGV = returnItem.getRelatedOne("Product");
                taxObjectAllocations.add(UtilMisc.toMap("returnItemSeqId", returnItem.getString("returnItemSeqId"),
                        "productName", productGV.getString("internalName"), "assessableAmount", lineTotal, "productUom",
                        productGV.getString("quantityUomId"), "returnQty", returnItem.getBigDecimal("returnQuantity"),
                        "GlAccount", glAccount, "tax", each.getBigDecimal("amount"), "unitPrice",
                        returnItem.getBigDecimal("returnPrice"), "amount", lineTotal, "taxClassification", taxClassification));
            }
        } catch (GenericEntityException e) {
            e.printStackTrace();
        }
        context.put("vatAssessableAmount", vatAssessableAmount);
        context.put("taxObjectAllocations", taxObjectAllocations);
        return context;
    }

    private static BigDecimal computeGrandTotal(GenericValue returnHeader) {
        Delegator delegator = returnHeader.getDelegator();
        BigDecimal grandTotal = BigDecimal.ZERO;
        try {
            List<GenericValue> returnItems = delegator.findByAnd("ReturnItem",
                    UtilMisc.toMap("returnId", returnHeader.getString("returnId")));
            for (GenericValue returnItem : returnItems) {
                BigDecimal lineTotal = returnItem.getBigDecimal("returnQuantity").multiply(
                        returnItem.getBigDecimal("returnPrice"));
                grandTotal = grandTotal.add(lineTotal);
            }
            List<GenericValue> returnAdjustments = delegator.findByAnd("ReturnAdjustment",
                    UtilMisc.toMap("returnId", returnHeader.getString("returnId")));
            for (GenericValue returnItemAdj : returnAdjustments) {
                BigDecimal lineTotal = returnItemAdj.getBigDecimal("amount");
                grandTotal = grandTotal.add(lineTotal);
            }
        } catch (GenericEntityException e) {
    ...
    public static Map processRentalReturn(DispatchContext dctx, Map context) {
        Delegator delegator = dctx.getDelegator();
        String returnId = (String) context.get("returnId");
        Locale locale = (Locale) context.get("locale");
        GenericValue returnHeader;
        ...
        try {
            returnHeader = delegator.findByPrimaryKey("ReturnHeader", UtilMisc.toMap("returnId", returnId));
            if (returnHeader != null) {
                returnItems = returnHeader.getRelatedByAnd("ReturnItem", UtilMisc.toMap("returnTypeId", "RTN_RENTAL"));
            }
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            return ServiceUtil.returnError(e.getMessage());
        ...
        if (returnItems != null) {
            ...
            while (ri.hasNext()) {
                ...
                String orderId = returnItem.getString("orderId");
                try {
                    List<GenericValue> workOrderItems = delegator.findByAnd("WorkOrderItemFulfillment", UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItemSeqId));
                    GenericValue workOrderItem = EntityUtil.getFirst(workOrderItems);
                    GenericValue workEffort = workOrderItem.getRelatedOne("WorkEffort");
                    GenericValue fixedAsset = delegator.findByPrimaryKey("FixedAsset",
                    ...
                    while (--dayCount >= 0) {
                        ...
                        BigDecimal newUsedCapacity =
                                techDataCalendarExcDay.getBigDecimal("usedCapacity").subtract(returnItem.getBigDecimal("receivedQuantity"));
                    ...
                    Debug.logError(e, "Problem with updating the Return status to Complete", module);
                    return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
