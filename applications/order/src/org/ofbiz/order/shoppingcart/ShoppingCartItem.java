...
public class ShoppingCartItem implements java.io.Serializable {
    ...
    /** for reservations: end start*/
    private Timestamp reservEnd = null;
    /** for reservations: length */
    private BigDecimal reservLength = BigDecimal.ONE;
    ...
    public static ShoppingCartItem makePurchaseOrderItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity,
        ...
        newItem.setEstimatedShipDate(shipAfterDate != null ? shipAfterDate : cart.getDefaultShipAfterDate());
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice,
        ...
        return makeItem(cartLocation,productId,selectedAmount,quantity,unitPrice,
                reservStart,null,reservLength,reservPersons,null,null,shipBeforeDate,shipAfterDate,
     ...
     */
    public static ShoppingCartItem makeItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice,
            Timestamp reservStart, Timestamp reservEnd, BigDecimal reservLength, BigDecimal reservPersons,String accommodationMapId,String accommodationSpotId, Timestamp shipBeforeDate, Timestamp shipAfterDate,
            Map additionalProductFeatureAndAppls, Map attributes, String prodCatalogId, ProductConfigWrapper configWrapper,
            String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, LocalDispatcher dispatcher, ShoppingCart cart, Boolean triggerExternalOpsBool, Boolean triggerPriceRulesBool, String parentProductId, Boolean skipInventoryChecks, Boolean skipProductChecks)
            throws CartItemModifyException, ItemNotFoundException {
        Delegator delegator = cart.getDelegator();
        ...
        return makeItem(cartLocation, product, selectedAmount, quantity, unitPrice,
                reservStart, reservEnd,reservLength, reservPersons, accommodationMapId, accommodationSpotId, shipBeforeDate, shipAfterDate,
                additionalProductFeatureAndAppls, attributes, prodCatalogId, configWrapper,
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount,
        ...
        return makeItem(cartLocation,product,selectedAmount,
               quantity,unitPrice,reservStart,null,reservLength,reservPersons,
    ...
    */
    public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount,
            BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart,Timestamp reservEnd,BigDecimal reservLength, BigDecimal reservPersons,
        ...
        if ("ASSET_USAGE".equals(product.getString("productTypeId")) || "ASSET_USAGE_OUT_IN".equals(product.getString("productTypeId"))) {
            ...
            newItem.setReservEnd(reservEnd);
        ...
        if (_product != null && isModifiedPrice == false) {
            try {
                ...
                if (cart.getOrderType().equals("PURCHASE_ORDER")) {
                    ...
                    this.setDefaultProductPrice(((BigDecimal) priceResult.get("price")));
                    ...
                    if (priceResult.get("basePrice") != null) {
                        this.setDefaultProductPrice(((BigDecimal) priceResult.get("basePrice")));
                    ...
                    if (configWrapper != null) {
                        ...
                        if (UtilValidate.isNotEmpty(productStore) && "Y".equals(productStore.get("showPricesWithVatTax"))) {
                            ...
                            // Get Taxes
                            String taxAuthorityType=null;
                            if(cart.getOrderType().equals("PURCHASE_ORDER")) {
                            	taxAuthorityType="PURCHASE";
                            }else if(cart.getOrderType().equals("SALES_ORDER")) {
                            	taxAuthorityType="SALES";
                            }
                            Map totalPriceWithTaxMap = null;
                            if(taxAuthorityType!=null) {
                            	totalPriceWithTaxMap = dispatcher.runSync("calcTaxForDisplay", UtilMisc.toMap("taxAuthorityType",taxAuthorityType,"basePrice", totalPrice, "productId", this.productId, "productStoreId", cart.getProductStoreId()));
                            }else {
                            	totalPriceWithTaxMap = dispatcher.runSync("calcTaxForDisplay", UtilMisc.toMap("basePrice", totalPrice, "productId", this.productId, "productStoreId", cart.getProductStoreId()));
                            }
    ...
    public Timestamp getReservEnd() {
        return this.reservEnd;
    }
    public void setReservEnd(Timestamp reservEnd) {
    ...
    public Timestamp getDesiredDeliveryDate() {
        ...
        if (ddDate != null) {
            try {
            	if (ddDate.length() == 10) ddDate += " 00:00:00.000";
            	Date date = UtilDateTime.SIMPLE_DATE_TIME_FORMAT.parse(ddDate);
            	return new Timestamp(date.getTime());
                //return Timestamp.valueOf(ddDate + " 00:00:00.000");
            } catch (Exception e) {
...
//        Debug.logInfo("Price" + getBasePrice() + " quantity" +  quantity + " Rental adj:" + getRentalAdjustment() + " other adj:" + getOtherAdjustments(), module);
          ...
          //.multiply(getRentalAdjustment()).add(getOtherAdjustments())
    }

    public BigDecimal getItemSubTotal() {
        if(this.getReservLength().compareTo(BigDecimal.ZERO)==1)
            return this.getItemSubTotal(this.getQuantity().multiply(this.getReservLength()));
	...
	public BigDecimal getDefaultProductPrice() {
