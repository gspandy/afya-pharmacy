...
public class ShoppingCartServices {
    ...
    public static Map<String, Object>loadCartFromOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        cart.setFormToIssue(orderHeader.getString("formToIssue"));
        cart.setFormToReceive(orderHeader.getString("formToReceive"));
        List<GenericValue> orderContacts=FastList.newInstance();
		try {
			orderContacts = delegator.findByAnd("OrderContactMech", "orderId", cart.getOrderId(),
    ...
    public static Map<String, Object> loadCartFromQuote(DispatchContext dctx, Map<String, Object> context) {
        ...
        if ("PURCHASE_QUOTE".equals(quote.getString("quoteTypeId"))) {
			...
			try {
				shippingContactMechId = getPartyContactMechId(quote.getString("partyId"), delegator);
        ...
        if(!"PURCHASE_ORDER".equals(cart.getOrderType())){
        	String shippingContactMechId = null;
			try {
				shippingContactMechId = getPartyContactMechId("Company", delegator);
				cart.setShippingOriginContactMechId(-1, shippingContactMechId);
			} catch (GenericEntityException e) {
				e.printStackTrace();
			}
