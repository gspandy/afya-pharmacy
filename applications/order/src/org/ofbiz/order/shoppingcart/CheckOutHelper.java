...
public class CheckOutHelper {
	...
	if (UtilValidate.isNotEmpty(shippingContactMechId)) {
		...
		if (cart.shippingApplies()) {
			...
			errMsg = UtilProperties.getMessage(resource_error, "checkhelper.select_shipping_destination",
					(cart != null ? cart.getLocale() : Locale.getDefault()));
			errorMessages.add(errMsg);
		}
	...
	public Map setCheckOutShippingOptions(String shippingMethod, String shippingInstructions,
			String orderAdditionalEmails, String maySplit, String giftMessage, String isGift, String internalCode,
	...
	private List setCheckOutShippingOptionsInternal(String shippingMethod, String shippingInstructions,
			String orderAdditionalEmails, String maySplit, String giftMessage, String isGift, String internalCode,
			String shipBeforeDate, String shipAfterDate) {
	List errorMessages = new ArrayList();
	String errMsg = null;
	...
	if (UtilValidate.isNotEmpty(shippingMethod)) {
		...
		if (cart.shippingApplies()) {
			// only return an error if shipping is required for this purchase
			errMsg = UtilProperties.getMessage(resource_error, "checkhelper.select_shipping_method",
					(cart != null ? cart.getLocale() : Locale.getDefault()));
			errorMessages.add(errMsg);
		}
	...
	if (UtilValidate.isNotEmpty(maySplit)) {
		...
		/*errMsg = UtilProperties.getMessage(resource_error, "checkhelper.select_splitting_preference",
				(cart != null ? cart.getLocale() : Locale.getDefault()));
		errorMessages.add(errMsg);*/
	}
	...
	if (UtilValidate.isNotEmpty(isGift)) {
		...
		errMsg = UtilProperties.getMessage(resource_error, "checkhelper.specify_if_order_is_gift",
				(cart != null ? cart.getLocale() : Locale.getDefault()));
		errorMessages.add(errMsg);
	}
	...
	if (UtilValidate.isNotEmpty(shipBeforeDate)) {
		if (UtilValidate.isDate(shipBeforeDate)) {
			...
			errMsg = UtilProperties.getMessage(resource_error, "checkhelper.specify_if_shipBeforeDate_is_date",
					(cart != null ? cart.getLocale() : Locale.getDefault()));
			errorMessages.add(errMsg);
		}
	}
	...
	if (UtilValidate.isNotEmpty(shipAfterDate)) {
		if (UtilValidate.isDate(shipAfterDate)) {
			...
			errMsg = UtilProperties.getMessage(resource_error, "checkhelper.specify_if_shipAfterDate_is_date",
					(cart != null ? cart.getLocale() : Locale.getDefault()));
			errorMessages.add(errMsg);
		}
	}
	...
	if (UtilValidate.isNotEmpty(selectedPaymentMethods)) {
		...
		if (UtilValidate.isNotEmpty(billingAccountId)) {
			...
			try {
				List billingAccountTerms = delegator.findByAnd("BillingAccountTerm",
				...
				if (UtilValidate.isNotEmpty(billingAccountTerms)) {
					...
					while (billingAccountTermsIt.hasNext()) {
						...
						if (!cart.hasOrderTerm(billingAccountTerm.getString("termTypeId"))) {
							cart.addOrderTerm(billingAccountTerm.getString("termTypeId"),
									billingAccountTerm.getBigDecimal("termValue"),
		...
		if (selectedPaymentMethods.containsKey("EXT_BILLACT") && selectedPaymentMethods.size() == 1) {
			...
			if (amountToUse.compareTo(BigDecimal.ZERO) > 0 && amountToUse.compareTo(accountCredit) > 0) {
				errMsg = UtilProperties.getMessage(resource_error,
						"checkhelper.insufficient_credit_available_on_account", (cart != null ? cart.getLocale()
								: Locale.getDefault()));
				errorMessages.add(errMsg);
			} else {
				// otherwise use the available account credit (The user might enter 10.00 for an order worth 20.00 from
				// an account with 30.00. This makes sure that the 30.00 is used)
			...
			if (grandTotal.compareTo(amountToUse) > 0) {
				...
				// since this is the only selected payment method, let's make this amount the grand total for
				// convenience
				amountToUse = grandTotal;
			}

			// associate the cart billing account amount and EXT_BILLACT selected payment method with whatever amount we
			// have now
		...
		if (cart.getGrandTotal().compareTo(BigDecimal.ZERO) != 0) {
	...
	public Map setCheckOutOptions(String shippingMethod, String shippingContactMechId, Map selectedPaymentMethods,
			List singleUsePayments, String billingAccountId, String shippingInstructions, String orderAdditionalEmails,
			String maySplit, String giftMessage, String isGift, String internalCode, String shipBeforeDate,
	...
	if (params.get("addGiftCard") != null) {
		...
		if (UtilValidate.isEmpty(gcNum)) {
			errMsg = UtilProperties.getMessage(resource_error, "checkhelper.enter_gift_card_number",
					(cart != null ? cart.getLocale() : Locale.getDefault()));
			errorMessages.add(errMsg);
			gcFieldsOkay = false;
		}
		if (cart.isPinRequiredForGC(delegator)) {
			...
			if (UtilValidate.isEmpty(gcPin)) {
				errMsg = UtilProperties.getMessage(resource_error, "checkhelper.enter_gift_card_pin_number",
						(cart != null ? cart.getLocale() : Locale.getDefault()));
				errorMessages.add(errMsg);
				gcFieldsOkay = false;
			}
		...
		if (cart.isValidateGCFinAccount(delegator)) {
			try {
				...
				if (!cart.isPinRequiredForGC(delegator)) {
					...
					if (finAccount == null) {
						errMsg = UtilProperties.getMessage(resource_error, "checkhelper.gift_card_does_not_exist",
								(cart != null ? cart.getLocale() : Locale.getDefault()));
						errorMessages.add(errMsg);
						gcFieldsOkay = false;
					} else
						if ((finAccount.getBigDecimal("availableBalance") == null)
								|| !((finAccount.getBigDecimal("availableBalance")).compareTo(FinAccountHelper.ZERO) > 0)) {
							// if account's available balance (including authorizations) is not greater than zero, then
							// return an error
							errMsg = UtilProperties.getMessage(resource_error, "checkhelper.gift_card_has_no_value",
									(cart != null ? cart.getLocale() : Locale.getDefault()));
							errorMessages.add(errMsg);
							gcFieldsOkay = false;
						}
		...
		if (UtilValidate.isNotEmpty(selectedPaymentMethods)) {
			if (UtilValidate.isEmpty(gcAmt)) {
				errMsg = UtilProperties.getMessage(resource_error, "checkhelper.enter_amount_to_place_on_gift_card",
						(cart != null ? cart.getLocale() : Locale.getDefault()));
				errorMessages.add(errMsg);
				gcFieldsOkay = false;
			}
		...
		if (UtilValidate.isNotEmpty(gcAmt)) {
			try {
				...
				Debug.logError(e, module);
				errMsg = UtilProperties.getMessage(resource_error, "checkhelper.invalid_amount_for_gift_card",
						(cart != null ? cart.getLocale() : Locale.getDefault()));
				errorMessages.add(errMsg);
				gcFieldsOkay = false;
			}
		...
		if (gcFieldsOkay) {
			...
			if (gcResult != null) {
				...
				errMsg = UtilProperties.getMessage(resource_error, "checkhelper.problem_with_gift_card_information",
						(cart != null ? cart.getLocale() : Locale.getDefault()));
				errorMessages.add(errMsg);
			}
		}
	...
	context.put("formToIssue", this.cart.getFormToIssue());
	context.put("formToIssueFormNo", this.cart.getFormToIssueFormNo());
	context.put("formToIssueSeriesNo", this.cart.getFormToIssueSeriesNo());
	context.put("billingLedger", this.cart.getGlAccountId());
	if (!StringUtils.isEmpty(this.cart.getFormToIssueDate())) {
		try {
			context.put("formToIssueDate", new java.sql.Date(dateFormatter.parse(this.cart.getFormToIssueDate())
					.getTime()));
		} catch (ParseException e1) {
			e1.printStackTrace();
		}
	}
	context.put("formToReceive", this.cart.getFormToReceive());
	context.put("formToReceiveSeriesNo", this.cart.getFormToReceiveSeriesNo());
	if (!StringUtils.isEmpty(this.cart.getFormToReceiveDate())) {
		try {
			context.put("formToReceiveDate", new java.sql.Date(dateFormatter.parse(this.cart.getFormToReceiveDate())
					.getTime()));
		} catch (ParseException e1) {
			e1.printStackTrace();
		}
	}
	context.put("formToReceiveFormNo", this.cart.getFormToReceiveFormNo());
	context.put("voucherType", this.cart.getAttribute("voucherType"));
	context.put("orderDate", this.cart.getOrderDate());
	...
	GenericValue gv = delegator.makeValue(
			"OrderContactMech",
			UtilMisc.toMap("contactMechPurposeTypeId", "SHIP_ORIG_LOCATION", "contactMechId",
					cart.getShippingOriginContactMechId()));
	List<GenericValue> contactMechList = new ArrayList<GenericValue>();
	contactMechList.add(gv);
	context.put("orderContactMechs", contactMechList);
	try {
		...
		String errMsg = UtilProperties.getMessage(resource_error,
				"checkhelper.could_not_create_order_invoking_service", messageMap, (cart != null ? cart.getLocale()
						: Locale.getDefault()));
	...
	if (ServiceUtil.isError(storeResult)) {
		String errMsg = UtilProperties.getMessage(resource_error,
				"checkhelper.did_not_complete_order_following_occurred",
				(cart != null ? cart.getLocale() : Locale.getDefault()));
		...
		if (productId != null) {
			try {
				...
				if ("AGGREGATED_CONF".equals(product.getString("productTypeId"))) {
					org.ofbiz.product.config.ProductConfigWrapper config = this.cart.findCartItem(counter)
							.getConfigWrapper();
	...
	while (shoppingCartItems.hasNext()) {
		...
		if (requirementId != null) {
			try {
				List<GenericValue> requirements = delegator.findList("Requirement",
						EntityCondition.makeCondition("requirementId", requirementId), null, null, null, false);
				for (GenericValue qty : requirements) {
					BigDecimal orderedQty = shoppingCartItem.getQuantity();
					BigDecimal requirementQty = qty.getBigDecimal("quantity");
					BigDecimal remainingQuantity = orderedQty != null ? requirementQty.subtract(orderedQty)
							: requirementQty;
					int remqty = remainingQuantity.intValue();
					if (remqty == 0) {
					...
					if (remqty > 0) {
						Map inputMap = UtilMisc.toMap("requirementId", requirementId, "statusId", "REQ_APPROVED",
								"quantity", remainingQuantity);
						inputMap.put("userLogin", userLogin);
						// TODO: check service result for an error return
						Map outMap = dispatcher.runSync("updateRequirement", inputMap);
					}
					GenericValue orderReqCommitment = delegator.makeValue("OrderRequirementCommitment", UtilMisc.toMap("orderId",orderId,"requirementId",requirementId,
							"orderItemSeqId",shoppingCartItem.getOrderItemSeqId(),"quantity",requirementQty ));
	...
	try {
		...
		Debug.logWarning(e,
				UtilProperties.getMessage(resource_error, "OrderProblemsGettingPartyRecord", cart.getLocale()), module);
		party = null;
	}
	...
	if (party != null) {
		...
		while (emailIter != null && emailIter.hasNext()) {
			...
			GenericValue orderContactMech = this.delegator.makeValue("OrderContactMech", UtilMisc.toMap("orderId",
					orderId, "contactMechId", email.getString("contactMechId"), "contactMechPurposeTypeId",
					"ORDER_EMAIL"));
	...
	while (eli.hasNext()) {
		...
		GenericValue contactMech = this.delegator.makeValue("ContactMech", UtilMisc.toMap("contactMechId",
				contactMechId, "contactMechTypeId", "EMAIL_ADDRESS", "infoString", email));

		GenericValue orderContactMech = this.delegator.makeValue("OrderContactMech", UtilMisc.toMap("orderId", orderId,
	...
	if (toBeStored.size() > 0) {
		try {
			...
			Debug.logWarning(
					e,
					UtilProperties.getMessage(resource_error, "OrderProblemsStoringOrderEmailContactInformation",
	...
	for (int i = 0; i < shipGroups; i++) {
		...
		List orderAdj = FastList.newInstance();
		...
		if ("PURCHASE_ORDER".equals(this.cart.getOrderType())) {
			...
			if("PRICEINCLUDETAX".equals(this.cart.getFormToIssue())) {
				serviceContext.put("taxForm", "PRICEINCLUDETAX");
			}
		} else
			if ("SALES_ORDER".equals(this.cart.getOrderType())) {
		...
		orderAdj = (List) taxReturn.get(0);
		itemAdj = (List) taxReturn.get(1);
	...
	private Map makeTaxContext(int shipGroup, GenericValue shipAddress, Map shoppingCartItemIndexMap)
	...
	String shippingOriginContactId = csi.getShippingOriginContactMechId();
	if (UtilValidate.isEmpty(shippingOriginContactId)) {
		shippingOriginContactId = cart.getShippingOriginContactMechId();
	}
	if (shippingOriginContactId == null && UtilValidate.isNotEmpty(cart.getOrderId())) {
		List<GenericValue> orderContacts = delegator.findByAnd("OrderContactMech", "orderId", cart.getOrderId(),
				"contactMechPurposeTypeId", "SHIP_ORIG_LOCATION");
		shippingOriginContactId = EntityUtil.getFirst(orderContacts).getString("contactMechId");
	}
	GenericValue shippingOriginAddress = (GenericValue) delegator.findOne("PostalAddress",
			UtilMisc.toMap("contactMechId", shippingOriginContactId), true);
	for (int i = 0; i < totalItems; i++) {
		ShoppingCartItem cartItem = it.next();
		BigDecimal itemSubTotal = cartItem.getBasePrice().multiply(cartItem.getQuantity());
		if("RENTAL_ORDER_ITEM".equals(cartItem.getItemType())){
			itemSubTotal=itemSubTotal.multiply(cartItem.getReservLength());
		}
		Map serviceContext = UtilMisc.toMap("payToPartyId", cart.getBillFromVendorPartyId());
		serviceContext.put("billToPartyId", cart.getBillToCustomerPartyId());
		serviceContext.put("product", cartItem.getProduct());

		for (Iterator<GenericValue> iter = cartItem.getAdjustments().iterator(); iter.hasNext();) {
			String adjType = iter.next().getString("orderAdjustmentTypeId");
			if ("EXCISE_TAX".equals(adjType) || "SALES_TAX".equals(adjType)|| "EDUCESS_ADJUSTMENT".equals(adjType)|| "HIGH_EDU_CESS".equals(adjType)) {
				iter.remove();
				iter = cartItem.getAdjustments().iterator();
			}
		}

		Map<String, BigDecimal> assessableAmountMap = OrderReadHelper.getAssessableAdjustmentMap(cartItem);
		boolean applyApportion = assessableAmountMap.size() > 0;
		if (applyApportion) {
			for (String adjType : assessableAmountMap.keySet()) {
				GenericValue orderAdj = delegator.findOne("OrderAdjustmentType",
						UtilMisc.toMap("orderAdjustmentTypeId", adjType), false);
				String methodOfApportion = orderAdj.getString("methodOfApportion");
				BigDecimal adjAmount = BigDecimal.ONE.multiply(assessableAmountMap.get(adjType));
				adjAmount = adjAmount.setScale(taxDecimals, taxRounding);
				itemSubTotal = adjAmount.add(itemSubTotal);
			}
		}
		serviceContext.put("itemAmount", itemSubTotal);
		serviceContext.put("itemPrice", itemSubTotal);

		Map result = dispatcher.runSync("calcExciseAndCSTTax", serviceContext);
		List itemAdj = (List) result.get("orderAdjustments");
		// set the item adjustments
		if (itemAdj != null) {
			for (int x = 0; x < itemAdj.size(); x++) {
				GenericValue adj = (GenericValue) itemAdj.get(x);
				itemSubTotal = itemSubTotal.add(adj.getBigDecimal("amount"));
				cartItem.addAdjustment(adj);
				if (Debug.verboseOn())
					Debug.logVerbose(
							"Added item adjustments to ship group [" + i + " / " + x + "] - " + itemAdj.get(x), module);
			}
		}
		product.add(i, cartItem.getProduct());
		amount.add(i, itemSubTotal);
	...
	if (shipAddress == null) {
		...
		// Debug.logInfo("====== makeTaxContext set shipAddress to cart.getShippingAddress(shipGroup): " + shipAddress,
		// module);
	}
	...
	if (shipAddress == null) {
		for (int i = 0; i < cart.selectedPayments(); i++) {
			...
			if (billAddr != null) {
				...
				Debug.logInfo(
						"In makeTaxContext no shipping address, but found address with ID ["
								+ shipAddress.get("contactMechId") + "] from payment method.", module);
				break;
			}
		}
	}

	Map serviceContext = UtilMisc.toMap("productStoreId",  !"PURCHASE_ORDER".equalsIgnoreCase(cart.getOrderType())? cart.getProductStoreId():null);
	...
	serviceContext.put("shippingOriginAddress", shippingOriginAddress);
	if ("PURCHASE_ORDER".equals(cart.getOrderType())) serviceContext.put("taxForm", cart.getFormToIssue());
	if ("SALES_ORDER".equals(cart.getOrderType())) serviceContext.put("taxForm", cart.getFormToIssue());
	return serviceContext;
	}

	// Calc the tax adjustments.
	private List getTaxAdjustments(LocalDispatcher dispatcher, String taxService, Map serviceContext)
			throws GeneralException {
	...
	public Map<String, Object> processPayment(GenericValue productStore, GenericValue userLogin)
			throws GeneralException {
	return CheckOutHelper.processPayment(this.cart.getOrderId(), this.cart.getGrandTotal(), this.cart.getCurrency(),
			productStore, userLogin, false, false, dispatcher, delegator);
	}

	public Map<String, Object> processPayment(GenericValue productStore, GenericValue userLogin, boolean faceToFace)
			throws GeneralException {
	return CheckOutHelper.processPayment(this.cart.getOrderId(), this.cart.getGrandTotal(), this.cart.getCurrency(),
			productStore, userLogin, faceToFace, false, dispatcher, delegator);
	}

	public Map<String, Object> processPayment(GenericValue productStore, GenericValue userLogin, boolean faceToFace,
			boolean manualHold) throws GeneralException {
	return CheckOutHelper.processPayment(this.cart.getOrderId(), this.cart.getGrandTotal(), this.cart.getCurrency(),
			productStore, userLogin, faceToFace, manualHold, dispatcher, delegator);
	}

	public static Map<String, Object> processPayment(String orderId, BigDecimal orderTotal, String currencyUomId,
			GenericValue productStore, GenericValue userLogin, boolean faceToFace, boolean manualHold,
	...
	List canExpr = UtilMisc.toList(EntityCondition.makeCondition("statusId", EntityOperator.NOT_EQUAL,
			"PAYMENT_CANCELLED"));
	...
	if (UtilValidate.isNotEmpty(manualRefPaymentPrefs)) {
		...
		while (i.hasNext()) {
			...
			// approve the order
			//comment by harish as discussed with nafis  16/7/2012
	...
	if (UtilValidate.isNotEmpty(payPalPaymentPrefs)) {
		...
		ExpressCheckoutEvents.doExpressCheckout(productStore.getString("productStoreId"), orderId, payPalPaymentPref,
	...
	if (UtilValidate.isNotEmpty(onlinePaymentPrefs)) {
				...
				if (authResp.equals("APPROVED")) {
					...
					if (authResp.equals("ERROR")) {
						...
						return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,
								"OrderPleaseContactCustomerServicePaymentReturnCodeUnknown", Locale.getDefault()));
					}
		} else {
		...
		if (UtilValidate.isNotEmpty(cashCodPcBaPaymentPreferences) && UtilValidate.isNotEmpty(allPaymentPreferences)
			...
			List checkPareferences = EntityUtil.filterByAnd(cashCodPcBaPaymentPreferences,
	...
	public static void adjustFaceToFacePayment(String orderId, BigDecimal cartTotal, List allPaymentPrefs,
			GenericValue userLogin, Delegator delegator) throws GeneralException {
	...
	List exprs = UtilMisc.toList(EntityCondition.makeCondition(
			EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("blacklistString"), EntityOperator.EQUALS,
					EntityFunction.UPPER(shippingAddress)), EntityOperator.AND, EntityCondition.makeCondition(
					"orderBlacklistTypeId", EntityOperator.EQUALS, "BLACKLIST_ADDRESS")));
	String errMsg = null;
	...
	while (i.hasNext()) {
		...
		if ((paymentMethod != null) && ("CREDIT_CARD".equals(paymentMethod.getString("paymentMethodTypeId")))) {
			...
			try {
				...
				errMsg = UtilProperties.getMessage(resource_error, "checkhelper.problems_reading_database",
						(cart != null ? cart.getLocale() : Locale.getDefault()));
				return ServiceUtil.returnError(errMsg);
			}
			if (creditCard != null) {
				String creditCardNumber = UtilFormatOut.checkNull(creditCard.getString("cardNumber"));
				exprs.add(EntityCondition.makeCondition(EntityCondition.makeCondition("blacklistString",
						EntityOperator.EQUALS, creditCardNumber), EntityOperator.AND, EntityCondition.makeCondition(
						"orderBlacklistTypeId", EntityOperator.EQUALS, "BLACKLIST_CREDITCARD")));
			}
			if (billingAddress != null) {
				...
				exprs.add(EntityCondition.makeCondition(EntityCondition.makeCondition(
						EntityFunction.UPPER_FIELD("blacklistString"), EntityOperator.EQUALS,
						EntityFunction.UPPER(address)), EntityOperator.AND, EntityCondition.makeCondition(
						"orderBlacklistTypeId", EntityOperator.EQUALS, "BLACKLIST_ADDRESS")));
			}
		}
	}
	...
	if (UtilValidate.isNotEmpty(blacklistFound)) {
		return ServiceUtil.returnFailure(UtilProperties.getMessage(resource_error, "OrderFailed",
				(cart != null ? cart.getLocale() : Locale.getDefault())));
	} else {
		return ServiceUtil.returnSuccess("success");
	}
	...
	try {
		...
		Debug.logError(e, module);
		errMsg = UtilProperties.getMessage(resource_error, "checkhelper.database_error",
				(cart != null ? cart.getLocale() : Locale.getDefault()));
	...
	try {
		...
		errMsg = UtilProperties.getMessage(resource_error, "checkhelper.problems_getting_order_header",
				(cart != null ? cart.getLocale() : Locale.getDefault()));
		result = ServiceUtil.returnError(errMsg);
		return result;
	}
	if (orderHeader != null) {
		...
		try {
			...
			errMsg = UtilProperties.getMessage(resource_error, "checkhelper.problems_getting_payment_preference",
					(cart != null ? cart.getLocale() : Locale.getDefault()));
			result = ServiceUtil.returnError(errMsg);
			return result;
		}
		if (UtilValidate.isNotEmpty(paymentPrefs)) {
			if (paymentPrefs.size() > 1) {
				Debug.logError(
						"Too many payment preferences, you cannot have more then one when using external gateways",
						module);
			}
			...
			if (paymentMethodTypeId.startsWith("EXT_")) {
				...
				if (!(paymentMethodTypeId.equals("EXT_PAYPAL") && UtilValidate.isNotEmpty(paymentPreference
	 ...
	 * any error messages
	 */
	public Map<String, Object> finalizeOrderEntryShip(int shipGroupIndex, String shippingContactMechId,
			String shippingOriginContactMechId, String supplierPartyId) {
	Map<String, Object> result;
	String errMsg = null;
	// Verify the field is valid
	if (UtilValidate.isNotEmpty(shippingContactMechId)) {
		this.cart.setShippingContactMechId(shipGroupIndex, shippingContactMechId);
		this.cart.setShippingOriginContactMechId(shipGroupIndex, shippingOriginContactMechId);
		if (UtilValidate.isNotEmpty(supplierPartyId)) {
			this.cart.setSupplierPartyId(shipGroupIndex, supplierPartyId);
		}
		result = ServiceUtil.returnSuccess();
	} else {
		errMsg = UtilProperties.getMessage(resource_error, "checkhelper.enter_shipping_address",
				(cart != null ? cart.getLocale() : Locale.getDefault()));
		result = ServiceUtil.returnError(errMsg);
	}
	 ...
	 * any error messages
	 */
	public Map<String, Object> finalizeOrderEntryOptions(int shipGroupIndex, String shippingMethod,
			String shippingInstructions, String maySplit, String giftMessage, String isGift, String internalCode,
			String shipBeforeDate, String shipAfterDate, String orderAdditionalEmails) {
	this.cart.setOrderAdditionalEmails(orderAdditionalEmails);
	return finalizeOrderEntryOptions(shipGroupIndex, shippingMethod, shippingInstructions, maySplit, giftMessage,
			isGift, internalCode, shipBeforeDate, shipAfterDate, null, null);
	}

	public Map<String, Object> finalizeOrderEntryOptions(int shipGroupIndex, String shippingMethod,
			String shippingInstructions, String maySplit, String giftMessage, String isGift, String internalCode,
			String shipBeforeDate, String shipAfterDate, String internalOrderNotes, String shippingNotes,
			BigDecimal shipEstimate) {
	this.cart.setItemShipGroupEstimate(shipEstimate, shipGroupIndex);
	return finalizeOrderEntryOptions(shipGroupIndex, shippingMethod, shippingInstructions, maySplit, giftMessage,
			isGift, internalCode, shipBeforeDate, shipAfterDate, internalOrderNotes, shippingNotes);
	}
	...
	this.cart.setInternalCode(internalCode); // FIXME: the internalCode is not a ship group field and should be moved
	...
	if ((shipBeforeDate != null) && (shipBeforeDate.length() > 8)) {
		...
		try {
			this.cart.setShipBeforeDate(shipGroupIndex,
					(Timestamp) ObjectType.simpleTypeConvert(shipBeforeDate, "Timestamp", null, null));
		} catch (Exception e) {
	...
	if ((shipAfterDate != null) && (shipAfterDate.length() > 8)) {
		...
		try {
			this.cart.setShipAfterDate(shipGroupIndex,
					(Timestamp) ObjectType.simpleTypeConvert(shipAfterDate, "Timestamp", null, null));
		} catch (Exception e) {
	...
	if (billingAccountAmt.compareTo(availableAmount) > 0) {
		...
		errMsg = UtilProperties.getMessage(resource_error, "checkevents.not_enough_available_on_account", messageMap,
				(cart != null ? cart.getLocale() : Locale.getDefault()));
		return ServiceUtil.returnError(errMsg);
	}
	...
	if (paymentTypes.contains("EXT_BILLACT") && paymentTypes.size() == 1 && paymentMethods.size() == 0) {
		if (cart.getGrandTotal().compareTo(availableAmount) > 0) {
			errMsg = UtilProperties.getMessage(resource_error, "checkevents.insufficient_credit_available_on_account",
					(cart != null ? cart.getLocale() : Locale.getDefault()));
			return ServiceUtil.returnError(errMsg);
		}
	}
	...
	if (UtilValidate.isNotEmpty(paymentMethods) && requiredAmount.compareTo(selectedPaymentTotal) < 0) {
		...
		if (!paymentTypes.contains("CASH")) {
			...
			errMsg = UtilProperties.getMessage(resource_error,
					"checkhelper.change_returned_cannot_be_greater_than_cash", (cart != null ? cart.getLocale()
							: Locale.getDefault()));
			return ServiceUtil.returnError(errMsg);
		} else {
	...
	String paymentConfig = ProductStoreWorker.getProductStorePaymentProperties(delegator, cart.getProductStoreId(),
			"GIFT_CARD", null, true);
