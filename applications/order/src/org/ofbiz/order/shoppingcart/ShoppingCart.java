...
public class ShoppingCart implements Iterable<ShoppingCartItem>, Serializable {
    ...
    private final List<GenericValue> adjustments = FastList.newInstance();
    private final List orderTerms = new LinkedList();
    private final List<ShoppingCartItem> cartLines = FastList.newInstance();
    ...
    private final List internalOrderNotes = FastList.newInstance(); // internal
                                                                    // notes
    private final List orderNotes = FastList.newInstance(); // public notes
    /**
     ...
     * productPromoId
     */
    private final List<ProductPromoUseInfo> productPromoUseInfoList = FastList.newInstance();
    private final Timestamp cartCreatedTs = UtilDateTime.nowTimestamp();
    ...
    /**
     * General partyId for the Order, all other IDs default to this one if not
     ...
     * when the promos are cleared, it is a preference that will be used in the
    ...
    // Indian Tax Related Fields
    private String glAccountId;
    private String[] additionalTaxAuthorities = null;

    private String formToIssue;
    private String formToIssueSeriesNo;
    private String formToIssueFormNo;
    private String formToIssueDate;

    private String formToReceive;
    private String formToReceiveFormNo;
    private String formToReceiveSeriesNo;
    private String formToReceiveDate;
    ...
    public ShoppingCart(ShoppingCart cart) {
        ...
        this.ignoreTaxes = cart.getIgnoreTaxes();
        ...
        while (groupIt.hasNext()) {
            ...
            // get the new parent group by number from the existing set; as
            // before the parent must come before all children
            // to work...
    ...
    public ShoppingCart(Delegator delegator, String productStoreId, String webSiteId, Locale locale, String currencyUom,
        ...
        this.currencyUom = (currencyUom != null) ? currencyUom : UtilProperties.getPropertyValue("general.properties",
        ...
        if (productStoreId != null) {
            ...
            if (billFromVendorPartyId == null) {
                // since default cart is of type SALES_ORDER, set to store's
                // payToPartyId
                this.billFromVendorPartyId = productStore.getString("payToPartyId");
            }

            holdOrder = "N".equalsIgnoreCase(productStore.getString("autoApproveOrder"));
        }/*else{
            //[Pradyumna: While trying to create a PO from Requirement, billToCustomerPartyId is not populated.]
            // Meaning its a Purchase Order
            if(billToCustomerPartyId==null){
    ...
    public static BigDecimal getMinimumOrderQuantity(Delegator delegator, BigDecimal itemBasePrice, String itemProductId)
        ...
        List<GenericValue> minimumOrderPriceList = delegator.findList("ProductPrice",
        ...
        if (itemBasePrice == null) {
            List<GenericValue> productPriceList = EntityUtil.filterByDate(delegator.findList("ProductPrice",
     ...
     * changes a lot of stuff including: - some items in the cart may not be
     * valid in any catalog in the new store - promotions need to be
     * recalculated for the products that remain - what else? lots of settings
     * on the ProductStore...
     *
     * So for now this can only be called if the cart is empty... otherwise it
     * wil throw an exception
     *
     */
    public void setProductStoreId(String productStoreId) {
        if ((productStoreId == null && this.productStoreId == null)
                || (productStoreId != null && productStoreId.equals(this.productStoreId))) {
            return;
        }
        ...
        if (this.size() == 0) {
            ...
            throw new IllegalArgumentException("Cannot set productStoreId when the cart is not empty; cart size is "
                    + this.size());
        }
    }
    ...
    public String getCurrency() {
        if (this.currencyUom != null) {
            ...
            // uh oh, not good, should always be passed in on init, we can't
            // really do anything without it, so throw an
            // exception
    ...
    public GenericValue getSupplierProduct(String productId, BigDecimal quantity, LocalDispatcher dispatcher) {
        ...
        Map params = UtilMisc.toMap("productId", productId, "partyId", this.getPartyId(), "currencyUomId", this.getCurrency(),
                "quantity", quantity);
        try {
            ...
            if ((productSuppliers != null) && (productSuppliers.size() > 0)) {
                ...
                supplierProduct = delegator.makeValidValue("SupplierProduct",UtilMisc.toMap("supplierProductId",productId,
                        "supplierProductName","","lastPrice",BigDecimal.ONE,"currencyUomId",this.getCurrency(),"quantity", quantity));
            }
            // } catch (GenericServiceException e) {
        } catch (Exception e) {
            Debug.logWarning(
                    UtilProperties.getMessage(resource_error, "OrderRunServiceGetSuppliersForProductError", locale)
                            + e.getMessage(), module);
        }
    ...
    /**
     * Add an item to the shopping cart, or if already there, increase the
     * quantity.
     *
     * @return the new/increased item index
     * @throws CartItemModifyException
     */
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart,
            BigDecimal reservLength, BigDecimal reservPersons, Timestamp shipBeforeDate, Timestamp shipAfterDate, Map features,
            Map attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, String itemGroupNumber,
            String parentProductId, LocalDispatcher dispatcher) throws CartItemModifyException, ItemNotFoundException {

        return addOrIncreaseItem(productId, selectedAmount, quantity, reservStart,null, reservLength, reservPersons, null, null,
                shipBeforeDate, shipAfterDate, features, attributes, prodCatalogId, configWrapper, itemType, itemGroupNumber,
                parentProductId, dispatcher);
    }

    /** add rental (with accommodation) item to cart */
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart,Timestamp reservEnd,
        ...
        for (int i = 0; i < this.cartLines.size(); i++) {
            ...
            if (sci.equals(productId, reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId,
                    features, attributes, prodCatalogId, selectedAmount, configWrapper, itemType, itemGroup, false)) {
                ...
                if (sci.getItemType().equals("RENTAL_ORDER_ITEM")) {
                    // check to see if the related fixed asset is available for
                    // the new quantity
                    String isAvailable = ShoppingCartItem.checkAvailability(productId, newQuantity, reservStart, reservLength,
                            this);
                ...
                if (Debug.verboseOn())
                    Debug.logVerbose("Found a match for id " + productId + " on line " + i + ", updating quantity to "
                            + newQuantity, module);
                ...
                if (getOrderType().equals("PURCHASE_ORDER")) {
                    ...
                    if (supplierProduct != null && supplierProduct.getBigDecimal("lastPrice") != null) {
                        ...
                        sci.setName(ShoppingCartItem.getPurchaseOrderItemDescription(sci.getProduct(), supplierProduct,
                                this.getLocale()));
                    } else {
        ...
        if (getOrderType().equals("PURCHASE_ORDER")) {
            ...
            if (supplierProduct != null || "_NA_".equals(this.getPartyId())) {
                return this.addItem(0, ShoppingCartItem.makePurchaseOrderItem(Integer.valueOf(0), productId, selectedAmount,
                        quantity, features, attributes, prodCatalogId, configWrapper, itemType, itemGroup, dispatcher, this,
                        supplierProduct, shipBeforeDate, shipAfterDate, cancelBackOrderDate));
            } else {
                throw new CartItemModifyException("SupplierProduct not found");
            }
        } else {
            ...
            return this.addItem(0, ShoppingCartItem.makeItem(Integer.valueOf(0), productId, selectedAmount, quantity, null,
                    reservStart,reservEnd, reservLength, reservPersons, accommodationMapId, accommodationSpotId, shipBeforeDate,
                    shipAfterDate, features, attributes, prodCatalogId, configWrapper, itemType, itemGroup, dispatcher, this,
                    Boolean.TRUE, Boolean.TRUE, parentProductId, Boolean.FALSE, Boolean.FALSE));
        }
    }
    ...
    public int addNonProductItem(String itemType, String description, String categoryId, BigDecimal price, BigDecimal quantity,
            Map attributes, String prodCatalogId, String itemGroupNumber, LocalDispatcher dispatcher)
            throws CartItemModifyException {
        ShoppingCart.ShoppingCartItemGroup itemGroup = this.getItemGroupByNumber(itemGroupNumber);
        return this.addItem(0, ShoppingCartItem.makeItem(Integer.valueOf(0), itemType, description, categoryId, price, null,
                quantity, attributes, prodCatalogId, itemGroup, dispatcher, this, Boolean.TRUE));
    }
                ...
                if (billingAddress != null) {
                    if (!ProductWorker.isBillableToAddress(product, billingAddress)) {
                        throw new CartItemModifyException(
                                "The billing address is not compatible with ProductGeos rules of this product.");
                    }
                }
            }
    ...
    /** Add an item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, HashMap features,
            HashMap attributes, String prodCatalogId, String itemType, ProductConfigWrapper configWrapper,
            LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules) throws CartItemModifyException,
            ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, null, null, null, null, null,
                features, attributes, prodCatalogId, configWrapper, itemType, null, dispatcher, this, triggerExternalOps,
                triggerPriceRules, null, Boolean.FALSE, Boolean.FALSE));
    }

    /** Add an item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, HashMap features,
            HashMap attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps,
            Boolean triggerPriceRules) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(productId, amount, quantity, unitPrice, features, attributes, prodCatalogId, itemType, dispatcher,
                triggerExternalOps, triggerPriceRules, Boolean.FALSE, Boolean.FALSE);
    }

    /** Add an (rental)item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice,
            Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, HashMap features, HashMap attributes,
            String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps,
            Boolean triggerPriceRules) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength,
                reservPersons, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this,
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice,
            Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, HashMap features, HashMap attributes,
            String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, LocalDispatcher dispatcher,
            Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks)
            throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength,
                reservPersons, null, null, features, attributes, prodCatalogId, configWrapper, itemType, null, dispatcher, this,
                triggerExternalOps, triggerPriceRules, null, skipInventoryChecks, skipProductChecks));
    }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice,
            ...
            String accommodationSpotId, HashMap features, HashMap attributes, String prodCatalogId, String itemType,
            LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules) throws CartItemModifyException,
            ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart,null,reservLength,
                reservPersons, accommodationMapId, accommodationSpotId, null, null, features, attributes, prodCatalogId, null,
                itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, Boolean.FALSE, Boolean.FALSE));
    }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice,
            Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, String accommodationMapId,
            String accommodationSpotId, HashMap features, HashMap attributes, String prodCatalogId, String itemType,
            LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks,
            Boolean skipProductChecks) throws CartItemModifyException, ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart,null, reservLength,
                reservPersons, accommodationMapId, accommodationSpotId, null, null, features, attributes, prodCatalogId, null,
                itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, skipInventoryChecks,
                skipProductChecks));
    }

    /** Add an accommodation(rental/aggregated)item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice,
            Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersonsDbl, String accommodationMapId,
            String accommodationSpotId, HashMap features, HashMap attributes, String prodCatalogId,
            ProductConfigWrapper configWrapper, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps,
            Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException,
            ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, null,reservLength,
                reservPersonsDbl, accommodationMapId, accommodationSpotId, null, null, features, attributes, prodCatalogId,
                configWrapper, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null,
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, HashMap features,
            ...
            ItemNotFoundException {
        return addItemToEnd(ShoppingCartItem.makeItem(null, productId, amount, quantity, unitPrice, null, null, null, null, null,
                features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps,
    ...
    /**
     * Get all ShoppingCartItems from the cart object with the given productId
     * and optional groupNumber to limit it to a specific item group
     */
    ...
    /**
     * Get all ShoppingCartItems from the cart object with the given
     * productCategoryId and optional groupNumber to limit it to a specific item
     * group
     */
    public List findAllCartItemsInCategory(String productCategoryId, String groupNumber) {
        ...
        try {
            ...
            for (ShoppingCartItem cartItem : cartLines) {
                ...
                if (UtilValidate.isNotEmpty(groupNumber) && !cartItem.isInItemGroup(groupNumber)) {
                    ...
                    // cartItem.getProductId() +
                    // "] becuase not in group [" + groupNumber + "]", module);
                    continue;
                }
                if (CategoryWorker.isProductInCategory(delegator, cartItem.getProductId(), productCategoryId)) {
                    ...
                    // Debug.logInfo("Not using cartItem with product [" +
                    // cartItem.getProductId() +
                    // "] becuase not in category [" + productCategoryId + "]",
                    // module);
                }
            }
        } catch (GenericEntityException e) {
        ...
        // Debug.logInfo("Got [" + itemsToReturn.size() +
        // "] cart items in category [" + productCategoryId +
        // "] and item group [" + groupNumber + "]", module);
        return itemsToReturn;
    }
    ...
    // =============== some misc utility methods, mostly for dealing with lists
    // of items =================
    public void removeExtraItems(List multipleItems, LocalDispatcher dispatcher, int maxItems) throws CartItemModifyException {
        ...
        // remove all except first <maxItems> in list from the cart, first
        // because new cart items are added to the
        // beginning...
    ...
    public void ensureItemsQuantity(List cartItems, LocalDispatcher dispatcher, BigDecimal quantity)
            throws CartItemModifyException {
        Iterator localIter = cartItems.iterator();
        while (localIter.hasNext()) {
            ShoppingCartItem item = (ShoppingCartItem) localIter.next();
    ...
    public BigDecimal ensureItemsTotalQuantity(List cartItems, LocalDispatcher dispatcher, BigDecimal quantity)
            throws CartItemModifyException {
        BigDecimal quantityRemoved = BigDecimal.ZERO;
        // go through the items and reduce quantityToKeep by the item quantities
        // until it is 0, then remove the remaining...
        ...
        while (localIter.hasNext()) {
            ...
            if (quantityToKeep.compareTo(item.getQuantity()) >= 0) {
                // quantityToKeep sufficient to keep it all... just reduce
                // quantityToKeep and move on
                quantityToKeep = quantityToKeep.subtract(item.getQuantity());
            } else {
                // there is more in this than we want to keep, so reduce the
                // quantity, or remove altogether...
                if (quantityToKeep.compareTo(BigDecimal.ZERO) == 0) {
                    ...
                    // there is some to keep, so reduce quantity to
                    // quantityToKeep, at this point we know we'll take up all
                    // of the rest of the quantityToKeep
    ...
    public boolean containAnyWorkEffortCartItems() {
        ...
        for (int i = 0; i < this.cartLines.size(); i++) {
            ShoppingCartItem cartItem = cartLines.get(i);
            if (cartItem.getItemType().equals("RENTAL_ORDER_ITEM")) { // create
                                                                      // workeffort
                                                                      // items?
                return true;
            }
    ...
    public boolean containAllWorkEffortCartItems() {
        ...
        for (int i = 0; i < this.cartLines.size(); i++) {
            ShoppingCartItem cartItem = cartLines.get(i);
            if (!cartItem.getItemType().equals("RENTAL_ORDER_ITEM")) { // not a
                                                                       // item
                                                                       // to
                                                                       // create
                                                                       // workefforts?
    ...
    /**
     * Check to see if the cart contains only Digital Goods, ie no Finished
     * Goods and no Finished/Digital Goods, et cetera. This is determined by
     ...
     * Finished Goods and no Finished/Digital Goods, et cetera. This is
     * determined by making sure no Product has a type where
    ...
    public String addItemGroup(String groupName, String parentGroupNumber) {
        ...
        ShoppingCart.ShoppingCartItemGroup newGroup = new ShoppingCart.ShoppingCartItemGroup(this.nextGroupNumber, groupName,
                parentGroup);
    ...
    public void deleteItemGroup(String groupNumber) {
        ...
        if (itemGroup != null) {
            ...
            // if this is a parent of any set them to this group's parent (or
            // null)
        ...
        if (UtilValidate.isNotEmpty(partyId)) {
            ...
            while (promoCodeIter.hasNext()) {
                ...
                if (checkResult != null) {
                    ...
                    Debug.logWarning(
                            UtilProperties.getMessage(resource_error, "OrderOnUserChangePromoCodeWasRemovedBecause",
    ...
    public void setBillToCustomerPartyId(String billToCustomerPartyId) {
        ...
        if ((UtilValidate.isEmpty(this.orderPartyId)) && !(orderType.equals("PURCHASE_ORDER"))) {
            this.orderPartyId = billToCustomerPartyId; // orderPartyId should be
                                                       // bill-to-customer when
    ...
    public void setBillFromVendorPartyId(String billFromVendorPartyId) {
        this.billFromVendorPartyId = billFromVendorPartyId;
        if ((UtilValidate.isEmpty(this.orderPartyId)) && (orderType.equals("PURCHASE_ORDER"))) {
            this.orderPartyId = billFromVendorPartyId; // orderPartyId should be
                                                       // bill-from-vendor when
                                                       // it is a purchase
            // order
        }
    ...
    public void clear() {
        ...
        this.additionalTaxAuthorities = null;
        this.formToIssue = null;
        this.formToIssueSeriesNo = null;
        this.formToIssueFormNo = null;
        this.formToIssueDate = null;
        this.formToReceive = null;
        this.formToReceiveFormNo = null;
        this.formToReceiveSeriesNo = null;
        this.formToReceiveDate = null;
        ...
        if (ProductStoreWorker.autoSaveCart(this.getDelegator(), this.getProductStoreId())) {
            ...
            if (autoSaveListId == null) {
                try {
                    autoSaveListId = ShoppingListEvents.getAutoSaveListId(this.getDelegator(), null, null, ul,
    ...
    public String getPaymentMethodTypeId(String paymentMethodId) {
        try {
            GenericValue pm = this.getDelegator().findByPrimaryKey("PaymentMethod",
    ...
    /**
     * Locates the index of an existing CartPaymentInfo object or -1 if none
     * found
     */
    ...
    public CartPaymentInfo addPaymentAmount(String id, BigDecimal amount, String refNum, String authCode, boolean isSingleUse,
        ...
        if (isSalesOrder()) {
            ...
            if (billingAddress != null) {
                // this payment method will set the billing address for the
                // order;
    ...
    /**
     * remove declined payment methods for an order from cart. The idea is to
     * call this after an attempted order is rejected
     */
    public void clearDeclinedPaymentMethods(Delegator delegator) {
        String orderId = this.getOrderId();
        if (UtilValidate.isNotEmpty(orderId)) {
            try {
                List declinedPaymentMethods = delegator.findByAnd("OrderPaymentPreference",
                        UtilMisc.toMap("orderId", orderId, "statusId", "PAYMENT_DECLINED"));
    ...
    private void expireSingleUsePayments() {
        ...
        while (i.hasNext()) {
            ...
            try {
                paymentMethod = this.getDelegator().findByPrimaryKey("PaymentMethod",
                        UtilMisc.toMap("paymentMethodId", inf.paymentMethodId));
            } catch (GenericEntityException e) {
            ...
            if (paymentMethod != null) {
                ...
                Debug.logError("ERROR: Received back a null payment method record for expired ID : " + inf.paymentMethodId,
                        module);
            }
        }
    }
    ...
    public List getPaymentMethods() {
        ...
        if (UtilValidate.isNotEmpty(paymentInfo)) {
            ...
            while (paymentMethodIdIter.hasNext()) {
                ...
                try {
                    GenericValue paymentMethod = this.getDelegator().findByPrimaryKeyCache("PaymentMethod",
                            UtilMisc.toMap("paymentMethodId", paymentMethodId));
                    if (paymentMethod != null) {
                        methods.add(paymentMethod);
                    } else {
                        Debug.logError("Error getting cart payment methods, the paymentMethodId [" + paymentMethodId
                                + "] is not valid", module);
                    }
                } catch (GenericEntityException e) {
    ...
    public List getPaymentMethodTypes() {
        ...
        if (UtilValidate.isNotEmpty(paymentInfo)) {
            ...
            while (i.hasNext()) {
                ...
                try {
                    types.add(this.getDelegator().findByPrimaryKeyCache("PaymentMethodType",
                            UtilMisc.toMap("paymentMethodTypeId", id)));
                } catch (GenericEntityException e) {
    ...
    /*
     * determines if the id supplied is a payment method or not by searching in
     * the entity engine
     */
    public boolean isPaymentMethodType(String id) {
        GenericValue paymentMethodType = null;
        try {
            paymentMethodType = this.getDelegator().findByPrimaryKeyCache("PaymentMethodType",
                    UtilMisc.toMap("paymentMethodTypeId", id));
        } catch (GenericEntityException e) {
    ...
    /**
     * Returns ProductStoreFinActSetting based on cart's productStoreId and
     * FinAccountHelper's defined giftCertFinAcctTypeId
     *
     * @param delegator
     * @return
     * @throws GenericEntityException
    ...
    public GenericValue getGiftCertSettingFromStore(Delegator delegator) throws GenericEntityException {
        return delegator.findByPrimaryKeyCache("ProductStoreFinActSetting", UtilMisc.toMap("productStoreId", getProductStoreId(),
                "finAccountTypeId", FinAccountHelper.giftCertFinAccountTypeId));
    }

    /**
     * Determines whether pin numbers are required for gift cards, based on
     * ProductStoreFinActSetting. Default to true.
     *
     * @return
     */
    ...
    /**
     * Returns whether the cart should validate gift cards against FinAccount
     * (ie, internal gift certificates). Defaults to false.
     *
     * @param delegator
     * @return
     */
    ...
    /**
     * Returns the ShoppingCartItem (key) and quantity (value) associated with
     * the ship group
     */
    ...
    public void clearItemShipInfo(ShoppingCartItem item) {
        ...
        // DEJ20100107: commenting this out because we do NOT want to clear out
        // ship group info since there is information
        // there that will be lost; good enough to clear the item/group
        // association which can be restored later (though
        // questionable, the whole processes using this should be rewritten to
    ...
    /**
     * Updates the shipBefore and shipAfterDates of all ship groups that the
     * item belongs to, re-setting ship group ship before date if item ship
     * before date is before it and ship group ship after date if item ship
     * after date is before it.
     *
     * @param item
     */
    ...
    public void positionItemToGroup(ShoppingCartItem item, BigDecimal quantity, int fromIndex, int toIndex,
    ...
    public void setShippingContactMechId(int idx, String shippingContactMechId) {
        ...
        if (isSalesOrder() && UtilValidate.isNotEmpty(shippingContactMechId)) {
            // Verify if the new address is compatible with the ProductGeos
            // rules of
            // the products already in the cart
            GenericValue shippingAddress = null;
            try {
                shippingAddress = this.getDelegator().findByPrimaryKey("PostalAddress",
                        UtilMisc.toMap("contactMechId", shippingContactMechId));
            } catch (GenericEntityException gee) {
                Debug.logError(gee, "Error retrieving the shipping address for contactMechId [" + shippingContactMechId + "].",
                        module);
            }
                        ...
                        if (UtilValidate.isNotEmpty(product)) {
                            if (!ProductWorker.isShippableToAddress(product, shippingAddress)) {
                                throw new IllegalArgumentException(
                                        "The shipping address is not compatible with ProductGeos rules.");
                            }
                        }
                    }
                }
    ...
    public GenericValue getShippingAddress(int idx) {
        if (this.getShippingContactMechId(idx) != null) {
            try {
                return getDelegator().findByPrimaryKey("PostalAddress",
                        UtilMisc.toMap("contactMechId", this.getShippingContactMechId(idx)));
            } catch (GenericEntityException e) {
    ...
    // Preset with default values some of the checkout options to get a quicker
    // checkout process.
    public void setDefaultCheckoutOptions(LocalDispatcher dispatcher) {
        ...
        if (getOrderType().equals("SALES_ORDER")) {
            ...
            // set as the default shipping location the first from the list of
            // available shipping locations
            if (this.getPartyId() != null && !this.getPartyId().equals("_NA_")) {
                try {
                    GenericValue orderParty = this.getDelegator().findByPrimaryKey("Party",
                            UtilMisc.toMap("partyId", this.getPartyId()));
            ...
            ShippingEstimateWrapper shipEstimateWrapper = org.ofbiz.order.shoppingcart.shipping.ShippingEstimateWrapper
            ...
            String companyId = this.getBillFromVendorPartyId();
            if (companyId != null) {
                // the facilityId should be set prior to triggering default
                // options, otherwise we do not set up facility
                // information
                String defaultFacilityId = getFacilityId();
                if (defaultFacilityId != null) {
                    GenericValue facilityContactMech = ContactMechWorker.getFacilityContactMechByPurpose(this.getDelegator(),
                            facilityId, UtilMisc.toList("SHIPPING_LOCATION", "PRIMARY_LOCATION"));
                    if (facilityContactMech != null) {
                        this.setShippingOriginContactMechId(0, facilityContactMech.getString("contactMechId"));
                    }
                }
            }
        } else {
            // checkout options for purchase orders
            // TODO: should we select a default agreement? For now we don't do
            // this.
    ...
    /** Returns the sub-total in the cart (item-total - discount). */
    public BigDecimal getItemAdjustmentTotal() {
        BigDecimal itemsTotal = BigDecimal.ZERO;
        Iterator i = iterator();
        while (i.hasNext()) {
            for(GenericValue adjustment : (((ShoppingCartItem) i.next()).getAdjustments())){
                itemsTotal = itemsTotal.add(adjustment.getBigDecimal("amount"));
            }
        }
        return itemsTotal;
    }
    ...
    public BigDecimal getGrandTotal() {
        if(UtilValidate.isNotEmpty(this.productStoreId)){
            try {
                ...
                if("Y".equals(productStore.getString("pricesIncludeTax"))) {
                    return this.getSubTotal().add(this.getItemAdjustmentTotal());
                }
                if("PURCHASE_ORDER".equals(this.getOrderType())&& "PRICEINCLUDETAX".equals(this.getFormToIssue()))
    ...
    public BigDecimal getDisplayGrandTotal() {
        return this.getDisplaySubTotal().add(this.getTotalShipping()).add(this.getTotalSalesTax())
                .add(this.getOrderOtherAdjustmentTotal());
    }
    ...
    public BigDecimal getOrderPaymentPreferenceTotalByType(String paymentMethodTypeId) {
        ...
        for (Iterator iter = paymentInfo.iterator(); iter.hasNext();) {
            ...
            if (payment.paymentMethodId != null) {
                try {
                    // need to determine the payment method type from the
                    // payment method
                    GenericValue paymentMethod = this.getDelegator().findByPrimaryKeyCache("PaymentMethod",
                            UtilMisc.toMap("paymentMethodId", payment.paymentMethodId));
                    if (paymentMethod != null) {
    ...
    /**
     * Add an adjustment to the order; don't worry about setting the orderId,
     * orderItemSeqId or orderAdjustmentId; they will be set when the order is
     * created
     */
    ...
    /**
     * go through the order adjustments and remove all adjustments with the
     * given type
     */
    public void removeAdjustmentByType(String orderAdjustmentTypeId) {
        ...
        // make a list of adjustment lists including the cart adjustments and
        // the cartItem adjustments for each item
    ...
    /**
     * Returns the total SHIPPABLE quantity in the cart for a specific ship
     * group.
     */
    ...
    /**
     * Returns a List of shippable item info (quantity, size, weight) for a
     * specific ship group
     */
    ...
    /**
     * Returns a Map of all features applied to products in the cart with
     * quantities for a specific ship group.
     */
    ...
    /**
     * Returns true if the user wishes to view the cart everytime an item is
     * added.
     */
    ...
    /**
     * Returns the order ID associated with this cart or null if no order has
     * been created yet.
     */
    ...
    /**
     * Returns the first attempt order ID associated with this cart or null if
    ...
    /**
     * Removes a free shipping ProductPromoAction by trying to find one in the
     * list with the same primary key.
     */
    ...
    /**
     * Adds a ProductPromoAction to be used for free shipping (must be of type
     * free shipping, or nothing will be done).
     */
    ...
    public void addProductPromoUse(String productPromoId, String productPromoCodeId, BigDecimal totalDiscountAmount,
        ...
        if (Debug.verboseOn())
            Debug.logVerbose("Used promotion [" + productPromoId + "] with code [" + productPromoCodeId
                    + "] for total discount [" + totalDiscountAmount + "] and quantity left in actions [" + quantityLeftInActions
                    + "]", module);
    ...
    /**
     * Get total discount for a given ProductPromo, or for ANY ProductPromo if
     * the passed in productPromoId is null.
     */
    ...
    public void clearAllPromotionInformation() {
        ...
        // clear promo uses & reset promo code uses, and reset info about cart
        // items used for promos (ie qualifiers and
        // benefiters)
    ...
    public void clearAllPromotionAdjustments() {
        ...
        // remove cart lines that are promos (ie GWPs) and cart line adjustments
        // from promo actions
        Iterator cartItemIter = this.iterator();
        while (cartItemIter.hasNext()) {
            ShoppingCartItem checkItem = (ShoppingCartItem) cartItemIter.next();
            if (checkItem.getIsPromo()) {
                ...
                // found a promo item with the productId, see if it has a
                // matching adjustment on it
                Iterator checkOrderAdjustments = UtilMisc.toIterator(checkItem.getAdjustments());
                while (checkOrderAdjustments != null && checkOrderAdjustments.hasNext()) {
                    GenericValue checkOrderAdjustment = (GenericValue) checkOrderAdjustments.next();
                    if (UtilValidate.isNotEmpty(checkOrderAdjustment.getString("productPromoId"))
    ...
    /**
     * Adds a promotion code to the cart, checking if it is valid. If it is
     * valid this will return null, otherwise it will return a message stating
     * why it was not valid
     *
     * @param productPromoCodeId
     *            The promotion code to check and add
     * @return String that is null if valid, and added to cart, or an error
     *         message of the code was not valid and not added to the cart.
     */
     ...
     *
     * @param partyId
     *            identifier of the party to associate to order
     * @param roleTypeId
    ...
    /**
     * Returns the Id of an AGGREGATED_CONF product having exact configId. If
     * AGGREGATED_CONF product do not exist, creates one, associates it to the
    ...
    public String getAggregatedInstanceId(ShoppingCartItem item, LocalDispatcher dispatcher) {
        ...
        try {
            ...
            // AGGREGATED product while creating production
            // run.
            List productionRunTemplates = delegator.findByAnd("WorkEffortGoodStandard", UtilMisc.toMap("productId",
                    item.getProductId(), "workEffortGoodStdTypeId", "ROU_PROD_TEMPLATE", "statusId", "WEGS_CREATED"));
        ...
        synchronized (cartLines) {
            ...
            for (ShoppingCartItem item : cartLines) {
                ...
                orderItem.set("productId",
                        UtilValidate.isNotEmpty(aggregatedInstanceId) ? aggregatedInstanceId : item.getProductId());
                ...
                orderItem.set("estimatedShipDate", item.getShipAfterDate());
                ...
                // don't do anything with adjustments here, those will be added
                // below in makeAllAdjustments
            }
            return result;
        }
    }
    ...
    /**
     * create WorkEfforts from the shoppingcart items when itemType =
     * RENTAL_ORDER_ITEM
     */
    public List makeWorkEfforts() {
        ...
        for (ShoppingCartItem item : cartLines) {
            if ("RENTAL_ORDER_ITEM".equals(item.getItemType())) {
                ...
                workEffort.set("estimatedCompletionDate", item.getReservEnd());
                workEffort.set("reservLength",item.getReservLength());
    ...
    /**
     * make a list of all adjustments including order adjustments, order line
     * adjustments, and special adjustments (shipping and tax if applicable)
     */
    public List<GenericValue> makeAllAdjustments() {
        List<GenericValue> allAdjs = FastList.newInstance();

        // before returning adjustments, go through them to find all that need
        // counter adjustments (for instance: free
        // shipping)
        for (GenericValue orderAdjustment : this.getAdjustments()) {
            ...
            if ("SHIPPING_CHARGES".equals(orderAdjustment.get("orderAdjustmentTypeId"))) {
                ...
                while (fsppas.hasNext()) {
                    ...
                    // TODO - we need to change the way free shipping promotions
                    // work
                    /*
                     * if ((productPromoAction.get("productId") == null ||
                     * productPromoAction
                     * .getString("productId").equals(this.getShipmentMethodTypeId
                     * ())) && (productPromoAction.get("partyId") == null ||
                     * productPromoAction
                     * .getString("partyId").equals(this.getCarrierPartyId())))
                     * { Double shippingAmount =
                     * Double.valueOf(OrderReadHelper.calcOrderAdjustment
                     * (orderAdjustment, new
                     * BigDecimal(getSubTotal())).negate().doubleValue()); //
                     * always set orderAdjustmentTypeId to SHIPPING_CHARGES for
                     * free shipping adjustments GenericValue fsOrderAdjustment
                     * = getDelegator().makeValue("OrderAdjustment",
                     * UtilMisc.toMap("orderItemSeqId",
                     * orderAdjustment.get("orderItemSeqId"),
                     * "orderAdjustmentTypeId", "SHIPPING_CHARGES", "amount",
                     * shippingAmount, "productPromoId",
                     ...
                     * allAdjs.add(fsOrderAdjustment); // if free shipping IS
                     * applied to this orderAdjustment, break // out of the loop
                     * so that even if there are multiple free // shipping
                     * adjustments that apply to this orderAdjustment it // will
                     * only be compensated for once break; }
                     */
                }
            }
        }
                ...
                for (GenericValue orderAdjustment : adjs) {
                    ...
                    if ("SHIPPING_CHARGES".equals(orderAdjustment.get("orderAdjustmentTypeId"))) {
                        ...
                        while (fsppas.hasNext()) {
                             ...
                             * if ((productPromoAction.get("productId") == null
                             * ||
                             * productPromoAction.getString("productId").equals
                             * (item.getShipmentMethodTypeId())) &&
                             * (productPromoAction.get("partyId") == null ||
                             * productPromoAction
                             * .getString("partyId").equals(item
                             * .getCarrierPartyId()))) { Double shippingAmount =
                             * Double
                             * .valueOf(OrderReadHelper.calcItemAdjustment(
                             * orderAdjustment, new
                             * BigDecimal(item.getQuantity()), new
                             * BigDecimal(item
                             * .getItemSubTotal())).negate().doubleValue()); //
                             ...
                             * "amount", shippingAmount, "productPromoId",
                             * productPromoAction.get("productPromoId"),
                             * "productPromoRuleId",
                             * productPromoAction.get("productPromoRuleId"),
                             * "productPromoActionSeqId",
                             * productPromoAction.get(
                             * "productPromoActionSeqId")));
                             * allAdjs.add(fsOrderAdjustment); // if free
                             * shipping IS applied to this orderAdjustment,
                             * break // out of the loop so that even if there
                             * are multiple free // shipping adjustments that
                             * apply to this orderAdjustment it // will only be
                             * compensated for once break; }
                             */
                        }
                    }
                }
    ...
    /**
     * make a list of all quote adjustments including header adjustments, line
     * adjustments, and special adjustments (shipping and tax if applicable).
    ...
    /**
     * make a list of all OrderPaymentPreferences and Billing info including all
    ...
    public List makeAllOrderPaymentInfos(LocalDispatcher dispatcher) {
        ...
        while (i.hasNext()) {
            CartPaymentInfo inf = (CartPaymentInfo) i.next();
            if("PATIENT".equals(inf.paymentMethodTypeId) || "CASH".equals(inf.paymentMethodTypeId) || "CASH PAYING".equals(inf.paymentMethodTypeId) || "CREDIT_CARD".equals(inf.paymentMethodTypeId)) {
        ...
        while (itemIter.hasNext()) {
            ...
            if (responses != null) {
                ...
                while (ri.hasNext()) {
                    ...
                    try {
                        response = this.getDelegator().findByPrimaryKey("SurveyResponse",
                                UtilMisc.toMap("surveyResponseId", responseId));
                    } catch (GenericEntityException e) {
    ...
    /**
     * make a list of OrderContactMechs from the ShoppingCart and the
     * ShoppingCartItems
     */
    ...
    public List makeAllOrderItemAssociations() {
        ...
        for (ShoppingCartItem item : cartLines) {
            ...
            if (requirementId != null) {
                try {
                    List commitments = getDelegator().findByAnd("OrderRequirementCommitment",
                    ...
                    Debug.logError(e, "Unable to load OrderRequirementCommitment records for requirement ID : " + requirementId,
                            module);
                }
            }
    ...
    public Map makeCartMap(LocalDispatcher dispatcher, boolean explodeItems) {
        ...
        result.put("deliveryDate", this.getDeliveryDate());
    ...
    public void createDropShipGroups(LocalDispatcher dispatcher) throws CartItemModifyException {
        ...
        // ShoppingCart.setFacilityId() doesn't seem to be
        // used anywhere
        String facilityId = null;
        if (UtilValidate.isNotEmpty(this.getProductStoreId())) {
            try {
                GenericValue productStore = this.getDelegator().findByPrimaryKeyCache("ProductStore",
                        UtilMisc.toMap("productStoreId", this.getProductStoreId()));
                facilityId = productStore.getString("inventoryFacilityId");
            } catch (Exception e) {
                Debug.logError(
                        UtilProperties.getMessage(resource_error, "OrderProblemGettingProductStoreRecords", locale)
                                + e.getMessage(), module);
                return;
            }
        }
        ...
        // originalShipGroupIndex = dropShipQuantity } } to
        // collect drop-shippable items
        ...
        for (int shipGroupIndex = 0; shipGroupIndex < shipGroups.size(); shipGroupIndex++) {
            ...
            while (siit.hasNext()) {
                ...
                if ("PRODRQM_DS".equals(requirementMethodEnumId)) {
                    ...
                    // Drop ship the quantity not available in inventory if the
                    // product is marked drop-ship on low
                    // inventory
                    try {

                        // Get ATP for the product
                        Map getProductInventoryAvailableResult = dispatcher.runSync("getInventoryAvailableByFacility",
                                UtilMisc.toMap("productId", productId, "facilityId", facilityId));
                        BigDecimal availableToPromise = (BigDecimal) getProductInventoryAvailableResult
                                .get("availableToPromiseTotal");
                        ...
                        Debug.logWarning(
                                UtilProperties.getMessage(resource_error, "OrderRunServiceGetInventoryAvailableByFacilityError",
                ...
                try {
                    Map getSuppliersForProductResult = dispatcher.runSync("getSuppliersForProduct", UtilMisc
                            .<String, Object> toMap("productId", productId, "quantity", dropShipQuantity, "canDropShip", "Y",
                                    "currencyUomId", getCurrency()));
                    List supplierProducts = (List) getSuppliersForProductResult.get("supplierProducts");

                    // Order suppliers by supplierPrefOrderId so that preferred
                    // suppliers are used first
    ...
    protected void finalize() throws Throwable {
        // DEJ20050518 we should not call clear because it kills the auto-save
        // shopping list and is unnecessary given that
        // when this object is GC'ed it will cause everything it points to that
        // isn't referenced anywhere else to be GC'ed
        // too: this.clear();
        super.finalize();
    }
    ...
    public String[] getAdditionalTaxAuthorities() {
        return additionalTaxAuthorities;
    }

    public void setAdditionalTaxAuthorities(String[] taxAuthorities) {
        this.additionalTaxAuthorities = taxAuthorities;
    }

    public String getFormToIssue() {
        return formToIssue;
    }

    public void setFormToIssue(String formToIssue) {
        this.formToIssue = formToIssue;
    }

    public String getFormToIssueSeriesNo() {
        return formToIssueSeriesNo;
    }

    public void setFormToIssueSeriesNo(String formToIssueSeriesNo) {
        this.formToIssueSeriesNo = formToIssueSeriesNo;
    }

    public String getFormToIssueFormNo() {
        return formToIssueFormNo;
    }

    public void setFormToIssueFormNo(String formToIssueFormNo) {
        this.formToIssueFormNo = formToIssueFormNo;
    }

    public String getFormToIssueDate() {
        return formToIssueDate;
    }

    public void setFormToIssueDate(String formToIssueDate) {
        this.formToIssueDate = formToIssueDate;
    }

    public String getFormToReceive() {
        return formToReceive;
    }

    public void setFormToReceive(String formToReceive) {
        this.formToReceive = formToReceive;
    }

    public String getFormToReceiveSeriesNo() {
        return formToReceiveSeriesNo;
    }

    public void setFormToReceiveSeriesNo(String formToReceiveSeriesNo) {
        this.formToReceiveSeriesNo = formToReceiveSeriesNo;
    }

    public String getFormToReceiveDate() {
        return formToReceiveDate;
    }

    public void setFormToReceiveDate(String formToReceiveDate) {
        this.formToReceiveDate = formToReceiveDate;
    }

    public String getFormToReceiveFormNo() {
        return formToReceiveFormNo;
    }

    public void setFormToReceiveFormNo(String formToReceiveFormNo) {
        this.formToReceiveFormNo = formToReceiveFormNo;
    }
    ...
    public Boolean getIgnoreTaxes() {
        return ignoreTaxes;
    }

    public void setIgnoreTaxes(Boolean ignoreTaxes) {
        this.ignoreTaxes = ignoreTaxes;
    }

    public void setShippingOriginContactMechId(int shipGroupIndex, String shippingOriginContactMechId) {
        if (shipGroupIndex != -1) {
            CartShipInfo csi = this.getShipInfo(shipGroupIndex);
            if (csi != null) {
                csi.setShippingOriginContactMechId(shippingOriginContactMechId);
            }
        }
        this.shippingOriginContactMechId = shippingOriginContactMechId;
    }
    ...
    public static class ShoppingCartItemGroup implements Serializable {
        ...
        /**
         * Note that to avoid foreign key issues when the groups are created a
         * parentGroup should have a lower number than the child group.
         */
    ...
    public static class CartShipInfo implements Serializable {
        ...
        public String getShippingOriginContactMechId() {
            return shippingOriginContactMechId;
        }

        public void setShippingOriginContactMechId(String shippingOriginContactMechId) {
            this.shippingOriginContactMechId = shippingOriginContactMechId;
        }
        ...
        public List makeItemShipGroupAndAssoc(Delegator delegator, ShoppingCart cart, long groupIndex) {
            ...
            if (this.shippingOriginContactMechId != null) {
                GenericValue orderCm = delegator.makeValue("OrderContactMech");
                orderCm.set("contactMechPurposeTypeId", "SHIP_ORIG_LOCATION");
                orderCm.set("contactMechId", this.shippingOriginContactMechId);
                values.add(orderCm);
            }
            ...
            if (shipEstimate.compareTo(BigDecimal.ZERO) != 0) {
                ...
                shipAdj.set("comments", "Shipping and Handling");
                shipAdj.set("description", "Shipping and Handling");
                String applicableOn = "";
                String overrideGlAccountId = null;
                if ("SALES_ORDER".equals(this.orderTypeId)) {
                    applicableOn = "SALES";
                    overrideGlAccountId = "1200000";
                } else {
                    applicableOn = "PURCHASES";
                    overrideGlAccountId = "2200000";
                }
                try {
                    List<GenericValue> adjustments = delegator
                            .findByAnd("OrderAdjustmentType",
                                    UtilMisc.toMap("orderAdjustmentTypeId", "SHIPPING_CHARGES", "applicableOn", applicableOn),
                                    null, true);
                    if (UtilValidate.isNotEmpty(adjustments)) {
                        GenericValue adjValue = EntityUtil.getFirst(adjustments);
                        if (adjValue != null) {
                            String glAccountId = adjValue.getString("glAccountId");
                ...
                shipAdj.set("overrideGlAccountId", overrideGlAccountId);
                values.add(shipAdj);
            }
        ...
        private boolean isShippableToAddress(ShoppingCartItem item) {
            if ("SALES_ORDER".equals(getOrderTypeId())) {
                ...
                GenericValue shippingAddress = null;
                try {
                    shippingAddress = item.getDelegator().findByPrimaryKey("PostalAddress",
                            UtilMisc.toMap("contactMechId", this.internalContactMechId));
                } catch (GenericEntityException gee) {
                    Debug.logError(gee, "Error retrieving the shipping address for contactMechId [" + this.internalContactMechId
                            + "].", module);
                }
                if (shippingAddress != null) {
                    GenericValue product = item.getProduct();
    ...
    public static class CartPaymentInfo implements Serializable, Comparable {
        ...
        public GenericValue getBillingAddress(Delegator delegator) {
            ...
            if ("PaymentMethod".equals(valueObj.getEntityName())) {
                ...
                // billing account, credit card, gift card, eft account all have
                // postal address
                try {
            ...
            if (valueObj != null) {
                ...
                if ("Y".equals(splitPayPrefPerShpGrp) && cart.paymentInfo.size() > 1) {
                    throw new GeneralRuntimeException(
                            "Split Payment Preference per Ship Group does not yet support multiple Payment Methods");
                }
                if ("Y".equals(splitPayPrefPerShpGrp) && cart.paymentInfo.size() == 1) {
                    ...
                    while (shipIter.hasNext()) {
                        ...
                        maxAmount = csi
                                .getTotal()
                                .add(cart.getOrderOtherAdjustmentTotal().divide(new BigDecimal(cart.getShipGroupSize()),
                        ...
                        if (paymentMethodId != null || "FIN_ACCOUNT".equals(paymentMethodTypeId)) {
                            ...
                            // external payment method types require
                            // notification when received
                            // internal payment method types are assumed to be
                            // in-hand
                    ...
                    if (paymentMethodId != null || "FIN_ACCOUNT".equals(paymentMethodTypeId)) {
                        ...
                        // external payment method types require notification
                        // when received
                        // internal payment method types are assumed to be
                        // in-hand
                        if (paymentMethodTypeId.startsWith("EXT_")) {
        ...
        public String toString() {
            return "Pm: " + paymentMethodId + " / PmType: " + paymentMethodTypeId + " / Amt: " + amount + " / Ref: " + refNum[0]
                    + "!" + refNum[1];
        }
    }
