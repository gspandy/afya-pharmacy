...
public class CheckOutEvents {
	...
	public static String setCheckOutPages(HttpServletRequest request, HttpServletResponse response) {
		...
		if ("shippingaddress".equals(curPage) == true) {
			...
			for (int shipGroupIndex = 0; shipGroupIndex < cart.getShipGroupSize(); shipGroupIndex++) {
				...
				Map<String, ? extends Object> callResult = checkOutHelper.finalizeOrderEntryShip(shipGroupIndex, shippingContactMechId, null,
						supplierPartyId);
			...
			if (UtilValidate.isNotEmpty(taxAuthPartyGeoIds)) {
				try {
					Map<String, ? extends Object> createCustomerTaxAuthInfoResult = dispatcher.runSync("createCustomerTaxAuthInfo", UtilMisc
							.<String, Object> toMap("partyId", cart.getPartyId(), "taxAuthPartyGeoIds", taxAuthPartyGeoIds, "partyTaxId", partyTaxId,
									"isExempt", isExempt, "userLogin", userLogin));
	...
	/**
	 * Method to determine the initial checkout page based on requirements. This
	 * will also set any cart variables necessary to satisfy the requirements,
	 * such as setting the shipment method according to the type of items in the
	 * cart.
	 */
	 ...
	 * payment options. Shipment option should already be set by the
	 * quickcheckout form.
	 */
	public static String setQuickCheckOutOptions(HttpServletRequest request, HttpServletResponse response) {
		ShoppingCart cart = (ShoppingCart) request.getSession().getAttribute("shoppingCart");
		String shippingOriginContactMechId = request.getParameter("shipping_origin_contact_mech_id");
		if (shippingOriginContactMechId == null) {
			request.setAttribute("_ERROR_MESSAGE_", "Shipping Origin is missing.");
			return "error";
		}
		if (cart.getShippingOriginContactMechId() == null && shippingOriginContactMechId != null) {
			cart.setShippingOriginContactMechId(-1, shippingOriginContactMechId);
		}
	...
	public static String setCheckOutOptions(HttpServletRequest request, HttpServletResponse response) {
		...
		if (orderItemShipGroupInfo != null) {
			for (GenericValue osiInfo : orderItemShipGroupInfo) {
		...
		for (GenericValue gv : orderAdjustments) {
			try {
				...
				if (taxAuthority != null && "Y".equals(taxAuthority.get("requireFormC")))
					request.setAttribute("formCRequired", true);
		...
		if (UtilValidate.isNotEmpty(taxAuthPartyGeoIds)) {
			try {
				Map createCustomerTaxAuthInfoResult = dispatcher.runSync("createCustomerTaxAuthInfo", UtilMisc.toMap("partyId", cart.getPartyId(),
						"taxAuthPartyGeoIds", taxAuthPartyGeoIds, "partyTaxId", partyTaxId, "isExempt", isExempt));
				ServiceUtil.getMessages(request, createCustomerTaxAuthInfoResult, null);
				if (ServiceUtil.isError(createCustomerTaxAuthInfoResult)) {
					return "error";
				}
		...
		Map optResult = checkOutHelper.setCheckOutOptions(shippingMethod, shippingContactMechId, selectedPaymentMethods, singleUsePayments,
				billingAccountId, shippingInstructions, orderAdditionalEmails, maySplit, giftMessage, isGift, internalCode, shipBeforeDate,
				shipAfterDate);
	...
	public static String createOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		/*
		 * Change made on 28-05-10 for not to use marketing service
		...
		if (callResult != null) {
			ServiceUtil.getMessages(request, callResult, null);
			if (ServiceUtil.isError(callResult)) {
				// messages already setup with the getMessages call, just return
	...
	public static String calcTax(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			// "Shipping Origin or Shipping Destination is missing .");
			return "success";
		}
		return "success";
	}
	...
	public static String processPayment(HttpServletRequest request, HttpServletResponse response) {
		// run the process payment process + approve order when complete; may
		// also run sync fulfillments
		int failureCode = 0;
		try {
	...
	public static String checkExternalCheckout(HttpServletRequest request, HttpServletResponse response) {
		...
		if ("EXT_PAYPAL".equals(paymentMethodTypeId) || cart.getPaymentMethodTypeIds().contains("EXT_PAYPAL")) {
			...
			try {
				payPalProdStorePaySettings = delegator.findByAnd("ProductStorePaymentSetting", "productStoreId",
						productStore.getString("productStoreId"), "paymentMethodTypeId", "EXT_PAYPAL");
	...
	public static String finalizeOrderEntry(HttpServletRequest request, HttpServletResponse response) {
		...
		if (UtilValidate.isNotEmpty(cart.getOrderDate()) && UtilValidate.isNotEmpty(cart.getDeliveryDate()) && (cart.getDeliveryDate().before(cart.getOrderDate()))) {
		...
		// necessary to avoid infinite looping when in a funny state, and will
		// go right back to beginning
		if (mode == null) {
			return "customer";
		}
		...
		if (mode != null && (mode.equals("ship") || mode.equals("options"))) {
			...
			for (int shipGroupIndex = 0; shipGroupIndex < cart.getShipGroupSize(); shipGroupIndex++) {
				...
				if (mode != null && mode.equals("ship")) {
					...
					callResult = checkOutHelper.finalizeOrderEntryShip(shipGroupIndex, shippingContactMechId, shippingOriginContactMechId,
							supplierPartyId);
					ServiceUtil.addErrors(errorMessages, errorMaps, callResult);
				}
				// set the options
		...
		if (mode != null && mode.equals("payment")) {
			...
			// If the user has just created a new payment method, add it to the
			// map with a null amount, so that
			...
			if (errorMessages.size() == 0 && errorMaps.size() == 0) {
				...
				// WARNING: if gcPaymentMethodId is not empty, all the
				// previously set payment methods will be removed
	...
	public static String determineNextFinalizeStep(HttpServletRequest request, HttpServletResponse response) {
		...
		String[] processOrder = new String[] { "adjustments" };
		if ("Advance".equals(request.getSession().getAttribute("orderFlow"))) {
			processOrder = new String[] { "customer", "shipping", "shipGroups", "options", "term", "payment", "addparty", "paysplit", "adjustments" };
		}
		...
		if (cart.getOrderType().equals("PURCHASE_ORDER")) {
			...
			if ("Advance".equals(request.getSession().getAttribute("orderFlow"))) {
				...
				processOrder = new String[] { "shipping", "adjustments" };
			}
		}
		...
		for (int i = 0; i < processOrder.length; i++) {
			...
			} else if (currProcess.equals("adjustments")) {
				return "adjustments";
			}
		}
		...
		// this is used to go back to a previous page in checkout after
		// processing all of the changes, just to make sure we
		// get everything...
	 ...
	 * This method returns the amount that will be charged to the billing
	 * account.
	 * 
	 * An amount can be associated with the billingAccountId with a parameter
	 * billingAccountAmount. If no amount is specified, then the entire
	 * available balance of the given billing account will be used. If there is
	 * an error, a null will be returned.
	 * 
	...
	private static BigDecimal determineBillingAccountAmount(String billingAccountId, String billingAccountAmount, LocalDispatcher dispatcher) {
		...
		// set the billing account amount to the minimum of billing account
		// available balance or amount input if less than
		// balance
		if (UtilValidate.isNotEmpty(billingAccountId)) {
			...
			// set amount to be charged to entered amount unless it exceeds the
			// available balance
	...
	/**
	 * Create a replacement order from an existing order against a lost shipment
	 * etc.
	 **/
	...
	public static String generateTallyXML(HttpServletRequest request, HttpServletResponse reponse) {

		GenericDispatcher dispatcher = (GenericDispatcher) request.getAttribute("dispatcher");
		GenericValue userLogin = (GenericValue) request.getSession().getAttribute("userLogin");
		try {
			Map results = dispatcher.runSync("massGenerateTallyVouchers",
					UtilMisc.toMap("userLogin", userLogin, "orderIdList", UtilMisc.toList(request.getParameter("orderId"))));
			...
			reponse.addHeader("content-disposition", "attachment; filename=Vouchers.xml");
			StringBuilder buffer = new StringBuilder();
			buffer.append("<ENVELOPE><HEADER><TALLYREQUEST>Import Data</TALLYREQUEST></HEADER><BODY><IMPORTDATA><REQUESTDESC>");
			buffer.append("<REPORTNAME>Vouchers</REPORTNAME></REQUESTDESC><REQUESTDATA>");
	...
	public static String checkOrderName(HttpServletRequest request, HttpServletResponse response) {
		ShoppingCart cart = (ShoppingCart) request.getSession().getAttribute("shoppingCart");
		if (cart.getOrderName() == null || UtilValidate.isEmpty(cart.getOrderName().trim())) {
			request.setAttribute("_ERROR_MESSAGE_", "Order Name cannot be blank.");
			return "error";
