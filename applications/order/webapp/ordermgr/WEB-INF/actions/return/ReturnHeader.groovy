...
if(UtilValidate.isNotEmpty(parameters.get("returnId"))){

	returnContactAddressList =delegator.findByAnd("ReturnItem", [returnId : returnId]);
	contactMechList = [];
	if(UtilValidate.isNotEmpty(returnContactAddressList)){
	contactMechList = delegator.findByAnd("OrderContactMech", [orderId : returnContactAddressList.get(0).getString("orderId")]);
	}
	GenericValue contactOrgin = null;
	GenericValue contactDestination = null;
	for(GenericValue contact : contactMechList){
		if("SHIPPING_LOCATION".equals(contact.getString("contactMechPurposeTypeId"))){
			contactOrgin = delegator.findOne("PostalAddress", [contactMechId : contact.contactMechId], true);
		}
		if("SHIP_ORIG_LOCATION".equals(contact.getString("contactMechPurposeTypeId"))){
			contactDestination = delegator.findOne("PostalAddress", [contactMechId : contact.contactMechId], true);
		}

	}


if(UtilValidate.isNotEmpty(contactOrgin)&& UtilValidate.isNotEmpty(contactDestination)){
	context.contactOrgin = contactOrgin;
	context.contactDestination = contactDestination;
}
}
