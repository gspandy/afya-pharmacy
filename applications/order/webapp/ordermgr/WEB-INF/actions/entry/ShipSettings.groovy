...
if ("SALES_ORDER".equals(cart.getOrderType())) {
    ...
    productStore = delegator.findByAnd("ProductStore", [productStoreId : cart.getProductStoreId()]);
     
    companyId = productStore.payToPartyId;
    ...
    if (companyId) {
		if(companyId.equals(orderPartyId)){
			companyId = "Company";
		}
        facilityMaps = FastList.newInstance();
        facilities = delegator.findByAndCache("Facility", [ownerPartyId : companyId]);
	...
	vendorId = cart.getBillFromVendorPartyId();
	shipToParty = delegator.findByPrimaryKey("Party", [partyId : vendorId]);
	shipToPartyShippingContactMechList = ContactHelper.getContactMech(shipToParty,"SHIP_ORIG_LOCATION", "POSTAL_ADDRESS", false);
	context.shipToPartyShippingContactMechList = shipToPartyShippingContactMechList;
}
}
