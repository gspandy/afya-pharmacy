...
def getCreditDatils(Map<String,Object> map,String thruDate){
	...
	while (result.next()){
		if("DEBIT_MEMO".equals(result.getString("paymentTypeId")) || "DEBIT_MEMO".equals(result.getString("paymentTypeId")) )
...
def getDebitDatils(Map<String,Object> map,String thruDate){
	...
	while (result.next()){
		map.put("debit", result.getBigDecimal("amount") == null ? BigDecimal.ZERO : result.getBigDecimal("amount").negate() );
	}
	sqlProc.close();
	return map;
}
...
def buildCreditOrDebitQuery(String partyId,String currency,String flag,String thruDate){
	String query = ""; 
	if("D".equals(flag)){
		query = "SELECT b.orig_amount AS amount,c.payment_type_id AS paymentTypeId,b.gl_account_type_id AS glAccountType FROM acctg_trans AS a JOIN acctg_trans_entry AS b ON a.acctg_trans_id=b.acctg_trans_id " +
				...
				" AND b.orig_currency_uom_id = " + "'" + currency + "'" + " AND IF(c.payment_type_id='DEBIT_MEMO',b.debit_credit_flag='C',b.debit_credit_flag='D') " ;
		query = query + " AND a.payment_id IS NOT NULL AND a.invoice_id IS NULL AND c.status_id <> 'PMNT_CANCELLED' AND c.status_id <> 'PMNT_VOID' "
	}else{
	query = "SELECT SUM(b.orig_amount) AS amount FROM acctg_trans AS a JOIN acctg_trans_entry AS b ON a.acctg_trans_id=b.acctg_trans_id " +
			" JOIN Invoice c ON a.invoice_id=c.invoice_id WHERE " +
			" a.party_id = " + "'"  + partyId + "'" + " AND a.transaction_date >= " + "'"  + fromDate + "'" + " AND a.transaction_date <= " + "'" + thruDate + "'" +
			" AND b.orig_currency_uom_id = " + "'" + currency + "'" + " AND b.debit_credit_flag = " + "'" + flag + "'" ;
