...
if(UtilValidate.isEmpty(partyId)){
	String supplierQuery = getSupplierPartyIdQuery();
	sqlProcMain.prepareStatement(supplierQuery);
	ResultSet supplierResult = sqlProcMain.executeQuery();
	while (supplierResult.next()){
		supplierIds.add(supplierResult.getString(1));
	}
}


String mainQuery = buildQuery(supplierIds,partyId,fromDate,thruDate);
...
for(Map mainMap : mainList){
	...
	for(Map obMap : mainList){
		if( currencyKey.equals(obMap.get("currency")) && mapPartyId.equals(obMap.get("partyId")) ){
			...
			map.put("amount", debit.subtract(credit));
			map.put("currency", currencyKey);
			List itemDetailList = getItemDetail(obMap.get("partyId"),currencyKey,"D");
...
def getItemDetail(String partyId,String currency,String flag){
	SQLProcessor sqlProc1 = new SQLProcessor(delegator.getGroupHelperInfo("org.ofbiz"));
	String query = buildCreditOrDebitQuery(partyId,currency,flag);
	...
	while ( result.next() ){
		...
		if("VENDOR_PAYMENT".equals(result.getString(2)) || "VENDOR_PREPAY".equals(result.getString(2)) || "PURC_RTN_INVOICE".equals(result.getString(2)) 
			|| "TAX_PAYMENT".equals(result.getString(2)) || "SALES_TAX_PAYMENT".equals(result.getString(2)) || "PAYROLL_TAX_PAYMENT".equals(result.getString(2))
			|| "PAYROL_PAYMENT".equals(result.getString(2)) || "PAY_CHECK".equals(result.getString(2)) || "INCOME_TAX_PAYMENT".equals(result.getString(2)) 
			|| "COMMISSION_PAYMENT".equals(result.getString(2))  || "CUSTOMER_REFUND".equals(result.getString(2)) )
...
def buildCreditOrDebitQuery(String partyId,String currency,String flag){
    ...
    " JOIN Invoice c ON b.invoice_id=c.invoice_id WHERE a.party_id=" + "'" + partyId + "'" + 
	" AND IF(c.invoice_type_id='PURC_RTN_INVOICE',debit_credit_flag='D',debit_credit_flag='C')" +
    ...
    " JOIN acctg_trans B ON a.acctg_trans_id=b.acctg_trans_id " +
    " JOIN payment c ON b.payment_id=c.payment_id WHERE b.party_id=" + "'" + partyId + "'" + " AND debit_credit_flag='D'"+
    " AND b.transaction_date >= " + "'" + fromDate + "'" + " AND b.transaction_date <= " + "'" + thruDate + "'" + "AND a.orig_currency_uom_id = " + "'" + currency + "'" +
...
def buildQuery(List<String> supplierIds,String partyId,String fromDate,String thruDate){
	String query = "SELECT a.party_id, b.orig_currency_uom_id FROM acctg_trans AS a JOIN acctg_trans_entry AS b ON a.acctg_trans_id = b.acctg_trans_id WHERE "
				   ...
				   if(UtilValidate.isNotEmpty(supplierIds)){
					   String sb = "(";
					   int i = 1;
					   for(String custId : supplierIds){
						  sb = sb + "'" + custId.replace("'", "''") + "'";
						  if(i != supplierIds.size())
							  sb = sb + ",";
						  i++;
					   }
					   sb = sb + ")";
...
def getSupplierPartyIdQuery(){
	return "SELECT party_id FROM party_role WHERE role_type_id='SUPPLIER'";
}