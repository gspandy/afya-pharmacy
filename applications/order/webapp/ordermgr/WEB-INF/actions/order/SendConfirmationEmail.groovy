...
if (orderHeader.productStoreId) {
    list = delegator.findByAnd("ProductStoreEmailSetting",[productStoreId : orderHeader.productStoreId, emailType : emailType]);
    if (UtilValidate.isNotEmpty(list)) {
        context.productStoreEmailSetting = list.get(0);
    }else{
		list = delegator.findByAnd("ProductStoreEmailSetting",[emailType : emailType]);
		if (UtilValidate.isNotEmpty(list)) {
			context.productStoreEmailSetting = list.get(0);
		}
	}
