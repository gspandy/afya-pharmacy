                        ...
                        <table cellspacing="0" class="basic-table hover-bar">
                          ...
                          <#list poList as orderHeaderAndRole>
                            ...
                            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                              <td><a href="<@ofbizUrl>orderview?orderId=${orderHeaderAndRole.orderId}</@ofbizUrl>" class='btn-link'>${orderHeaderAndRole.orderId}</a></td>
                              <td>
                                <div>
                                  <#assign partyId = "_NA_">
                              ...
                              <td width="1">&nbsp;&nbsp;</td>
                              <td align='right'>
                                <a href="<@ofbizUrl>OrderDeliveryScheduleInfo?orderId=${orderHeaderAndRole.orderId}</@ofbizUrl>" class='btn-link'>Schedule&nbsp;Delivery</a>
                              </td>
                            </tr>
                          ...
                          <#list partyTasks as task>
                            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                              <td>
                                ...
                                <a href="<@ofbizUrl>orderview?${orderStr}</@ofbizUrl>" class="btn-link">
                                  ${task.orderId}
                                </a>
                              </td>
                              <td>
                                <div>
                                  <#if task.customerPartyId?exists>
                                    <a href="${customerDetailLink}${task.customerPartyId}${externalKeyParam}" target="partymgr" class="btn-link">${Static["org.ofbiz.order.task.TaskWorker"].getCustomerName(task)}</a>
                                  <#else>
                                    N/A
                                  </#if>
                                </div>
                              ...
                              <td>
                                <a href="/workeffort/control/activity?workEffortId=${task.workEffortId}${externalKeyParam}" target="workeffort" class="btn-link">
                        ...
                        <table cellspacing="0" class="basic-table hover-bar">
                          ...
                          <#list roleTasks as task>
                            <form method="get" name="F${task.workEffortId}">
                              ...
                              <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                                <td>
                                  <a href="javascript:viewOrder(document.F${task.workEffortId});" class="btn-link">
                                    ${task.orderId}
                                  </a>
                                </td>
                                <td>
                                ...
                                <td>
                                  <#if task.wepaPartyId == "_NA_">
                                    <div>N/A</div>
                                  <#else>
                                    <a href="${customerDetailLink}${task.wepaPartyId}${externalKeyParam}" target="partymgr" class="btn-link">${task.wepaPartyId}</a>
                                  </#if>
                                </td>
                                <td>${Static["org.ofbiz.order.task.TaskWorker"].getRoleDescription(task)}</td>
                                <td>${task.priority?default("0")}</td>
                                <td>
                                  <a href="/workeffort/control/activity?workEffortId=${task.workEffortId}" target="workeffort" class="btn-link">
                                    ${Static["org.ofbiz.order.task.TaskWorker"].getPrettyStatus(task)}
                                  </a>
                                </td>
                                <#if task.statusId?exists && task.statusId == "CAL_SENT">
