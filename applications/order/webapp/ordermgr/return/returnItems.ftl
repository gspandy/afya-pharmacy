...
<#macro displayReturnAdjustment returnAdjustment adjEditable>
    ...
    <tr>
       ...
       <#if (adjEditable)>
         <td align='right'><a href='javascript:document.removeReturnAdjustment_${rowCountForAdjRemove}.submit()' class='btn btn-mini btn-danger'>${uiLabelMap.CommonRemove}</a></td>
       <#else>
       <td>&nbsp;</td>
       </#if>
        <#if (adjEditable)>
    ...
    <#if returnHeader?has_content>
      <#if returnHeader.destinationFacilityId?has_content && returnHeader.statusId == "RETURN_ACCEPTED" && returnHeader.returnHeaderTypeId == "CUSTOMER_RETURN">
        <#list returnShipmentIds as returnShipmentId>
          <a href="/facility/control/ViewShipment?shipmentId=${returnShipmentId.shipmentId}${externalKeyParam}" class="btn">${uiLabelMap.ProductShipmentId} ${returnShipmentId.shipmentId}</a>
          <a href="/facility/control/ReceiveReturn?facilityId=${returnHeader.destinationFacilityId}&amp;returnId=${returnHeader.returnId?if_exists}&amp;shipmentId=${returnShipmentId.shipmentId}${externalKeyParam}&amp;externalLoginKey=${externalLoginKey}" class="btn">${uiLabelMap.OrderReceiveReturn}</a>
        </#list>
      <#elseif returnHeader.statusId == "SUP_RETURN_ACCEPTED" && returnHeader.returnHeaderTypeId == "VENDOR_RETURN">
         <#if returnShipmentIds?has_content>
           <#list returnShipmentIds as returnShipmentId>
             <a href="/facility/control/ViewShipment?shipmentId=${returnShipmentId.shipmentId}${externalKeyParam}" class="btn-link">${uiLabelMap.ProductShipmentId} ${returnShipmentId.shipmentId}</a>
           </#list>
         <#else>
            <a href="/facility/control/EditShipment?primaryReturnId=${returnHeader.returnId}&amp;partyIdTo=${returnHeader.toPartyId}&amp;partyIdFrom=${returnHeader.fromPartyId}&amp;statusId=SHIPMENT_INPUT&amp;shipmentTypeId=PURCHASE_RETURN&amp;externalLoginKey=${externalLoginKey}" class="btn btn-success">${uiLabelMap.OrderCreateReturnShipment}</a>
         </#if>
      </#if>
    </#if>
...
<#if !requestParameters.orderId?exists>
        ...
        <table cellspacing="0" class="basic-table">
          ...
          <tr class="header-row">
            ...
            <td>${uiLabelMap.FormFieldTitle_actionEnumId}</td>
          </tr>
          ...
          <#if returnItems?has_content>
            ...
            <#list returnItems as item>
              ...
              <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                <td><a href="<@ofbizUrl>orderview?orderId=${item.orderId}</@ofbizUrl>" class="btn-link">${item.orderId}</a> - ${item.orderItemSeqId?default("N/A")}
                ...
                <td><div>
                    <#if item.get("productId")?exists>
                        <a href="/catalog/control/EditProductInventoryItems?productId=${item.productId}&amp;externalLoginKey=${externalLoginKey}" class="btn-link">${item.productId}</a>
                    <#else>
                        N/A
                    </#if></div></td>
                <td><div>
                ...
                <td><div>
                    ...
                    <br />${uiLabelMap.OrderTotalQuantityReceive}: ${item.receivedQuantity}
                        <#list shipmentReceipts?if_exists as shipmentReceipt>
                            <br />${uiLabelMap.OrderQty}: ${shipmentReceipt.quantityAccepted}, ${shipmentReceipt.datetimeReceived}, <a href="/facility/control/EditInventoryItem?inventoryItemId=${shipmentReceipt.inventoryItemId}" class="btn-link">${shipmentReceipt.inventoryItemId}</a>
                        </#list>
                    </#if>
                    </div></td>
                <td><div>
                  ...
                  <#if returnHeader.statusId == "RETURN_COMPLETED" || returnHeader.statusId == "SUP_RETURN_COMPLETED">
                    ...
                    <#if itemResp?has_content>
                      <#if itemResp.paymentId?has_content>
                        <div>${uiLabelMap.AccountingPayment} #<a href="/accounting/control/paymentOverview?paymentId=${itemResp.paymentId}${externalKeyParam}" class="btn-link">${itemResp.paymentId}</a></div>
                      <#elseif itemResp.replacementOrderId?has_content>
                        <div>${uiLabelMap.OrderOrder} #<a href="<@ofbizUrl>orderview?orderId=${itemResp.replacementOrderId}</@ofbizUrl>" class="btn-link">${itemResp.replacementOrderId}</a></div>
                      <#elseif itemResp.billingAccountId?has_content>
                        <div>${uiLabelMap.AccountingAccountId} #<a href="/accounting/control/EditBillingAccount?billingAccountId=${itemResp.billingAccountId}${externalKeyParam}" class="btn-link">${itemResp.billingAccountId}</a></div>
                      </#if>
                    <#else>
                ...
                <#if returnHeader.statusId == "RETURN_REQUESTED" || returnHeader.statusId == "SUP_RETURN_REQUESTED">
                  <td align='right'><a href='javascript:document.removeReturnItem_${item_index}.submit()' class='btn btn-mini btn-danger'>${uiLabelMap.CommonRemove}</a></td>
                <#else>
                  <td>&nbsp;</td>
                </#if>
              </tr>
        ...
        <#if (returnAdjustments?has_content)>
            ...
            <#if (!readOnly) && (rowCount > 0)>
               <tr>
                  ...
                  <input type="submit" class="btn btn-success" value="${uiLabelMap.CommonUpdate}" /></td>
              </tr>
           </#if>
           <tr><td colspan="10"><hr/></td></tr>
        </table>
        ...
        <form name="acceptReturn" method="post" action="<@ofbizUrl>/updateReturn</@ofbizUrl>">
          ...
          <div align="right"><input type="submit" value="${uiLabelMap.OrderReturnAccept}" class="btn" style="font-weight: bold;"/></div>
        </form>
        </#if>
        ...
        <form name="returnItems" method="post" action="<@ofbizUrl>returnItems</@ofbizUrl>">
          ...
          <table border='0' cellpadding='2' cellspacing='0'>
            ...
            <#else>
              <tr>
                <td colspan="4" nowrap="nowrap"><div>${uiLabelMap.OrderNoOrderFoundForParty}: <a href="${customerDetailLink}${partyId?default('_NA_')}" class="btn btn-mini btn-danger">${partyId?default('[null]')}</a></div></td>
              </tr>
              <tr>
            ...
            <tr>
              <td colspan="2">&nbsp;</td>
              <td colspan="2">
                <a href="javascript:document.returnItems.submit();" class="btn">${uiLabelMap.OrderReturnLoadItems}</a>
              </td>
            </tr>
          </table>
        </form>
