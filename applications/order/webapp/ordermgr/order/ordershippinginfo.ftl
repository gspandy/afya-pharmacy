  ...
  <div class="screenlet">
    <div class="screenlet-title-bar">
       <ul>
         ...
         <li class="collapsed"><a onclick="javascript:toggleScreenlet(this, 'ShipGroupScreenletBody_${shipGroup.shipGroupSeqId}', 'true', '${uiLabelMap.CommonExpand}', '${uiLabelMap.CommonCollapse}');" title="Expand">&nbsp;</a></li>
    ...
    <div class="screenlet-body" id="ShipGroupScreenletBody_${shipGroup.shipGroupSeqId}" style="display: none;">
           ...
           <!--     <#if orderHeader.orderTypeId == "SALES_ORDER" && shipGroup.shipmentMethodTypeId?has_content>
            ...
            <!--    <#if orderHeader?has_content && orderHeader.statusId != "ORDER_CANCELLED" && orderHeader.statusId != "ORDER_COMPLETED" && orderHeader.statusId != "ORDER_REJECTED">
                <tr>
                    ...
                    <td width="5">&nbsp;</td>
                    <td valign="top" width="80%">
                        <input type="submit" value="${uiLabelMap.CommonUpdate}" class="btn btn-success"/>
                         <a class="btn" id="newShippingAddress" href="javascript:void(0);">${uiLabelMap.OrderNewShippingAddress}</a> 
                    </td>
                </tr>
                </#if>  -->
      ...
      <div id="newShippingAddressForm" class="popup" style="display: none;">
        <form id="addShippingAddress" name="addShippingAddress" method="post" action="addShippingAddress">
          ...
          <div class="form-row">
            ...
            <div class="form-field">
              <select name="shipToStateProvinceGeoId" id="state" class="required" style="width: 70%">
                <#if stateProvinceGeoId?exists>
          ...
          <div class="form-row">
            ...
            <form action="">
              <input class="popup_closebox btn" type="button" value="${uiLabelMap.CommonClose}"/>
            </form>
          </div>
        </form>
      </div>
                    ...
                    <#if security.hasEntityPermission("ORDERMGR", "_UPDATE", session)>
                      <#if orderHeader.statusId != "ORDER_COMPLETED" && orderHeader.statusId != "ORDER_CANCELLED">
                        <form name="allowordersplit_${shipGroup.shipGroupSeqId}" method="post" action="<@ofbizUrl>allowordersplit</@ofbizUrl>">
                          ...
                          <#if orderHeader.orderTypeId == "SALES_ORDER" && productStore?has_content && productStore.reserveInventory == "N">
                              <a href="javascript:document.allowordersplit_${shipGroup.shipGroupSeqId}.submit()" class="btn btn-mini">${uiLabelMap.OrderAllowSplit}</a>
                          </#if>
        ...
        <tr>
          ...
          <td align="left" valign="top" width="80%">
            <#if (!orderHeader.statusId.equals("ORDER_COMPLETED")) && !(orderHeader.statusId.equals("ORDER_REJECTED")) && !(orderHeader.statusId.equals("ORDER_CANCELLED"))>
              <form id="updateShippingInstructionsForm_${shipGroup.shipGroupSeqId}" method="post" name="updateShippingInstructionsForm"  action="<@ofbizUrl>setShippingInstructions</@ofbizUrl>">
                ...
                <#if shipGroup.shippingInstructions?has_content>
                  <table>
                    <tr>
                      ...
                      <td>
                        <a href="javascript:editInstruction('${shipGroup.shipGroupSeqId}');" class="btn btn-mini" id="editInstruction_${shipGroup.shipGroupSeqId}">${uiLabelMap.CommonEdit}</a>
                      </td>
                    </tr>
                  </table>
                <#else>
                  <a href="javascript:addInstruction('${shipGroup.shipGroupSeqId}');" class="btn btn-mini" id="addInstruction_${shipGroup.shipGroupSeqId}">${uiLabelMap.CommonAdd}</a>
                </#if>
                <textarea name="shippingInstructions" id="shippingInstructions_${shipGroup.shipGroupSeqId}" style="display:none; width: 260px; height: 60px;" rows="3" cols="10">${shipGroup.shippingInstructions?if_exists}</textarea>
                <a href="javascript:saveInstruction('${shipGroup.shipGroupSeqId}');" class="btn btn-success" id="saveInstruction_${shipGroup.shipGroupSeqId}" style="display:none">${uiLabelMap.CommonSave}</a>
              </form>
            <#else>
              <#if shipGroup.shippingInstructions?has_content>
          ...
          <td>
            <form name="setGiftMessageForm" method="post" action="<@ofbizUrl>setGiftMessage</@ofbizUrl>">
              ...
              <#if shipGroup.giftMessage?has_content>
                ...
                <a href="javascript:editGiftMessage();" class="btn" id="editGiftMessage">${uiLabelMap.CommonEdit}</a>
              <#else>
                <a href="javascript:addGiftMessage();" class="btn btn-success" id="addGiftMessage">${uiLabelMap.CommonAdd}</a>
              </#if>
              <textarea name="giftMessage" id="giftMessage" style="display:none; width: 260px; height: 60px;" rows="0" cols="0">${shipGroup.giftMessage?if_exists}</textarea>
              <a href="javascript:saveGiftMessage();" class="btn btn-success" id="saveGiftMessage" style="display:none">${uiLabelMap.CommonSave}</a>
            </form>
          </td>
        </tr>
        </#if> -->
         ...
         <tr>
            ...
            <td valign="top" width="80%">
              <form name="setShipGroupDates_${shipGroup.shipGroupSeqId}" method="post" action="<@ofbizUrl>updateOrderItemShipGroup</@ofbizUrl>">
                <input type="hidden" name="orderId" value="${orderHeader.orderId}"/>
                <input type="hidden" name="shipGroupSeqId" value="${shipGroup.shipGroupSeqId}"/>
                 <@htmlTemplate.renderDateTimeField name="shipAfterDate" value="${shipGroup.shipAfterDate!''}" className="" alert="" 
                title="Format: yyyy-MM-dd HH:mm:ss.SSS" size="25" maxlength="30" id="shipAfterDate" dateType="date" shortDateInput=false 
                timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" 
                hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName="setShipGroupDates_${shipGroup.shipGroupSeqId}"/>
                <br/>
                <@htmlTemplate.renderDateTimeField name="shipByDate" value="${shipGroup.shipByDate!''}" className="" alert="" 
                title="Format: yyyy-MM-dd HH:mm:ss.SSS" size="25" maxlength="30" id="shipByDate" dateType="date" shortDateInput=false 
                timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" 
                hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName="setShipGroupDates_${shipGroup.shipGroupSeqId}"/>
                <br/>
                <input type="submit" class="btn btn-mini" value="${uiLabelMap.CommonUpdate}"/>
                </form>
            </td>
         </tr>
          ...
          <tr>
            ...
            <td valign="top" width="80%">
                <#list shipGroupShipments as shipment>
                    <div>
                      ${uiLabelMap.CommonNbr}<a href="/facility/control/ViewShipment?shipmentId=${shipment.shipmentId}&amp;externalLoginKey=${externalLoginKey}" class="btn-link">${shipment.shipmentId}</a>&nbsp;&nbsp;
                      <#if "SALES_ORDER" == orderHeader.orderTypeId && "SHIPMENT_PACKED" == shipment.statusId || "SHIPMENT_SHIPPED"  == shipment.statusId || "SHIPMENT_DELIVERED" == shipment.statusId>
                        <a href="/facility/control/PackingSlip.pdf?shipmentId=${shipment.shipmentId}&amp;externalLoginKey=${externalLoginKey}" target="_BLANK" class="btn">Delivery Note</a>
                      </#if>
                      <#if "SALES_ORDER" == orderHeader.orderTypeId && "ORDER_COMPLETED" == orderHeader.statusId>
                        ...
                        <#if shipmentRouteSegments?has_content>
                          ...
                          <#if "UPS" == (shipmentRouteSegment.carrierPartyId)?if_exists>
                            <a href="javascript:document.upsEmailReturnLabel${shipment_index}.submit();" class="btn">${uiLabelMap.ProductEmailReturnShippingLabelUPS}</a>
                          </#if>
            ...
            <td colspan="3" valign="top" width="100%" align="center">
             <#if orderHeader.orderTypeId == "SALES_ORDER">
               <#if !shipGroup.supplierPartyId?has_content>
                 <#--<#if orderHeader.statusId == "ORDER_APPROVED">
                   <a href="/facility/control/PackOrder?facilityId=${storeFacilityId?if_exists}&amp;orderId=${orderId}&amp;shipGroupSeqId=${shipGroup.shipGroupSeqId}&amp;externalLoginKey=${externalLoginKey}" class="btn btn-mini">${uiLabelMap.OrderPackShipmentForShipGroup}</a>
                 </#if>-->
                 <#if (orderHeader.statusId == "ORDER_APPROVED") || (orderHeader.statusId == "ORDER_SENT")>
                     <#if productStore?has_content && productStore.reserveInventory == "N">
                         <a href="javascript:document.createShipment_${shipGroup.shipGroupSeqId}.submit()" class="btn btn-mini">${uiLabelMap.OrderNewShipmentForShipGroup}</a>
                     </#if>
                 </#if>
                 <form name="createShipment_${shipGroup.shipGroupSeqId}" method="post" action="/facility/control/createShipment?externalLoginKey=${externalLoginKey}">
                   ...
                   <input type="hidden" name="partyIdFrom" value="${companyPartyId?if_exists}"/>
             ...
             <#else>
               ...
               <#if facilities?has_content>
                  <div>
                    <form name="createShipment2_${shipGroup.shipGroupSeqId}" method="post" action="/facility/control/createShipment?externalLoginKey=${externalLoginKey}">
                       <input type="hidden" name="primaryOrderId" value="${orderId}"/>
                       <input type="hidden" name="primaryShipGroupSeqId" value="${shipGroup.shipGroupSeqId}"/>
                       <input type="hidden" name="shipmentTypeId" value="PURCHASE_SHIPMENT"/>
                       <input type="hidden" name="statusId" value="PURCH_SHIP_CREATED"/>
                        <input type="hidden" name="partyIdTo" value="${companyPartyId?if_exists}"/>
                       ...
                       <#if (orderHeader.statusId == "ORDER_APPROVED") || (orderHeader.statusId == "ORDER_SENT")>
                         <input type="submit" class="btn btn-mini" value="${uiLabelMap.OrderNewShipmentForShipGroup} [${shipGroup.shipGroupSeqId}]"/>
                       </#if>
                    </form>
                  </div>
               <#else>
                   <a href="javascript:document.quickDropShipOrder_${shipGroup_index}.submit();" class="btn btn-mini">${uiLabelMap.ProductShipmentQuickComplete}</a>
                   <a href="javascript:document.createShipment3_${shipGroup.shipGroupSeqId}.submit();" class="btn btn-mini">${uiLabelMap.OrderNewDropShipmentForShipGroup} [${shipGroup.shipGroupSeqId}]</a>
                   <form name="quickDropShipOrder_${shipGroup_index}" method="post" action="<@ofbizUrl>quickDropShipOrder</@ofbizUrl>">
                        <input type="hidden" name="orderId" value="${orderId}"/>
                        <input type="hidden" name="shipGroupSeqId" value="${shipGroup.shipGroupSeqId}"/>
                        <input type="hidden" name="externalLoginKey" value="${externalLoginKey}" />
                    </form>
                    <form name="createShipment3_${shipGroup.shipGroupSeqId}" method="post" action="/facility/control/createShipment?externalLoginKey=${externalLoginKey}">
                        <input type="hidden" name="primaryOrderId" value="${orderId}"/>
                        <input type="hidden" name="primaryShipGroupSeqId" value="${shipGroup.shipGroupSeqId}"/>
