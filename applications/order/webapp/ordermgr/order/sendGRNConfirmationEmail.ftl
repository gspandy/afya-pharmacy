...
<div class="screenlet">
    ...
    <div class="screenlet-body">
      <table class="basic-table" cellspacing='0'>
        <tr>
          <#if parameters.shipmentId?has_content>
            <td width="6%">
              <form name="sendGrnEmailTopForm" method="post" action="<@ofbizUrl>ViewShipment?shipmentId=${parameters.shipmentId}</@ofbizUrl>">
                <input type="hidden" name="facilityId" value="${parameters.facilityId?if_exists}"/>
                <input type="hidden" name="initialSelected" value="Y"/>
                <a href="javascript:document.sendGrnEmailTopForm.submit();" class="buttontext">${uiLabelMap.CommonGoBack}</a>
              </form>
            </td>
      	  <#else>
      	    <td width="6%">
      ...
      <br />
      <form method="post" action="<@ofbizUrl>sendGrnConfirmationEmail?orderId=${parameters.orderId?if_exists}&amp;shipmentId=${parameters.shipmentId?if_exists}&amp;facilityId=${parameters.facilityId?if_exists}</@ofbizUrl>" name="sendConfirmationForm">
      ...
      <table class="basic-table" cellspacing='0'>
        <tr>
          <#if parameters.shipmentId?has_content>
            <td width="6%">
              <form name="sendGrnEmailBottomForm" method="post" action="<@ofbizUrl>ViewShipment?shipmentId=${parameters.shipmentId}</@ofbizUrl>">
                <input type="hidden" name="facilityId" value="${parameters.facilityId?if_exists}"/>
                <input type="hidden" name="initialSelected" value="Y"/>
                <a href="javascript:document.sendGrnEmailBottomForm.submit();" class="buttontext">${uiLabelMap.CommonGoBack}</a>
              </form>
            </td>
      	  <#else>
      	    <td width="6%">
