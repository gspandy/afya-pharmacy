...
<div class="screenlet">
  ...
  <table>
    <thead>
    <tr>
      <th width="50%">${uiLabelMap.OrderProduct}</th>
    ...
    <tfoot>
      <tr>
        <th colspan="7">Items Sub Total</th>
        <#assign orderSubTotalExcludeTax = orderSubTotal - salesTax.amount?default(0)>
        <td><@ofbizCurrency amount=orderSubTotalExcludeTax isoCode=currencyUomId/></td>
      ...
      <#if orderAdjustments?has_content>
        <tr>
          <#assign orderAdjustment = orderAdjustments.get(0)>
          <th colspan="7">Total ${salesTax.name?if_exists}</th>
          <td><@ofbizCurrency amount=salesTax.amount isoCode=currencyUomId/></td>
          <#if maySelectItems?default("N") == "Y"><td colspan="3"></td></#if>
        </tr>
      </#if>
      <#list orderAdjustments as orderAdjustment>
        <#assign salesTax = "SALES_TAX"/>
        <#if salesTax==orderAdjustment.orderAdjustmentTypeId>
        <#else>
          <tr>
            <th colspan="7">${orderAdjustment.comments}</th>
            <td><@ofbizCurrency amount=orderAdjustment.amount isoCode=currencyUomId/></td>
            <#if maySelectItems?default("N") == "Y"><td colspan="3"></td></#if>
          </tr>
        </#if>
      </#list>
        ...
        <#if !orderItem.productId?exists || orderItem.productId == "_?_">
          <td colspan="1" width="50%">
            ...
            <#if orderItem.comments?has_content>
              <br/><br/>${orderItem.comments?upper_case}
            </#if>
          </td>
        <#else>
    ...
    <#-- show the order/payment term details -->
      <#if orderTerms?has_content>
        <tr> <td colspan="7"> &nbsp; &nbsp; &nbsp; </td> </tr>
        <tr>
          <#list orderTerms as orderTerm>
            <td>
              <#if (orderTerm.termTypeId == "FIN_PAYMENT_TERM" || orderTerm.termTypeId == "FIN_PAY_NETDAYS_1" || orderTerm.termTypeId == "FIN_PAY_NETDAYS_2" || orderTerm.termTypeId == "FIN_PAY_NETDAYS_3") && orderTerm.termDays?has_content>
                Payment Terms: ${orderTerm.termDays?if_exists} Days
              <#else>
                Payment Terms: ${orderTerm.termValue?if_exists}
              </#if>
              <#if orderTerm.textValue?has_content>
                - ${orderTerm.textValue?if_exists}
              </#if>
            </td>
          </#list>
        </tr>
