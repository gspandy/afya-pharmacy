...
public class LoginEvents {
    ...
    public static String saveEntryParams(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (userLogin == null) {
            ...
            if ((username != null)
                    && ("true".equalsIgnoreCase(UtilProperties.getPropertyValue("security.properties", "username.lowercase")))) {
                username = username.toLowerCase();
            }
            if ((password != null)
                    && ("true".equalsIgnoreCase(UtilProperties.getPropertyValue("security.properties", "password.lowercase")))) {
                password = password.toLowerCase();
            }

            // save parameters into the session - so they can be used later, if
            // needed
            if (username != null)
    ...
    /**
     * The user forgot his/her password. This will call showPasswordHint,
     * emailPassword or simply returns "success" in case no operation has been
     * specified.
     * 
     * @param request
    ...
    /**
     * Show the password hint for the userLoginId specified in the request
     * object.
     * 
     * @param request
     *            The HTTPRequest object for the current request
    ...
    public static String showPasswordHint(HttpServletRequest request, HttpServletResponse response) {
        Delegator delegator = getDefaultOrTenantDelegator(request, response, (Delegator) request.getAttribute("delegator"));
        if (delegator == null) {
            String errMsg = "Tenant Id incorrect.";
        ...
        if (!UtilValidate.isNotEmpty(passwordHint)) {
            // the Username was not found
            errMsg = UtilProperties.getMessage(resource, "loginevents.no_password_hint_specified_try_password_emailed",
    ...
    public static String emailPassword(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (supposedUserLogin == null) {
            ...
            errMsg = UtilProperties.getMessage(resource, "loginevents.user_with_the_username_not_found", messageMap,
                    UtilHttp.getLocale(request));
            request.setAttribute("_ERROR_MESSAGE_", errMsg);
            return "error";
        }
        ...
        if (party != null) {
            Iterator<GenericValue> emailIter = UtilMisc.toIterator(ContactHelper.getContactMechByPurpose(party, "PRIMARY_EMAIL",
                    false));
        ...
        if (!UtilValidate.isNotEmpty(emails.toString())) {
            // the Username was not found
            errMsg = UtilProperties.getMessage(resource, "loginevents.no_primary_email_address_set_contact_customer_service",
                    UtilHttp.getLocale(request));
            request.setAttribute("_ERROR_MESSAGE_", errMsg);
            return "error";
        }
        ...
        try {
            personGv = delegator.findByPrimaryKey("Person", UtilMisc.toMap("partyId", party.getString("partyId")));
        ...
        if (personGv != null) {
            bodyParameters.put("firstName", personGv.getString("firstName"));
            bodyParameters.put("lastName", personGv.getString("lastName"));
        }
        ...
        if (productStoreEmail != null) {
            ...
            serviceContext.put("subject", "Request For Password");
            serviceContext.put("sendFrom", "smesupport@nthdimenzion.com");
        }
        ...
        try {
            ...
            Debug.logWarning(e, "", module);
            errMsg = UtilProperties.getMessage(resource, "loginevents.error_unable_email_password_contact_customer_service",
                    UtilHttp.getLocale(request));
            request.setAttribute("_ERROR_MESSAGE_", errMsg);
            return "error";
        }
        ...
        if (useEncryption) {
            try {
                ...
                Debug.logWarning(e, "", module);
                Map<String, String> messageMap = UtilMisc.toMap("errorMessage", e.toString());
                errMsg = UtilProperties.getMessage(resource, "loginevents.error_saving_new_password_email_not_correct_password",
        ...
        if (useEncryption) {
            ...
            errMsg = UtilProperties
                    .getMessage(resource, "loginevents.new_password_sent_check_email", UtilHttp.getLocale(request));
            request.setAttribute("_EVENT_MESSAGE_", errMsg);
        }
        return "success";
    }
    ...
    public static Delegator getDefaultOrTenantDelegator(HttpServletRequest request, HttpServletResponse response,
        ...
        if (UtilValidate.isNotEmpty(tenantId)) {
            ...
            try {
                tenantGv = defaultDelegator.findByAnd("UserDefineTenant",
                        UtilMisc.toMap("userDefineTenantId", tenantId, "disabled", "N"));
            } catch (GenericEntityException e1) {
                e1.printStackTrace();
            }
            if (UtilValidate.isEmpty(tenantGv)) {
                try {
                    tenantGv = defaultDelegator.findByAnd("UserDefineTenant",
                            UtilMisc.toMap("systemDefineTenantId", tenantId, "disabled", "N"));
                } catch (GenericEntityException e) {
                    e.printStackTrace();
                }
            }
            if (UtilValidate.isEmpty(tenantGv)) {
                request.setAttribute("_ERROR_MESSAGE_",
                        "Following error occurred during login:Tenant ID  incorrect or disabled by Admin.");
                return null;
            }
        }
        ...
        if (UtilValidate.isNotEmpty(tenantId)) {
            String oldDelegatorName = defaultDelegator.getDelegatorName();
            ...
            if (delegatorNameHashIndex > 0) {
                ...
                if (currentDelegatorTenantId != null)
                    currentDelegatorTenantId.trim();
            }

            if (delegatorNameHashIndex == -1 || (currentDelegatorTenantId != null && !tenantId.equals(currentDelegatorTenantId))) {
                String delegatorName = defaultDelegator.getDelegatorName() + "#" + tenantId;
                defaultDelegator = DelegatorFactory.getDelegator(delegatorName);
