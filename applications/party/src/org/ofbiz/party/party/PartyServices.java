...
public class PartyServices {
	...
	public static Map<String, Object> createPerson(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		if (party != null) {
			...
			Map<String, Object> newPartyMap = UtilMisc.toMap("partyId", partyId, "partyTypeId", "PERSON", "description", description, "createdDate",
					now, "lastModifiedDate", now, "statusId", statusId);
	...
	public static Map<String, Object> setPartyStatus(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		try {
			...
			if (!party.getString("statusId").equals(statusId)) {
				...
				GenericValue statusValidChange = delegator.findByPrimaryKey("StatusValidChange",
						UtilMisc.toMap("statusId", party.getString("statusId"), "statusIdTo", statusId));
			...
			return ServiceUtil
					.returnError(UtilProperties.getMessage(resource, "person.update.write_failure", new Object[] { e.getMessage() }, locale));
		}
	}
	...
	public static Map<String, Object> updatePerson(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		try {
			...
			return ServiceUtil
					.returnError(UtilProperties.getMessage(resource, "person.update.read_failure", new Object[] { e.getMessage() }, locale));
		}
		...
		if (UtilValidate.isNotEmpty(context.get("statusId")) && !context.get("statusId").equals(oldStatusId)) {
			try {
				dispatcher.runSync("setPartyStatus",
						UtilMisc.toMap("partyId", partyId, "statusId", context.get("statusId"), "userLogin", context.get("userLogin")));
			} catch (GenericServiceException e) {
				Debug.logWarning(e.getMessage(), module);
				return ServiceUtil.returnError(UtilProperties.getMessage(resource, "person.update.write_failure", new Object[] { e.getMessage() },
						locale));
			}
		}
		...
		try {
			...
			if (party != null) {
				...
				if (!EntityTypeUtil.isType(partyType, partyGroupPartyType)) {
					errMsg = UtilProperties.getMessage(resource, "partyservices.cannot_create_party_group_already_exists_not_PARTY_GROUP_type",
							locale);
					return ServiceUtil.returnError(errMsg);
				}
			} else {
				...
				if (UtilValidate.isNotEmpty(context.get("partyTypeId"))) {
					GenericValue desiredPartyType = delegator.findByPrimaryKeyCache("PartyType",
							UtilMisc.toMap("partyTypeId", context.get("partyTypeId")));
					if (desiredPartyType != null && EntityTypeUtil.isType(desiredPartyType, partyGroupPartyType)) {
						...
						return ServiceUtil.returnError("The specified partyTypeId [" + context.get("partyTypeId")
	...
	public static Map<String, Object> createPartyNote(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		if (noteId == null) {
			...
			try {
				noteRes = dispatcher.runSync("createNote", UtilMisc.toMap("partyId", userLogin.getString("partyId"), "note", noteString, "userLogin",
						userLogin, "locale", locale, "noteName", noteName));
			} catch (GenericServiceException e) {
				Debug.logError(e, e.getMessage(), module);
	...
	public static Map<String, Object> getPartyFromExactEmail(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			EntityExpr ee = EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("infoString"), EntityOperator.EQUALS,
					EntityFunction.UPPER(email.toUpperCase()));
			List<GenericValue> c = EntityUtil.filterByDate(
					delegator.findList("PartyAndContactMech", ee, null, UtilMisc.toList("infoString"), null, false), true);
	...
	public static Map<String, Object> getPartyFromEmail(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			EntityExpr ee = EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("infoString"), EntityOperator.LIKE,
					EntityFunction.UPPER(("%" + email.toUpperCase()) + "%"));
			List<GenericValue> c = EntityUtil.filterByDate(
					delegator.findList("PartyAndContactMech", ee, null, UtilMisc.toList("infoString"), null, false), true);
	...
	public static Map<String, Object> getPartyFromUserLogin(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			EntityExpr ee = EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("userLoginId"), EntityOperator.LIKE,
	...
	public static Map<String, Object> getPartyFromPartyGroup(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			EntityExpr ee = EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("groupName"), EntityOperator.LIKE,
	...
	public static Map<String, Object> createPartyDataSource(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		try {
			...
			if (party == null || dataSource == null) {
				...
				if (party == null)
					errorList.add("party with ID [" + partyId + "] was not found ");
				if (dataSource == null)
					errorList.add("data source with ID [" + dataSourceId + "] was not found ");
		...
		if ("Y".equals(lookupFlag)) {
			...
			if (UtilValidate.isNotEmpty(partyRelationshipTypeId)) {
				...
				if (UtilValidate.isEmpty(ownerPartyIds)) {
					...
					relationshipCond = EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyIdFrom"), EntityOperator.EQUALS,
			...
			if (!"Y".equals(showAll)) {
				...
				if (partyTypeId != null && !"ANY".equals(partyTypeId)) {
					...
					andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyTypeId"), EntityOperator.LIKE,
				...
				if ("T".equals(extInfo)) {
					...
					if (UtilValidate.isNotEmpty(countryCode)) {
						...
						andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("countryCode"), EntityOperator.EQUALS,
					...
					if (UtilValidate.isNotEmpty(areaCode)) {
						...
						andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("areaCode"), EntityOperator.EQUALS,
					...
					if (UtilValidate.isNotEmpty(contactNumber)) {
						...
						andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("contactNumber"), EntityOperator.EQUALS,
	...
	public static Map<String, Object> updatePersonAndGlAccount(DispatchContext ctx, Map<String, ? extends Object> context) {
		Map<String, Object> result = FastMap.newInstance();
		Delegator delegator = ctx.getDelegator();
		LocalDispatcher dispatcher = ctx.getDispatcher();
		Locale locale = (Locale) context.get("locale");
		String glAccountId = null;
		String glAccountTypeId = null;
		String partyId = getPartyId(context);
		String groupName=(String) context.get("groupName");
		if (UtilValidate.isEmpty(partyId)) {
			return ServiceUtil.returnError(UtilProperties.getMessage(ServiceUtil.resource, "serviceUtil.party_id_missing", locale));
		...
		try {
			List<GenericValue> partyGlAccontList = delegator
					.findByAnd("PartyGlAccount", UtilMisc.toMap("partyId", partyId));
			for (GenericValue partyGv : partyGlAccontList) {

				glAccountId = partyGv.getString("glAccountId");
				glAccountTypeId=partyGv.getString("glAccountTypeId");

				GenericValue glAccountGv = delegator.findByPrimaryKey("GlAccount", UtilMisc.toMap("glAccountId", glAccountId));
				String parentGlAccountId = glAccountGv.getString("parentGlAccountId");
				if (UtilValidate.areEqual("4160000", parentGlAccountId)) {

					GenericValue gv = delegator.makeValue("GlAccount",
							UtilMisc.toMap("glAccountId", glAccountId, "accountName", groupName, "parentGlAccountId", parentGlAccountId,"glAccountTypeId",glAccountTypeId));
					delegator.store(gv);
				}

				if (UtilValidate.areEqual("3500000", parentGlAccountId)) {
