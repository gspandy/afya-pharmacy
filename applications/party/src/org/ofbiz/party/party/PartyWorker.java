...
public class PartyWorker {
	...
	public static Map<String, GenericValue> getPartyOtherValues(ServletRequest request, String partyId, String partyAttr, String personAttr,
	...
	/**
	 * Generate a sequenced club id using the prefix passed and a sequence value
	 * + check digit
	 * 
	 * @param delegator
	 *            used to obtain a sequenced value
	 * @param prefix
	 *            prefix inserted at the beginning of the ID
	 * @param length
	 *            total length of the ID including prefix and check digit
	 * @return Sequenced Club ID string with a length as defined starting with
	 *         the prefix defined
	 */
	...
	public static GenericValue findPartyLatestContactMech(String partyId, String contactMechTypeId, Delegator delegator) {
		try {
			List<GenericValue> cmList = delegator.findByAnd("PartyAndContactMech",
					UtilMisc.toMap("partyId", partyId, "contactMechTypeId", contactMechTypeId), UtilMisc.toList("-fromDate"));
			cmList = EntityUtil.filterByDate(cmList);
			return EntityUtil.getFirst(cmList);
		} catch (GenericEntityException e) {
			Debug.logError(e,
					"Error while finding latest ContactMech for party with ID [" + partyId + "] TYPE [" + contactMechTypeId + "]: " + e.toString(),
					module);
			return null;
		}
	}
	...
	public static Timestamp findPartyLastLoginTime(String partyId, Delegator delegator) {
		try {
			List<GenericValue> loginHistory = delegator.findByAnd("UserLoginHistory", UtilMisc.toMap("partyId", partyId),
	...
	public static String findFirstMatchingPartyId(Delegator delegator, String address1, String address2, String city, String stateProvinceGeoId,
			String postalCode, String postalCodeExt, String countryGeoId, String firstName, String middleName, String lastName)
			throws GeneralException {

		String[] info = findFirstMatchingPartyAndContactMechId(delegator, address1, address2, city, stateProvinceGeoId, postalCode, postalCodeExt,
				countryGeoId, firstName, middleName, lastName);
		if (info != null) {
			return info[0];
		}
		return null;
	...
	public static String[] findFirstMatchingPartyAndContactMechId(Delegator delegator, String address1, String address2, String city,
			String stateProvinceGeoId, String postalCode, String postalCodeExt, String countryGeoId, String firstName, String middleName,
			String lastName) throws GeneralException {
	...
	public static List<GenericValue> findMatchingPartyAndPostalAddress(Delegator delegator, String address1, String address2, String city,
		...
		addrExprs.add(EntityCondition.makeCondition(EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, null), EntityOperator.OR,
		...
		// Debug.log("Checking for matching address: " + addrCond.toString() +
	...
	public static List<String> getAssociatedPartyIdsByRelationshipType(Delegator delegator, String partyIdFrom, String partyRelationshipTypeId) {
		...
		try {
			EntityConditionList<EntityExpr> baseExprs = EntityCondition.makeCondition(
					UtilMisc.toList(EntityCondition.makeCondition("partyIdFrom", partyIdFrom),
			...
			while (UtilValidate.isNotEmpty(associatedParties)) {
				...
				for (GenericValue associatedParty : associatedParties) {
					EntityConditionList<EntityExpr> innerExprs = EntityCondition.makeCondition(
							UtilMisc.toList(EntityCondition.makeCondition("partyIdFrom", associatedParty.get("partyIdTo")),
									EntityCondition.makeCondition("partyRelationshipTypeId", partyRelationshipTypeId)), EntityOperator.AND);
	...
	/**
	 * Generic service to find party by id. By default return the party find by
	 * partyId but you can pass searchPartyFirst at false if you want search in
	 * partyIdentification before or pass searchAllId at true to find apartyuct
	 * with this id (party.partyId and partyIdentification.idValue)
	 * 
	 * @param delegator
	 * @param idToFind
	...
	public static List<GenericValue> findPartiesById(Delegator delegator, String idToFind, String partyIdentificationTypeId,
		...
		if (Debug.verboseOn())
			Debug.logVerbose("Analyze partyIdentification: entered id = " + idToFind + ", partyIdentificationTypeId = " + partyIdentificationTypeId,
					module);
	...
	public static List<GenericValue> findParties(Delegator delegator, String idToFind, String partyIdentificationTypeId)
		...
		if (UtilValidate.isNotEmpty(partiesByIds)) {
			for (GenericValue party : partiesByIds) {
				...
				// retreive party GV if the actual genericValue came from
				// viewEntity
	...
	public static String UpdateMenuSelection(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			String userLoginMenuId = delegator.getNextSeqId("UserLoginMenuAssoc");
			String userLoginId = request.getParameter("userLoginId" + suffix);
			String isVisible = request.getParameter("displayNameIsVisible" + suffix) == null ? "N" : request.getParameter("displayNameIsVisible"
					+ suffix);
			String menuId = request.getParameter("menuId" + suffix);
			Timestamp nowDate = UtilDateTime.nowTimestamp();
			if ("N".equals(isVisible)) {
				GenericValue gv = delegator.makeValidValue(
						"UserLoginMenuAssoc",
						UtilMisc.toMap("userLoginMenuId", userLoginMenuId, "userLoginId", userLoginId, "menuId", menuId, "fromDate",
								"N".equals(isVisible) ? nowDate : null, "thruDate", "Y".equals(isVisible) ? nowDate : null));
				delegator.createOrStore(gv,true);
			}
			if ("Y".equals(isVisible) && UtilValidate.isNotEmpty(menuId)) {
				List<EntityCondition> conditionList = new ArrayList<EntityCondition>();
				conditionList.add(EntityCondition.makeCondition("menuId", EntityOperator.EQUALS, menuId));
				conditionList.add(EntityCondition.makeCondition("userLoginId", EntityOperator.EQUALS, userLoginId));
				delegator.storeByCondition("UserLoginMenuAssoc", UtilMisc.toMap("thruDate", "Y".equals(isVisible) ? nowDate : null),EntityCondition.makeCondition(conditionList,EntityOperator.AND),true);
			}
			request.setAttribute("_EVENT_MESSAGE_", "action performed successfully");

		}
		return "success";
	}

	public static String UpdateMenuSelections(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
		GenericValue userLogin = (GenericValue) request.getSession().getAttribute("userLogin");
		GenericDelegator delegator = (GenericDelegator) userLogin.getDelegator();
		List<EntityCondition> paymentList = new ArrayList<EntityCondition>();
		...
		List<GenericValue> menuLists = delegator.findList("Menu", EntityCondition.makeCondition(paymentList, EntityOperator.AND), null, orderByList,
				null, false);
		for (GenericValue menuGv : menuLists) {
			String isVisiable = request.getParameter("displayNameIsVisible") == null ? "N" : request.getParameter("displayNameIsVisible");
			Timestamp nowDate = UtilDateTime.nowTimestamp();
			if ("N".equals(isVisiable)) {
				GenericValue userLoginMenGv = delegator.create(
						"UserLoginMenuAssoc",
						UtilMisc.toMap("userLoginMenuId", delegator.getNextSeqId("UserLoginMenuAssoc"), "userLoginId",
								request.getParameter("userLoginId"), "menuId", menuGv.getString("menuId"), "fromDate", nowDate, "thruDate", null));
				delegator.createOrStore(userLoginMenGv,true);
			}

			if ("Y".equals(isVisiable) && UtilValidate.isNotEmpty(menuGv.getString("menuId"))) {
				List<EntityCondition> conditionList = new ArrayList<EntityCondition>();
				conditionList.add(EntityCondition.makeCondition("menuId", EntityOperator.EQUALS, menuGv.getString("menuId")));
				conditionList.add(EntityCondition.makeCondition("userLoginId", EntityOperator.EQUALS, request.getParameter("userLoginId")));
				delegator.storeByCondition("UserLoginMenuAssoc", UtilMisc.toMap("thruDate", "Y".equals(isVisiable) ? nowDate : null),EntityCondition.makeCondition(conditionList,EntityOperator.AND),true);
    ...
    /*Added below function to sync with HRMS*/
    /** @deprecated */
