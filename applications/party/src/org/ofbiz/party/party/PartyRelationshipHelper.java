...
public class PartyRelationshipHelper {
    ...
    public static List<String> getDepartmentPartyId(Delegator delegator, GenericValue userLogin){
    	String loginId = userLogin.getString("partyId");
    	Timestamp fromDate = UtilDateTime.nowTimestamp();
    	List<EntityCondition> condList = FastList.newInstance();
    	condList.add(EntityCondition.makeCondition("partyIdTo", loginId));
    	EntityCondition thruCond = EntityCondition.makeCondition(UtilMisc.toList(EntityCondition.makeCondition("thruDate", null),
                EntityCondition.makeCondition("thruDate", EntityOperator.GREATER_THAN, fromDate)),EntityOperator.OR);
    	condList.add(thruCond);
        EntityCondition condition = EntityCondition.makeCondition(condList);
        ...
        List<String> partyIdFromList = new ArrayList<String>();
    	for(GenericValue gv : partyRelationships){
    		partyIdFromList.add(gv.getString("partyIdFrom"));
    	}
       return partyIdFromList;
    }
}
