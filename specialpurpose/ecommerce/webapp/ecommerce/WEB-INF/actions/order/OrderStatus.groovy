...
if (orderHeader) {
    ...
    Map salesTax = new HashMap<>();
    for(GenericValue gv : orderAdjustments){
        if("SALES_TAX".equals(gv.getString("orderAdjustmentTypeId"))){
            BigDecimal amount = BigDecimal.ZERO;
            if(UtilValidate.isNotEmpty(salesTax.get("amount")))
                amount = (BigDecimal) salesTax.get("amount");
