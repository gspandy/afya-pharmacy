...
<#macro DateField formName="" fieldName="" fieldValue="" fieldClass="">
  <#if javaScriptEnabled>
    <@htmlTemplate.renderDateTimeField name="${fieldName}" event="${event!}" action="${action!}" className="${fieldClass!''}" alert="" title="Format: dd-MM-yyyy HH:mm:ss.SSS" value="${fieldValue!''}" size="25" maxlength="30" id="${fieldName}1" dateType="date" shortDateInput=false timeDropdownParamName="" defaultDateTimeString="" localizedIconTitle="" timeDropdown="" timeHourName="" classString="" hour1="" hour2="" timeMinutesName="" minutes="" isTwelveHour="" ampmName="" amSelected="" pmSelected="" compositeType="" formName=""/>
  <#else>
...
<#macro showGlAccounts glAccount>
	...
	<#if childPresent gt 0 >
		<optgroup label="${glAccount.accountName}">
			<#list childGlAccounts as childGl>
				<@showGlAccounts childGl/>
			</#list>
		</optgroup>
		<#else>
...
<#macro displayGlAccounts glAccount>
	<#assign parentGlAccount = delegator.findOne("GlAccount",true,{"glAccountId":glAccount.parentGlAccountId})>
	<option value="${glAccount.glAccountId}">${parentGlAccount.accountName} - ${glAccount.accountName}</option>
</#macro>


<#macro reportCurrency amount>
  <@ofbizCurrency amount=amount />
</#macro>
