...
function ConstructModelWindow(requestUrl, inputFieldId, dialogTarget, dialogOptionalTarget, formName, width, height, position, modal, ajaxUrl, showDescription, presentation, defaultMinLength, defaultDelay, args) {
    // add the presentation attribute to the request url to let the request know which decorator should be loaded
    if(!presentation) {
      var presentation = "layer"
    }
...
function clearTheFieldInTheForm(formElem,eleToClear){
    deptform = $(formElem).find(':input');
    $.each(deptform,function(key,tag){
        id = tag.id;
        /*Id are set in the form of underscore format automatically*/
        idArray =  id.split("_");
        /*most of the time last word in the id will be the element name that we give in the form*/
        if(idArray[idArray.length-1]==eleToClear){
           clear(tag);
        }else if(idArray[idArray.length-2]==eleToClear){/*some times it may not be the last word but previous to last word*/
            clear(tag);
        }
    });
}
...
function clear(tag){
    if(tag.value!=""){
        ...
        $.each(siblings,function(sibKey,sibTag){
            if(sibTag.id.toLowerCase().indexOf('lookupdescription') != -1){
                $('#'+sibTag.id).remove();
            }
        })
    }
