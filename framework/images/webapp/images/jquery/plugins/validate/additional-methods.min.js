...
(function() {
	...
	jQuery.validator.addMethod("rangeWords", function(b, c, d) {
		return this.optional(c) || a(b).match(/\b\w+\b/g).length >= d[0]
				&& b.match(/bw+b/g).length < d[1]
...
jQuery.validator.addMethod("civilidrange", function(a, b) {
	return this.optional(b) || /^\d{12}$/.test(a);
}, "Civil ID must be of exact 12 digit length");
jQuery.validator.addMethod("categorycoderange", function(a, b) {
	return this.optional(b) || /^\d{2}$/.test(a);
}, "Category Code must be of exact 12 digit length");
...
jQuery.validator.addMethod("positivedecimalvalue", function(a, b) {
	return this.optional(b) || /^\d+(\.(\d+))?$/.test(a);
}, "A positive decimal number please");
jQuery.validator.addMethod("quantity", function(a, b) {
	$(b).val($(b).val().replace(/,/g,''));
	return this.optional(b) || /^\d+(\.(\d+))?$/.test(a);
}, "A positive decimal number for quantity please");
jQuery.validator.addMethod("nonzeroquantity", function(a, b) {
	$(b).val($(b).val().replace(/,/g,''));
	return this.optional(b) || /^[1-9](\d+)?$/.test(a);
}, "A positive non-zero non-decimal number for quantity please");
jQuery.validator.addMethod("currency", function(a, b) {
	$(b).val($(b).val().replace(/,/g,''));
	return this.optional(b) || /^\d+(\.(\d+))?$/.test($(b).val());
}, "A positive decimal number for currency please");
jQuery.validator.addMethod("currencylimiter", function(a, b) {
	$(b).val($(b).val().replace(/,/g,''));
	return this.optional(b) || /^\d+(\.(\d{2}))?$/.test($(b).val());
}, "A positive (2 digit decimal) number for currency please");
jQuery.validator.addMethod("adjustment", function(a, b) {
	$(b).val($(b).val().replace(/,/g,''));
	return this.optional(b) || /^-?\d+(\.(\d+))?$/.test($(b).val());
}, "A positive or negative decimal number please");
jQuery.validator.addMethod("percentage", function(a, b) {
	$(b).val($(b).val().replace(/,/g,''));
	return this.optional(b) || /^\d+(\.(\d+))?$/.test($(b).val());
...
jQuery.validator.addMethod("dateITA",
		function(a, b) {
			...
			if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(a)) {
				...
				c = e.getFullYear() == d && e.getMonth() == f - 1
						&& e.getDate() == c ? true : false
			} else
				c = false;
...
jQuery.validator.addMethod("time12h", function(a, b) {
	return this.optional(b)
			|| /^((0?[1-9]|1[012])(:[0-5]\d){0,2}(\ [AP]M))$/i.test(a)
...
jQuery.validator.addMethod("mobileUK", function(a, b) {
	return this.optional(b) || a.length > 9
			&& a.match(/^((0|\+44)7(5|6|7|8|9){1}\d{2}\s?\d{6})$/)
...
jQuery.validator.addMethod("creditcardtypes", function(a, b, c) {
	...
	if (c.all)
		b = 255;
	if (b & 1 && /^(51|52|53|54|55)/.test(a))
		return a.length == 16;
	if (b & 2 && /^(4)/.test(a))
		return a.length == 16;
	if (b & 4 && /^(34|37)/.test(a))
		return a.length == 15;
	if (b & 8 && /^(300|301|302|303|304|305|36|38)/.test(a))
		return a.length == 14;
	if (b & 16 && /^(2014|2149)/.test(a))
		return a.length == 15;
	if (b & 32 && /^(6011)/.test(a))
		return a.length == 16;
	if (b & 64 && /^(3)/.test(a))
		return a.length == 16;
	if (b & 64 && /^(2131|1800)/.test(a))
		return a.length == 15;
	if (b & 128)
		return true;
	return false
