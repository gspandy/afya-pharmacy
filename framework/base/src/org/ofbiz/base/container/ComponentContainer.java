...
public class ComponentContainer implements Container {
	...
	protected Classpath classPath = new Classpath(System
	...
	public void init(String[] args, String configFile)
		...
		ContainerConfig.Container cc = ContainerConfig.getContainer(
				"component-container", configFileLocation);
		...
		if (cc.getProperty("update-classpath") != null) {
			updateClassPath = "true".equalsIgnoreCase(cc
					.getProperty("update-classpath").value);
		}
	...
	public synchronized void loadComponents(String loaderConfig,
			boolean updateClasspath) throws AlreadyLoadedException,
			ComponentException {
		...
		if (!loaded) {
			...
			throw new AlreadyLoadedException(
					"Components already loaded, cannot start");
		}

		// get the components to load
		List<ComponentLoaderConfig.ComponentDef> components = ComponentLoaderConfig
				.getRootComponents(loaderConfig);

		String parentPath;
		try {
			parentPath = FileUtil.getFile(System.getProperty("ofbiz.home"))
		...
		if (!parentPath.exists() || !parentPath.isDirectory()) {
			...
			if (componentLoadConfig != null && componentLoadConfig.exists()) {
				...
				try {
					...
					if (componentsToLoad != null) {
						for (ComponentLoaderConfig.ComponentDef def : componentsToLoad) {
							this.loadComponentFromConfig(parentPath.toString(),
									def);
						}
					}
				} catch (MalformedURLException e) {
					Debug.logError(e,
							"Unable to locate URL for component loading file: "
									+ componentLoadConfig.getAbsolutePath(),
							module);
				} catch (ComponentException e) {
				...
				for (String sub : parentPath.list()) {
					try {
						File componentPath = FileUtil.getFile(parentPath
								.getCanonicalPath()
								+ "/" + sub);
						if (componentPath.isDirectory() && !sub.equals("CVS")
								&& !sub.equals(".svn")) {
							// make sure we have a component configuraton file
							String componentLocation = componentPath
									.getCanonicalPath();
							File configFile = FileUtil
									.getFile(componentLocation
											+ "/ofbiz-component.xml");
	...
	private void loadComponent(ComponentConfig config) {
		...
		Debug.logInfo(
				"Loading component : [" + config.getComponentName() + "]",
				module);
		List<ComponentConfig.ClasspathInfo> classpathInfos = config
				.getClasspathInfos();
		...
		if (classpathInfos != null) {
			for (ComponentConfig.ClasspathInfo cp : classpathInfos) {
				...
				if ("dir".equals(cp.type)) {
					...
					Debug.logError("Classpath type '" + cp.type
	...
	/**
	 * Static method for easy loading of components for use when the container
	 * system is not.
	 * 
	 * @param updateClasspath
	 *            Tells the component loader to update the classpath, and thread
	 *            classloader
	 * @throws AlreadyLoadedException
	 * @throws ComponentException
	 */
	public static synchronized void loadComponents(boolean updateClasspath)
			throws AlreadyLoadedException, ComponentException {
