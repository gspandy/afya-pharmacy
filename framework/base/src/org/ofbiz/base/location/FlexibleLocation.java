...
public class FlexibleLocation {
    ...
    protected static String componentResolverForTenantName = ComponentLocationResolverForTenant.class.getName();
	...
	static {
		...
		defaultResolvers.put("component", componentResolverForTenantName);
	}

	/**
	 * Resolves the gives location into a URL object for use in various ways.
	 * 
	 * The general format of the location is like a URL:
	 * {locationType}://location/path/file.ext
	 * 
	 * Supports standard locationTypes like http, https, ftp, jar, & file
	 * Supports a classpath location type for when desired to be used like any
	 * other URL Supports OFBiz specific location types like ofbizhome and
	 * component Supports additional locationTypes specified in the
	 * locationresolvers.properties file
	 * 
	 * @param location
	 *            The location String to parse and create a URL from
	...
	public static URL resolveLocation(String location) throws MalformedURLException {
		URL url = null;
		try {
			url = resolveLocation(location, null);
		} catch (Throwable t) {
			url = findResourceInClasspath(location);
		}
		return url;
	}

	public static URL findResourceInClasspath(String location) throws MalformedURLException{
		URL url =null;
		if(location.startsWith("/"))
			location=location.substring(1);
		if(location.indexOf("://")!=-1){
		url = Thread.currentThread().getContextClassLoader().getResource("applications/" + location.substring(location.indexOf("://") + 3));
		if (url == null)
			url = Thread.currentThread().getContextClassLoader().getResource("framework/" + location.substring(location.indexOf("://") + 3));
		if (url == null)
			url = Thread.currentThread().getContextClassLoader()
					.getResource("hot-deploy/" + location.substring(location.indexOf("://") + 3));
		if (url == null)
			url = Thread.currentThread().getContextClassLoader()
					.getResource("themes/" + location.substring(location.indexOf("://") + 3));
		if (url == null)
			url = Thread.currentThread().getContextClassLoader()
					.getResource("specialpurpose/" + location.substring(location.indexOf("://") + 3));
		return url;
		}else{
			 url = Thread.currentThread().getContextClassLoader().getResource("applications/" + location);
			 if(url==null)
			 url = Thread.currentThread().getContextClassLoader().getResource("framework/" + location);
			 if(url==null)
			 url = Thread.currentThread().getContextClassLoader().getResource("hot-deploy/" + location);
		}
				...
				if (resolver == null) {
					...
					try {
						...
						try {
							...
							throw new MalformedURLException("Error loading Location Resolver class \"" + locationResolverName + "\": "
									+ e.toString());
						}
	 ...
	 * generally is all text before the first ":" character. If no type
	 * descriptor is found, defaults to "classpath".
	 */
	...
	public static String stripLocationType(String location) {
		...
		// now remove any extra forward slashes, ie as long as the first two are
		// forward slashes remove the first one
