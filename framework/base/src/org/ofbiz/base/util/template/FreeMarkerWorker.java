...
public class FreeMarkerWorker {
    ...
    public static Environment renderTemplate(Template template, Map<String, Object> context, Appendable outWriter) throws TemplateException, IOException {
        ...
        //Begin SME Business Basics Specific : Pradyumna - 08/11/2008 - Adding Http Objects into FreemakerContext and Freemarker objects.For Displaytag to work.
        BeansWrapper wrapper = BeansWrapper.getDefaultInstance();
        ServletContext servletCtx = (ServletContext) context.get("application");
        if(servletCtx!=null)
        {
            HttpServlet servlet =(HttpServlet) servletCtx.getAttribute("CONTROL_SERVLET");
           
            context.put(FreemarkerServlet.KEY_APPLICATION, new ServletContextHashModel(servlet, wrapper));
            context.put(FreemarkerServlet.KEY_SESSION, new HttpSessionHashModel((HttpSession) context.get("session"), wrapper));
            ...
            context.put("authz",servletCtx.getAttribute("authz"));
            context.put("security",servletCtx.getAttribute("security"));
            context.put(FreemarkerServlet.KEY_REQUEST, new HttpRequestHashModel(req, (HttpServletResponse) context.get("response"), wrapper));
        }
        //End SME Business Basics Specific : Pradyumna - 08/11/2008 - Adding Http Objects into FreemakerContext and Freemarker objects.For Displaytag to work.
