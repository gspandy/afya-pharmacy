...
public class UtilDateTime {
	...
	public static final String LOCALIZED_DATE_FORMAT = UtilProperties.getPropertyValue("general","date.localized.format", "yyyy-MM-dd");
    ...
    public static int getIntervalInDays(java.sql.Date from, java.sql.Date thru) {
        return thru != null ? (int) ((getDayEnd(new Timestamp(thru.getTime())).getTime() - getDayStart(new Timestamp(from.getTime())).getTime()) / (24 * 60 * 60 * 1000)) : 0;
    }
    ...
    public static Date getDayStart(Date stamp) {
        Timestamp dayStart = getDayStart(new Timestamp(stamp.getTime()),0);
        return dayStart;
    }
     ...
     *         minute, and second.
     */
    public static java.util.Date toDate(String monthStr, String dayStr, String yearStr, String hourStr, String minuteStr,
            String secondStr) {
    ...
    public static Timestamp getMonthEnd(Timestamp stamp, TimeZone timeZone, Locale locale) {
        Calendar tempCal = toCalendar(stamp, timeZone, locale);
        tempCal.set(tempCal.get(Calendar.YEAR), tempCal.get(Calendar.MONTH), tempCal.getActualMaximum(Calendar.DAY_OF_MONTH), 0,
                0, 0);
        return getDayEnd(new Timestamp(tempCal.getTimeInMillis()), timeZone, locale);
    }
    ...
    public static Timestamp getYearStart(Timestamp stamp, int daysLater, int monthsLater, int yearsLater, TimeZone timeZone,
            Locale locale) {
        Calendar tempCal = toCalendar(stamp, timeZone, locale);
    ...
    public static String getFormattedDate(Object date) {
        ...
        try {
            formattedDate = SIMPLE_DATE_FORMAT.parse(date.toString());
    ...
    public static String getFormattedDateTime(Object date) {
        Date formattedDate = null;
        try {
            formattedDate = SIMPLE_DATE_TIME_FORMAT.parse(date.toString());
        } catch (ParseException e) {
            return null;
        }
    ...
    public static Timestamp getDate(String dateString) {
    	Timestamp toConvert = null;
    	SimpleDateFormat df = new SimpleDateFormat(DATE_TIME_FORMAT);
    	try {
    		Date d = df.parse(dateString);
    		toConvert = new Timestamp(d.getTime());
    	} catch (ParseException e) {
    ...
    public static double getIntervalInMonths(Date d1, Date d2) {  
	    return (d2.getTime() - d1.getTime()) / AVERAGE_MILLIS_PER_MONTH;   
	}

    public static Date addYearsToTimestamp(Date d, int years) {
    ...
    public static Timestamp dateStringToTimestampParser(String date){
    ...
    public static Timestamp dateStringToTimestampParser(String date,String format){
    ...
    public static boolean isTimestampWithinRange(Timestamp dateToCheck,Timestamp startDate,Timestamp endDate) {
        return  dateToCheck.before(endDate)&& dateToCheck.after(startDate) ;
    }

    public static boolean isPastMonth(java.sql.Date toCheck){
        ...
        java.sql.Date firstDayOfTheMonth = new java.sql.Date(calendar.getTimeInMillis());
        return toCheck.before(firstDayOfTheMonth);
    }
    ...
    public static Timestamp getPreviousYear(Date date){
        java.util.Calendar calendar = java.util.Calendar.getInstance();
        calendar.setTime(date);
