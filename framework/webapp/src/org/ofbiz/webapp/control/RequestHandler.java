...
public class RequestHandler {
    ...
    public void doRequest(HttpServletRequest request, HttpServletResponse response, String chain,
        ...
        if (chain != null) {
            ...
            if (Debug.infoOn())
                Debug.logInfo("[RequestHandler]: Chain in place: requestUri=" + chainRequestUri + " overrideViewUri=" + overrideViewUri + " sessionId=" + UtilHttp.getSessionId(request), module);
        } else {
        ...
        if (eventReturnBasedRequestResponse != null) {
            ...
            if (Debug.verboseOn())
                Debug.logVerbose("[Response Qualified]: " + eventReturnBasedRequestResponse.name + ", " + eventReturnBasedRequestResponse.type + ":" + eventReturnBasedRequestResponse.value + " sessionId=" + UtilHttp.getSessionId(request), module);
        ...
        if (Debug.verboseOn())
            Debug.logVerbose("[RequestHandler]: previousRequest - " + previousRequest + " (" + loginPass + ")" + " sessionId=" + UtilHttp.getSessionId(request), module);
        ...
        if (previousRequest != null && loginPass != null && loginPass.equalsIgnoreCase("TRUE")) {
            ...
            if ("logout".equals(previousRequest) || "/logout".equals(previousRequest) || "login".equals(previousRequest) || "/login".equals(previousRequest) || "checkLogin".equals(previousRequest) || "/checkLogin".equals(previousRequest) || "/checkLogin/login".equals(previousRequest)) {
                ...
                if ("myportal".equals(cname)) {
                    String username = request.getParameter("USERNAME");
                    Iterator<GenericValue> securityGroups = ((OFBizSecurity) session.getAttribute("security")).findUserLoginSecurityGroupByUserLoginId(username);
                    StringBuilder controlPath = new StringBuilder("/");
                    controlPath.append(findApplicationUserHasAccessTo(securityGroups));
                    request.setAttribute("_CONTROL_PATH_", controlPath.toString());
                }
                String url = makeLink(request, response, redirectTarget);
                if ("myportal".equals(cname)) {
                    url = url + "?" + LoginWorker.EXTERNAL_LOGIN_KEY_ATTR + "=" + LoginWorker.getExternalLoginKey(request);
                }
                callRedirect(url, response, request);
        ...
        if ("myportal".equals(cname) && "Y".equalsIgnoreCase(request.getParameter("back"))) {
            if(userLogin==null)
        ...
        String url = makeLink(request, response, "main");
        if ("myportal".equals(cname)) {
            url = url + "?" + LoginWorker.EXTERNAL_LOGIN_KEY_ATTR + "=" + LoginWorker.getExternalLoginKey(request);
        }
            callRedirect(url, response, request);
    ...
    private StringBuilder findApplicationUserHasAccessTo(Iterator<GenericValue> securityGroups) {
        StringBuilder controlPath = new StringBuilder();
        while(securityGroups.hasNext()){
            GenericValue secGroup = securityGroups.next();
            String group = secGroup.getString("groupId");
            if(group.indexOf("ORDER")!=-1||group.indexOf("FLEX")!=-1||group.indexOf("FULL")!=-1||group.indexOf("BIZ")!=-1
                    ||group.indexOf("SALES")!=-1){
                controlPath.append("ordermgr").append("/control");
                break;
            }else if(group.indexOf("ACCTG")!=-1){
                controlPath.append("accounting").append("/control");
                break;
            }else if(group.indexOf("HRMS")!=-1 || group.indexOf("HUMAN")!=-1){
                controlPath.append("hrms").append("/control");
                break;
            }else if(group.indexOf("FACILITY")!=-1 || group.indexOf("STORE")!=-1){
                controlPath.append("facility").append("/control");
                break;
            }else if(group.indexOf("CATALOG")!=-1){
                controlPath.append("catalog").append("/control");
                break;
            }else if(group.indexOf("CRM")!=-1||group.indexOf("CSR")!=-1||group.indexOf("LEAD")!=-1){
                controlPath.append("sfaext").append("/control");
                break;
            }else if(group.indexOf("PROJECT")!=-1){
                controlPath.append("projectmgr").append("/control");
                break;
            }else if(group.indexOf("ISSUE")!=-1){
                controlPath.append("issuetracking").append("/control");
                break;
            }else if(group.indexOf("SME")!=-1){
                controlPath.append("ordermgr").append("/control");
                break;
            }else if(group.indexOf("MANU")!=-1){
            controlPath.append("manufacturing").append("/control");
            break;
        }
        }
        return controlPath;
