...
public class ModelMenu extends ModelWidget {
   ...
   // ===== CONSTRUCTORS =====
    ...
    public ModelMenu(Element menuElement) {
        ...
        String parentMenuName = menuElement.getAttribute("name");
        ...
        for (Element itemElement : itemElements) {
        	//Begin project name specified : Nafis - 27-10-10 - Restrict menu items
        	String menuItemName = itemElement.getAttribute("name");
        	try {
				menuItemRenderer(parentMenuName,menuItemName,itemElement,delegator);
			} catch (GenericEntityException e) {
			}
        	//End project name specified : Nafis - 27-10-10 - Restrict menu items
        }
    }
    
    private void menuItemRenderer(String parentMenuName,String menuItemName,Element itemElement,GenericDelegator delegator) throws GenericEntityException{
    	GenericValue gvMenu = EntityUtil.getFirst(delegator.findByAnd("Menu", UtilMisc.toMap("menuItemName",menuItemName,"menuName",parentMenuName),null,false));
		if (gvMenu != null) {
			if (!(gvMenu.getString("displayNameIsVisible").equalsIgnoreCase("N"))) {
				HttpServletRequest request=	Infrastructure.getRequest();
				GenericValue userLogin = (GenericValue) request.getSession().getAttribute("userLogin");
				List<GenericValue> userLoginMenuAssco = delegator.findByAnd("UserLoginMenuAssoc",UtilMisc.toMap("menuId", gvMenu.getString("menuId"), "userLoginId",userLogin.getString("userLoginId"),"thruDate", null),null,false);
				if (UtilValidate.isEmpty(userLoginMenuAssco)) {
					ModelMenuItem modelMenuItem = new ModelMenuItem(itemElement, this);
					modelMenuItem = this.addUpdateMenuItem(modelMenuItem);
				}
			}
		} else {
			delegator.createOrStore(delegator.makeValue("Menu", UtilMisc.toMap("menuId", delegator.getNextSeqId("Menu"), "menuName", parentMenuName,
					"menuItemName", menuItemName, "displayNameIsVisible", "Y")));
			ModelMenuItem modelMenuItem = new ModelMenuItem(itemElement, this);
			modelMenuItem = this.addUpdateMenuItem(modelMenuItem);
		}
    }
