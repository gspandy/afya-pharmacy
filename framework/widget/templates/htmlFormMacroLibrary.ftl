...
<#macro renderDateTimeField name className alert title value size maxlength id dateType shortDateInput timeDropdownParamName defaultDateTimeString localizedIconTitle timeDropdown timeHourName classString hour1 hour2 timeMinutesName minutes isTwelveHour ampmName amSelected pmSelected compositeType formName mask="" event="" action="" step="" timeValues="">
  <span class="view-calendar">
    <#if dateType!="time" >
       <input type="text" name="${name}_i18n"  class="dateITA"<#rt/>
        <#if title?has_content><#if shortDateInput?exists && shortDateInput>title="Format: dd/MM/yyyy"<#else>title="Format: dd/MM/yyyy HH:mm:ss AM/PM"</#if></#if>
              ...
              if (Date.CultureInfo != undefined) {
                  ...
                  jQuery("#${id}").change(function() {
                      ...
                      if (this.value != "") {
                          ...
                          <#-- Date.CultureInfo.formatPatterns.shortDate using date-en-US.js -->
                          var dateFormat = Date.CultureInfo.formatPatterns.shortDate<#if shortDateInput?exists && !shortDateInput> + " " + Date.CultureInfo.formatPatterns.longTime</#if>;
...
<#macro renderFormatFieldRowTitleCellOpen style>
    <td class="<#if style?has_content>${style}</#if>">
</#macro>
...
<input type="hidden" <@renderClass className alert /><#if name?has_content> name="${name}"</#if><#if value?has_content> value="${value}"</#if><#if size?has_content> size="${size}"</#if><#if maxlength?has_content> maxlength="${maxlength}"</#if><#if id?has_content> id="${id}"</#if><#rt/><#if readonly?has_content && readonly> readonly="readonly"</#if><#rt/><#if event?has_content && action?has_content> ${event}="${action}"</#if><#rt/><#if autocomplete?has_content> autocomplete="off"</#if>/><#rt/>
<#else>
...
<#macro renderModelWindow name formName fieldFormName className="" alert="false" value="" size="" maxlength="" id="" event="" action="" readonly=false autocomplete="" descriptionFieldName="" targetParameterIter="" imgSrc="" ajaxUrl="" ajaxEnabled=javaScriptEnabled presentation="layer" width="" height="" position="" fadeBackground="true" clearText="" showDescription="" initiallyCollapsed="" lastViewName="main" >
<#if Static["org.ofbiz.widget.ModelWidget"].widgetBoundaryCommentsEnabled(context)>
<#-- @renderLookupField -->
</#if>
<#if (!ajaxUrl?has_content) && ajaxEnabled>
...
<#else>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function(){
            new ConstructModelWindow("${fieldFormName}", "${id}", document.${formName?html}.${name?html}, <#if descriptionFieldName?has_content>document.${formName?html}.${descriptionFieldName}<#else>null</#if>, "${formName?html}", "${width}", "${height}", "${position}", "${fadeBackground}", <#if ajaxEnabled?has_content && ajaxEnabled>"${ajaxUrl}", ${showDescription}<#else>"", false</#if>, "${presentation!}", "${defaultMinLength!2}", "${defaultDelay!300}"<#rt/>
    <#if targetParameterIter?has_content>
      <#assign isFirst = true>
      <#lt/>, [<#rt/>
      <#lt/>${targetParameterIter}<#rt/>
      <#-- <#assign targetParameterList=Static["java.util.Arrays"].asList(targetParameterIter)>
      <#list targetParameterList as item>
        <#if isFirst>
          <#lt/>${item}<#rt/>
          <#assign isFirst = false>
        <#else>
...
<#macro makeHiddenFormLinkForm actionUrl name parameters targetWindow>
	<form method="post" action="${actionUrl}" <#if targetWindow?has_content>target="${targetWindow}"</#if> onsubmit="javascript:submitFormDisableSubmits(this)" name="${name}">
		<#list parameters as parameter><input name="${parameter.name}" value="${parameter.value}" type="hidden"/></#list>
	</form>
</#macro>
...
<#macro renderDateTimeFieldJsMethod name className alert title value size maxlength id dateType shortDateInput timeDropdownParamName defaultDateTimeString localizedIconTitle timeDropdown timeHourName classString hour1 hour2 timeMinutesName minutes isTwelveHour ampmName amSelected pmSelected compositeType formName javaScriptMethod mask="" event="" action="" step="" timeValues="">
  <span class="view-calendar">
    <#if dateType!="time" >
       <input type="text" name="${name}_i18n"  class="dateITA"<#rt/>
        <#if javaScriptMethod?exists>onChange="${javaScriptMethod}"</#if>
        <#if title?has_content><#if shortDateInput?exists && shortDateInput>title="Format: dd/MM/yyyy"<#else>title="Format: dd/MM/yyyy HH:mm:ss AM/PM"</#if></#if>
        <#if value?has_content> value="${value}"</#if>
        <#if size?has_content> size="${size}"</#if><#rt/>
        <#if maxlength?has_content>  maxlength="${maxlength}"</#if>
